...
public final class VkVideoCapabilityFlagsKHR {
    ...
    public static String explain(@enumtype(VkVideoCapabilityFlagsKHR.class) int flags) {
        ...
        if ((flags & VK_VIDEO_CAPABILITY_PROTECTED_CONTENT_BIT_KHR) != 0) {
            if (!sb.isEmpty()) {
                sb.append(" | ");
            }
            sb.append("VK_VIDEO_CAPABILITY_PROTECTED_CONTENT_BIT_KHR");
        }

        if ((flags & VK_VIDEO_CAPABILITY_SEPARATE_REFERENCE_IMAGES_BIT_KHR) != 0) {
            if (!sb.isEmpty()) {
                sb.append(" | ");
            }
            sb.append("VK_VIDEO_CAPABILITY_SEPARATE_REFERENCE_IMAGES_BIT_KHR");
        }
