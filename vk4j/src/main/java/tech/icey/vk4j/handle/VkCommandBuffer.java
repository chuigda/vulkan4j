...
/// @see <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkCommandBuffer.html">VkCommandBuffer</a>
public record VkCommandBuffer(MemorySegment segment) implements IPointer {
    public record Buffer(MemorySegment segment) {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public VkCommandBuffer read() {
            return new VkCommandBuffer(readRaw());
        }

        public VkCommandBuffer read(long index) {
            return new VkCommandBuffer(readRaw(index));
        }
        ...
        public VkCommandBuffer[] readAll() {
            VkCommandBuffer[] handles = new VkCommandBuffer[(int)size()];
        ...
        public void write(VkCommandBuffer value) {
            writeRaw(value.segment());
        }

        public void write(long index, VkCommandBuffer value) {
