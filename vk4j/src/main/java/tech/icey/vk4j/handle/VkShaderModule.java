...
/// @see <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkShaderModule.html">VkShaderModule</a>
public record VkShaderModule(MemorySegment segment) implements IPointer {
    public record Buffer(MemorySegment segment) {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public VkShaderModule read() {
            return new VkShaderModule(readRaw());
        }

        public VkShaderModule read(long index) {
            return new VkShaderModule(readRaw(index));
        }
        ...
        public VkShaderModule[] readAll() {
            VkShaderModule[] handles = new VkShaderModule[(int)size()];
        ...
        public void write(VkShaderModule value) {
            writeRaw(value.segment());
        }

        public void write(long index, VkShaderModule value) {
