...
/// @see <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkCommandPool.html">VkCommandPool</a>
public record VkCommandPool(MemorySegment segment) implements IPointer {
    public record Buffer(MemorySegment segment) {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public VkCommandPool read() {
            return new VkCommandPool(readRaw());
        }

        public VkCommandPool read(long index) {
            return new VkCommandPool(readRaw(index));
        }
        ...
        public VkCommandPool[] readAll() {
            VkCommandPool[] handles = new VkCommandPool[(int)size()];
        ...
        public void write(VkCommandPool value) {
            writeRaw(value.segment());
        }

        public void write(long index, VkCommandPool value) {
