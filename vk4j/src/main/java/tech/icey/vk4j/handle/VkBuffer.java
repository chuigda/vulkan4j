...
/// @see <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkBuffer.html">VkBuffer</a>
public record VkBuffer(MemorySegment segment) implements IPointer {
    public record Buffer(MemorySegment segment) {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public VkBuffer read() {
            return new VkBuffer(readRaw());
        }

        public VkBuffer read(long index) {
            return new VkBuffer(readRaw(index));
        }
        ...
        public VkBuffer[] readAll() {
            VkBuffer[] handles = new VkBuffer[(int)size()];
        ...
        public void write(VkBuffer value) {
            writeRaw(value.segment());
        }

        public void write(long index, VkBuffer value) {
