...
/// @see <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkPhysicalDevice.html">VkPhysicalDevice</a>
public record VkPhysicalDevice(MemorySegment segment) implements IPointer {
    public record Buffer(MemorySegment segment) {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public VkPhysicalDevice read() {
            return new VkPhysicalDevice(readRaw());
        }

        public VkPhysicalDevice read(long index) {
            return new VkPhysicalDevice(readRaw(index));
        }
        ...
        public VkPhysicalDevice[] readAll() {
            VkPhysicalDevice[] handles = new VkPhysicalDevice[(int)size()];
        ...
        public void write(VkPhysicalDevice value) {
            writeRaw(value.segment());
        }

        public void write(long index, VkPhysicalDevice value) {
