...
/// @see <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkSampler.html">VkSampler</a>
public record VkSampler(MemorySegment segment) implements IPointer {
    public record Buffer(MemorySegment segment) {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public VkSampler read() {
            return new VkSampler(readRaw());
        }

        public VkSampler read(long index) {
            return new VkSampler(readRaw(index));
        }
        ...
        public VkSampler[] readAll() {
            VkSampler[] handles = new VkSampler[(int)size()];
        ...
        public void write(VkSampler value) {
            writeRaw(value.segment());
        }

        public void write(long index, VkSampler value) {
