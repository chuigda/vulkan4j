...
/// @see <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkImageView.html">VkImageView</a>
public record VkImageView(MemorySegment segment) implements IPointer {
    public record Buffer(MemorySegment segment) {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public VkImageView read() {
            return new VkImageView(readRaw());
        }

        public VkImageView read(long index) {
            return new VkImageView(readRaw(index));
        }
        ...
        public VkImageView[] readAll() {
            VkImageView[] handles = new VkImageView[(int)size()];
        ...
        public void write(VkImageView value) {
            writeRaw(value.segment());
        }

        public void write(long index, VkImageView value) {
