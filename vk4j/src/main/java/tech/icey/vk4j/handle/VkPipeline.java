...
/// @see <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkPipeline.html">VkPipeline</a>
public record VkPipeline(MemorySegment segment) implements IPointer {
    public record Buffer(MemorySegment segment) {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public VkPipeline read() {
            return new VkPipeline(readRaw());
        }

        public VkPipeline read(long index) {
            return new VkPipeline(readRaw(index));
        }
        ...
        public VkPipeline[] readAll() {
            VkPipeline[] handles = new VkPipeline[(int)size()];
        ...
        public void write(VkPipeline value) {
            writeRaw(value.segment());
        }

        public void write(long index, VkPipeline value) {
