...
/// @see <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkSemaphore.html">VkSemaphore</a>
public record VkSemaphore(MemorySegment segment) implements IPointer {
    public record Buffer(MemorySegment segment) {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public VkSemaphore read() {
            return new VkSemaphore(readRaw());
        }

        public VkSemaphore read(long index) {
            return new VkSemaphore(readRaw(index));
        }
        ...
        public VkSemaphore[] readAll() {
            VkSemaphore[] handles = new VkSemaphore[(int)size()];
        ...
        public void write(VkSemaphore value) {
            writeRaw(value.segment());
        }

        public void write(long index, VkSemaphore value) {
