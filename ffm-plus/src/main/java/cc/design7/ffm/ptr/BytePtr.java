...
/// if the given {@link MemorySegment} is {@link MemorySegment#NULL}. The constructor is still very
...
public record BytePtr(@NotNull MemorySegment segment) implements IPointer {
    ...
    /// Create a new {@link BytePtr} using the given {@link MemorySegment}. The {@code segment} must
    /// be native. {@code null} will be returned for both {@code null} and
    /// {@link MemorySegment#NULL} input.
    ///
    ...
    /// otherwise a new {@link BytePtr} that uses {@code segment} as its backing storage
    /// @throws IllegalArgumentException if the given {@link MemorySegment} is not native
    @Contract("null -> null")
