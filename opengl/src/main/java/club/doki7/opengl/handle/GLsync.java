...
/// Represents an opaque handle type {@code GLsync}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record GLsync(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code GLsync} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<GLsync> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable GLsync read() {
        ...
        public void write(@Nullable GLsync value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable GLsync read(long index) {
        ...
        public void write(long index, @Nullable GLsync value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable GLsync[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable GLsync value0, @Nullable GLsync ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocateV(Arena arena, @Nullable GLsync value0, @Nullable GLsync ...values) {
        ...
        @Override
        public @NotNull Iterator<GLsync> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<GLsync> {
