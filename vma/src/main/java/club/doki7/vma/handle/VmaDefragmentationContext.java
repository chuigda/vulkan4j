...
@ValueBasedCandidate
@UnsafeConstructor
public record VmaDefragmentationContext(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code VmaDefragmentationContext} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<VmaDefragmentationContext> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable VmaDefragmentationContext read() {
        ...
        public void write(@Nullable VmaDefragmentationContext value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable VmaDefragmentationContext read(long index) {
        ...
        public void write(long index, @Nullable VmaDefragmentationContext value) {
