...
/// Represents an opaque handle type {@code SDL_Window}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Window(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Window} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Window> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Window read() {
        ...
        public void write(@Nullable SDL_Window value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Window read(long index) {
        ...
        public void write(long index, @Nullable SDL_Window value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Window[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Window value0, @Nullable SDL_Window ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Window[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Window value0, @Nullable SDL_Window ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Window> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Window> {
            ...
            @Override
            public @Nullable SDL_Window next() {
                if (!hasNext()) {
