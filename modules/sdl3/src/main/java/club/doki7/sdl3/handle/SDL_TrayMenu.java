...
/// Represents an opaque handle type {@code SDL_TrayMenu}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_TrayMenu(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_TrayMenu} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_TrayMenu> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_TrayMenu read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_TrayMenu(s);
        }

        public void write(@Nullable SDL_TrayMenu value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_TrayMenu read(long index) {
        ...
        public void write(long index, @Nullable SDL_TrayMenu value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_TrayMenu[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_TrayMenu value0, @Nullable SDL_TrayMenu ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_TrayMenu[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_TrayMenu value0, @Nullable SDL_TrayMenu ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_TrayMenu> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_TrayMenu> {
            ...
            @Override
            public @Nullable SDL_TrayMenu next() {
                if (!hasNext()) {
