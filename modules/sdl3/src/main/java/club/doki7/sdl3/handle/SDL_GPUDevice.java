...
/// Represents an opaque handle type {@code SDL_GPUDevice}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPUDevice(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPUDevice} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPUDevice> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPUDevice read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_GPUDevice(s);
        }

        public void write(@Nullable SDL_GPUDevice value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPUDevice read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPUDevice value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPUDevice[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPUDevice value0, @Nullable SDL_GPUDevice ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPUDevice[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPUDevice value0, @Nullable SDL_GPUDevice ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPUDevice> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPUDevice> {
            ...
            @Override
            public @Nullable SDL_GPUDevice next() {
                if (!hasNext()) {
