...
/// Represents an opaque handle type {@code SDL_Sensor}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Sensor(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Sensor} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Sensor> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Sensor read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Sensor(s);
        }

        public void write(@Nullable SDL_Sensor value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Sensor read(long index) {
        ...
        public void write(long index, @Nullable SDL_Sensor value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Sensor[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Sensor value0, @Nullable SDL_Sensor ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Sensor[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Sensor value0, @Nullable SDL_Sensor ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Sensor> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Sensor> {
            ...
            @Override
            public @Nullable SDL_Sensor next() {
                if (!hasNext()) {
