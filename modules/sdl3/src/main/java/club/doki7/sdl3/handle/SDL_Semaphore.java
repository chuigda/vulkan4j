...
/// Represents an opaque handle type {@code SDL_Semaphore}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Semaphore(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Semaphore} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Semaphore> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Semaphore read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Semaphore(s);
        }

        public void write(@Nullable SDL_Semaphore value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Semaphore read(long index) {
        ...
        public void write(long index, @Nullable SDL_Semaphore value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Semaphore[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Semaphore value0, @Nullable SDL_Semaphore ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Semaphore[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Semaphore value0, @Nullable SDL_Semaphore ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Semaphore> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Semaphore> {
            ...
            @Override
            public @Nullable SDL_Semaphore next() {
                if (!hasNext()) {
