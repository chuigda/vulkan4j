...
/// Represents an opaque handle type {@code SDL_IOStream}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_IOStream(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_IOStream} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_IOStream> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_IOStream read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_IOStream(s);
        }

        public void write(@Nullable SDL_IOStream value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_IOStream read(long index) {
        ...
        public void write(long index, @Nullable SDL_IOStream value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_IOStream[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_IOStream value0, @Nullable SDL_IOStream ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_IOStream[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_IOStream value0, @Nullable SDL_IOStream ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_IOStream> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_IOStream> {
            ...
            @Override
            public @Nullable SDL_IOStream next() {
                if (!hasNext()) {
