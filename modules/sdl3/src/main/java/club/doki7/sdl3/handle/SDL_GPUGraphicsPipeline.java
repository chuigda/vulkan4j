package club.doki7.sdl3.handle;
...
/// Represents an opaque handle type {@code SDL_GPUGraphicsPipeline}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPUGraphicsPipeline(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPUGraphicsPipeline} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPUGraphicsPipeline> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPUGraphicsPipeline read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_GPUGraphicsPipeline(s);
        }

        public void write(@Nullable SDL_GPUGraphicsPipeline value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPUGraphicsPipeline read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPUGraphicsPipeline value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPUGraphicsPipeline[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPUGraphicsPipeline value0, @Nullable SDL_GPUGraphicsPipeline ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPUGraphicsPipeline[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPUGraphicsPipeline value0, @Nullable SDL_GPUGraphicsPipeline ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPUGraphicsPipeline> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPUGraphicsPipeline> {
            ...
            @Override
            public @Nullable SDL_GPUGraphicsPipeline next() {
                if (!hasNext()) {
