...
/// Represents an opaque handle type {@code SDL_hid_device}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_hid_device(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_hid_device} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_hid_device> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_hid_device read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_hid_device(s);
        }

        public void write(@Nullable SDL_hid_device value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_hid_device read(long index) {
        ...
        public void write(long index, @Nullable SDL_hid_device value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_hid_device[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_hid_device value0, @Nullable SDL_hid_device ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_hid_device[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_hid_device value0, @Nullable SDL_hid_device ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_hid_device> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_hid_device> {
            ...
            @Override
            public @Nullable SDL_hid_device next() {
                if (!hasNext()) {
