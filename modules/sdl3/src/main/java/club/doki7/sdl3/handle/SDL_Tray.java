...
/// Represents an opaque handle type {@code SDL_Tray}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Tray(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Tray} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Tray> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Tray read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Tray(s);
        }

        public void write(@Nullable SDL_Tray value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Tray read(long index) {
        ...
        public void write(long index, @Nullable SDL_Tray value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Tray[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Tray value0, @Nullable SDL_Tray ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Tray[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Tray value0, @Nullable SDL_Tray ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Tray> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Tray> {
