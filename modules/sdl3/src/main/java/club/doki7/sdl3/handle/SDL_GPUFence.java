...
/// Represents an opaque handle type {@code SDL_GPUFence}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPUFence(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPUFence} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPUFence> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPUFence read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_GPUFence(s);
        }

        public void write(@Nullable SDL_GPUFence value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPUFence read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPUFence value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPUFence[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPUFence value0, @Nullable SDL_GPUFence ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPUFence[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPUFence value0, @Nullable SDL_GPUFence ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPUFence> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPUFence> {
            ...
            @Override
            public @Nullable SDL_GPUFence next() {
                if (!hasNext()) {
