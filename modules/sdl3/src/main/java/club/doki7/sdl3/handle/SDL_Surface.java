...
/// Represents an opaque handle type {@code SDL_Surface}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Surface(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Surface} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Surface> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Surface read() {
        ...
        public void write(@Nullable SDL_Surface value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Surface read(long index) {
        ...
        public void write(long index, @Nullable SDL_Surface value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Surface[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Surface value0, @Nullable SDL_Surface ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Surface[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Surface value0, @Nullable SDL_Surface ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Surface> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Surface> {
            ...
            @Override
            public @Nullable SDL_Surface next() {
                if (!hasNext()) {
