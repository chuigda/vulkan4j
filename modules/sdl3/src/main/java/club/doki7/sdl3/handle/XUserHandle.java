...
/// Represents an opaque handle type {@code XUserHandle}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record XUserHandle(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code XUserHandle} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<XUserHandle> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable XUserHandle read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new XUserHandle(s);
        }

        public void write(@Nullable XUserHandle value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable XUserHandle read(long index) {
        ...
        public void write(long index, @Nullable XUserHandle value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable XUserHandle[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable XUserHandle value0, @Nullable XUserHandle ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable XUserHandle[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable XUserHandle value0, @Nullable XUserHandle ...values) {
        ...
        @Override
        public @NotNull Iterator<XUserHandle> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<XUserHandle> {
