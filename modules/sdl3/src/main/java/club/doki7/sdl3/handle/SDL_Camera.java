...
/// Represents an opaque handle type {@code SDL_Camera}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Camera(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Camera} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Camera> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Camera read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Camera(s);
        }

        public void write(@Nullable SDL_Camera value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Camera read(long index) {
        ...
        public void write(long index, @Nullable SDL_Camera value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Camera[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Camera value0, @Nullable SDL_Camera ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Camera[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Camera value0, @Nullable SDL_Camera ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Camera> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Camera> {
            ...
            @Override
            public @Nullable SDL_Camera next() {
                if (!hasNext()) {
