...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GLContext(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GLContext} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GLContext> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GLContext read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_GLContext(s);
        }

        public void write(@Nullable SDL_GLContext value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GLContext read(long index) {
        ...
        public void write(long index, @Nullable SDL_GLContext value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GLContext[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GLContext value0, @Nullable SDL_GLContext ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GLContext[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GLContext value0, @Nullable SDL_GLContext ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GLContext> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GLContext> {
            ...
            @Override
            public @Nullable SDL_GLContext next() {
                if (!hasNext()) {
