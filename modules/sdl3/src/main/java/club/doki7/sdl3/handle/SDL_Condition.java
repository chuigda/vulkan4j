...
/// Represents an opaque handle type {@code SDL_Condition}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Condition(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Condition} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Condition> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Condition read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Condition(s);
        }

        public void write(@Nullable SDL_Condition value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Condition read(long index) {
        ...
        public void write(long index, @Nullable SDL_Condition value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Condition[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Condition value0, @Nullable SDL_Condition ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Condition[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Condition value0, @Nullable SDL_Condition ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Condition> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Condition> {
            ...
            @Override
            public @Nullable SDL_Condition next() {
                if (!hasNext()) {
