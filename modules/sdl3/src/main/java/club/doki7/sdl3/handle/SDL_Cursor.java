...
/// Represents an opaque handle type {@code SDL_Cursor}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Cursor(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Cursor} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Cursor> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Cursor read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Cursor(s);
        }

        public void write(@Nullable SDL_Cursor value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Cursor read(long index) {
        ...
        public void write(long index, @Nullable SDL_Cursor value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Cursor[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Cursor value0, @Nullable SDL_Cursor ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Cursor[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Cursor value0, @Nullable SDL_Cursor ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Cursor> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Cursor> {
            ...
            @Override
            public @Nullable SDL_Cursor next() {
                if (!hasNext()) {
