...
/// Represents an opaque handle type {@code SDL_AsyncIO}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_AsyncIO(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_AsyncIO} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_AsyncIO> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_AsyncIO read() {
        ...
        public void write(@Nullable SDL_AsyncIO value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_AsyncIO read(long index) {
        ...
        public void write(long index, @Nullable SDL_AsyncIO value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_AsyncIO[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_AsyncIO value0, @Nullable SDL_AsyncIO ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_AsyncIO[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_AsyncIO value0, @Nullable SDL_AsyncIO ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_AsyncIO> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_AsyncIO> {
            ...
            @Override
            public @Nullable SDL_AsyncIO next() {
                if (!hasNext()) {
