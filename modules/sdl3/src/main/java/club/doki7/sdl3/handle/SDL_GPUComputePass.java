...
/// Represents an opaque handle type {@code SDL_GPUComputePass}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPUComputePass(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPUComputePass} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPUComputePass> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPUComputePass read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_GPUComputePass(s);
        }

        public void write(@Nullable SDL_GPUComputePass value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPUComputePass read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPUComputePass value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPUComputePass[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPUComputePass value0, @Nullable SDL_GPUComputePass ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPUComputePass[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPUComputePass value0, @Nullable SDL_GPUComputePass ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPUComputePass> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPUComputePass> {
            ...
            @Override
            public @Nullable SDL_GPUComputePass next() {
                if (!hasNext()) {
