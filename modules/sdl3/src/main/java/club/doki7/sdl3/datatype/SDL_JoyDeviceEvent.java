...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_JoyDeviceEvent(@NotNull MemorySegment segment) implements ISDL_JoyDeviceEvent {
    /// Represents a pointer to / an array of null structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link SDL_JoyDeviceEvent}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// ISDL_JoyDeviceEvent to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code SDL_JoyDeviceEvent.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements ISDL_JoyDeviceEvent, Iterable<SDL_JoyDeviceEvent> {
        public long size() {
            return segment.byteSize() / SDL_JoyDeviceEvent.BYTES;
        }
        ...
        public @NotNull SDL_JoyDeviceEvent at(long index) {
            return new SDL_JoyDeviceEvent(segment.asSlice(index * SDL_JoyDeviceEvent.BYTES, SDL_JoyDeviceEvent.BYTES));
        }

        public void write(long index, @NotNull SDL_JoyDeviceEvent value) {
            MemorySegment s = segment.asSlice(index * SDL_JoyDeviceEvent.BYTES, SDL_JoyDeviceEvent.BYTES);
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * SDL_JoyDeviceEvent.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * SDL_JoyDeviceEvent.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * SDL_JoyDeviceEvent.BYTES,
                (end - start) * SDL_JoyDeviceEvent.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * SDL_JoyDeviceEvent.BYTES));
        }

        public SDL_JoyDeviceEvent[] toArray() {
            SDL_JoyDeviceEvent[] ret = new SDL_JoyDeviceEvent[(int) size()];
        ...
        @Override
        public @NotNull Iterator<SDL_JoyDeviceEvent> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<SDL_JoyDeviceEvent> {
            ...
            @Override
            public SDL_JoyDeviceEvent next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                SDL_JoyDeviceEvent ret = new SDL_JoyDeviceEvent(segment.asSlice(0, SDL_JoyDeviceEvent.BYTES));
                segment = segment.asSlice(SDL_JoyDeviceEvent.BYTES);
                return ret;
            }
