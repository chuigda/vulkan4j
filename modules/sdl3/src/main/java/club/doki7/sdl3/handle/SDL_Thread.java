...
/// Represents an opaque handle type {@code SDL_Thread}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Thread(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Thread} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Thread> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Thread read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Thread(s);
        }

        public void write(@Nullable SDL_Thread value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Thread read(long index) {
        ...
        public void write(long index, @Nullable SDL_Thread value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Thread[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Thread value0, @Nullable SDL_Thread ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Thread[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Thread value0, @Nullable SDL_Thread ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Thread> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Thread> {
            ...
            @Override
            public @Nullable SDL_Thread next() {
                if (!hasNext()) {
