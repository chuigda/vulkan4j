...
/// Represents an opaque handle type {@code XTaskQueueHandle}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record XTaskQueueHandle(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code XTaskQueueHandle} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<XTaskQueueHandle> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable XTaskQueueHandle read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new XTaskQueueHandle(s);
        }

        public void write(@Nullable XTaskQueueHandle value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable XTaskQueueHandle read(long index) {
        ...
        public void write(long index, @Nullable XTaskQueueHandle value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable XTaskQueueHandle[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable XTaskQueueHandle value0, @Nullable XTaskQueueHandle ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable XTaskQueueHandle[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable XTaskQueueHandle value0, @Nullable XTaskQueueHandle ...values) {
        ...
        @Override
        public @NotNull Iterator<XTaskQueueHandle> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<XTaskQueueHandle> {
