...
/// Represents an opaque handle type {@code SDL_GPUShader}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPUShader(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPUShader} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPUShader> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPUShader read() {
        ...
        public void write(@Nullable SDL_GPUShader value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPUShader read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPUShader value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPUShader[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPUShader value0, @Nullable SDL_GPUShader ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPUShader[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPUShader value0, @Nullable SDL_GPUShader ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPUShader> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPUShader> {
            ...
            @Override
            public @Nullable SDL_GPUShader next() {
                if (!hasNext()) {
