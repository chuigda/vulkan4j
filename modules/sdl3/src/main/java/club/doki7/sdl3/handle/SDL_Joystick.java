...
/// Represents an opaque handle type {@code SDL_Joystick}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Joystick(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Joystick} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Joystick> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Joystick read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Joystick(s);
        }

        public void write(@Nullable SDL_Joystick value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Joystick read(long index) {
        ...
        public void write(long index, @Nullable SDL_Joystick value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Joystick[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Joystick value0, @Nullable SDL_Joystick ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Joystick[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Joystick value0, @Nullable SDL_Joystick ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Joystick> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Joystick> {
            ...
            @Override
            public @Nullable SDL_Joystick next() {
                if (!hasNext()) {
