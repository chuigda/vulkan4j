...
/// Represents an opaque handle type {@code SDL_AudioStream}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_AudioStream(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_AudioStream} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_AudioStream> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_AudioStream read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_AudioStream(s);
        }

        public void write(@Nullable SDL_AudioStream value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_AudioStream read(long index) {
        ...
        public void write(long index, @Nullable SDL_AudioStream value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_AudioStream[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_AudioStream value0, @Nullable SDL_AudioStream ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_AudioStream[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_AudioStream value0, @Nullable SDL_AudioStream ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_AudioStream> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_AudioStream> {
            ...
            @Override
            public @Nullable SDL_AudioStream next() {
                if (!hasNext()) {
