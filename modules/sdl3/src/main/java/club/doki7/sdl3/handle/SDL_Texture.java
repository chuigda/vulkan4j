...
/// Represents an opaque handle type {@code SDL_Texture}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Texture(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Texture} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Texture> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Texture read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Texture(s);
        }

        public void write(@Nullable SDL_Texture value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Texture read(long index) {
        ...
        public void write(long index, @Nullable SDL_Texture value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Texture[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Texture value0, @Nullable SDL_Texture ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Texture[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Texture value0, @Nullable SDL_Texture ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Texture> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Texture> {
            ...
            @Override
            public @Nullable SDL_Texture next() {
                if (!hasNext()) {
