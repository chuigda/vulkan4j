...
/// Represents an opaque handle type {@code SDL_Renderer}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Renderer(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Renderer} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Renderer> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Renderer read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Renderer(s);
        }

        public void write(@Nullable SDL_Renderer value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Renderer read(long index) {
        ...
        public void write(long index, @Nullable SDL_Renderer value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Renderer[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Renderer value0, @Nullable SDL_Renderer ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Renderer[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Renderer value0, @Nullable SDL_Renderer ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Renderer> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Renderer> {
            ...
            @Override
            public @Nullable SDL_Renderer next() {
                if (!hasNext()) {
