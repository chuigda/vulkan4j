...
/// Represents an opaque handle type {@code SDL_GPUCommandBuffer}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPUCommandBuffer(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPUCommandBuffer} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPUCommandBuffer> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPUCommandBuffer read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_GPUCommandBuffer(s);
        }

        public void write(@Nullable SDL_GPUCommandBuffer value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPUCommandBuffer read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPUCommandBuffer value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPUCommandBuffer[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPUCommandBuffer value0, @Nullable SDL_GPUCommandBuffer ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPUCommandBuffer[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPUCommandBuffer value0, @Nullable SDL_GPUCommandBuffer ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPUCommandBuffer> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPUCommandBuffer> {
            ...
            @Override
            public @Nullable SDL_GPUCommandBuffer next() {
                if (!hasNext()) {
