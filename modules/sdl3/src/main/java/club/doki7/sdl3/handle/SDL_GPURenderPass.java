...
/// Represents an opaque handle type {@code SDL_GPURenderPass}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPURenderPass(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPURenderPass} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPURenderPass> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPURenderPass read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_GPURenderPass(s);
        }

        public void write(@Nullable SDL_GPURenderPass value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPURenderPass read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPURenderPass value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPURenderPass[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPURenderPass value0, @Nullable SDL_GPURenderPass ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPURenderPass[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPURenderPass value0, @Nullable SDL_GPURenderPass ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPURenderPass> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPURenderPass> {
            ...
            @Override
            public @Nullable SDL_GPURenderPass next() {
                if (!hasNext()) {
