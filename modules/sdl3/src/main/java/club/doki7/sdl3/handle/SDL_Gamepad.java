...
/// Represents an opaque handle type {@code SDL_Gamepad}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Gamepad(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Gamepad} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Gamepad> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Gamepad read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Gamepad(s);
        }

        public void write(@Nullable SDL_Gamepad value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Gamepad read(long index) {
        ...
        public void write(long index, @Nullable SDL_Gamepad value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Gamepad[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Gamepad value0, @Nullable SDL_Gamepad ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Gamepad[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Gamepad value0, @Nullable SDL_Gamepad ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Gamepad> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Gamepad> {
            ...
            @Override
            public @Nullable SDL_Gamepad next() {
                if (!hasNext()) {
