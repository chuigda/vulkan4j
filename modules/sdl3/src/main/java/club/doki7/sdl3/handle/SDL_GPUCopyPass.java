...
/// Represents an opaque handle type {@code SDL_GPUCopyPass}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPUCopyPass(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPUCopyPass} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPUCopyPass> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPUCopyPass read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_GPUCopyPass(s);
        }

        public void write(@Nullable SDL_GPUCopyPass value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPUCopyPass read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPUCopyPass value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPUCopyPass[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPUCopyPass value0, @Nullable SDL_GPUCopyPass ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPUCopyPass[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPUCopyPass value0, @Nullable SDL_GPUCopyPass ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPUCopyPass> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPUCopyPass> {
            ...
            @Override
            public @Nullable SDL_GPUCopyPass next() {
                if (!hasNext()) {
