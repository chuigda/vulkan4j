...
/// Represents an opaque handle type {@code SDL_GPUTexture}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPUTexture(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPUTexture} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPUTexture> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPUTexture read() {
        ...
        public void write(@Nullable SDL_GPUTexture value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPUTexture read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPUTexture value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPUTexture[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPUTexture value0, @Nullable SDL_GPUTexture ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPUTexture[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPUTexture value0, @Nullable SDL_GPUTexture ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPUTexture> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPUTexture> {
            ...
            @Override
            public @Nullable SDL_GPUTexture next() {
                if (!hasNext()) {
