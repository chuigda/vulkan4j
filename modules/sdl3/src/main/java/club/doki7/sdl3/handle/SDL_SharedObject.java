...
/// Represents an opaque handle type {@code SDL_SharedObject}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_SharedObject(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_SharedObject} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_SharedObject> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_SharedObject read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_SharedObject(s);
        }

        public void write(@Nullable SDL_SharedObject value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_SharedObject read(long index) {
        ...
        public void write(long index, @Nullable SDL_SharedObject value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_SharedObject[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_SharedObject value0, @Nullable SDL_SharedObject ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_SharedObject[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_SharedObject value0, @Nullable SDL_SharedObject ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_SharedObject> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_SharedObject> {
            ...
            @Override
            public @Nullable SDL_SharedObject next() {
                if (!hasNext()) {
