...
/// Represents an opaque handle type {@code SDL_GPUBuffer}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPUBuffer(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPUBuffer} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPUBuffer> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPUBuffer read() {
        ...
        public void write(@Nullable SDL_GPUBuffer value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPUBuffer read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPUBuffer value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPUBuffer[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPUBuffer value0, @Nullable SDL_GPUBuffer ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPUBuffer[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPUBuffer value0, @Nullable SDL_GPUBuffer ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPUBuffer> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPUBuffer> {
            ...
            @Override
            public @Nullable SDL_GPUBuffer next() {
                if (!hasNext()) {
