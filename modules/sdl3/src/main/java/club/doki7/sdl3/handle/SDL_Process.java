...
/// Represents an opaque handle type {@code SDL_Process}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Process(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Process} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Process> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Process read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Process(s);
        }

        public void write(@Nullable SDL_Process value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Process read(long index) {
        ...
        public void write(long index, @Nullable SDL_Process value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Process[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Process value0, @Nullable SDL_Process ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Process[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Process value0, @Nullable SDL_Process ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Process> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Process> {
            ...
            @Override
            public @Nullable SDL_Process next() {
                if (!hasNext()) {
