...
/// Represents an opaque handle type {@code SDL_RWLock}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_RWLock(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_RWLock} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_RWLock> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_RWLock read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_RWLock(s);
        }

        public void write(@Nullable SDL_RWLock value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_RWLock read(long index) {
        ...
        public void write(long index, @Nullable SDL_RWLock value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_RWLock[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_RWLock value0, @Nullable SDL_RWLock ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_RWLock[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_RWLock value0, @Nullable SDL_RWLock ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_RWLock> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_RWLock> {
            ...
            @Override
            public @Nullable SDL_RWLock next() {
                if (!hasNext()) {
