...
/// Represents an opaque handle type {@code SDL_Mutex}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Mutex(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Mutex} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Mutex> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Mutex read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Mutex(s);
        }

        public void write(@Nullable SDL_Mutex value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Mutex read(long index) {
        ...
        public void write(long index, @Nullable SDL_Mutex value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Mutex[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Mutex value0, @Nullable SDL_Mutex ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Mutex[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Mutex value0, @Nullable SDL_Mutex ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Mutex> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Mutex> {
            ...
            @Override
            public @Nullable SDL_Mutex next() {
                if (!hasNext()) {
