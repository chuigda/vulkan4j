...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_JoyButtonEvent(@NotNull MemorySegment segment) implements ISDL_JoyButtonEvent {
    /// Represents a pointer to / an array of null structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link SDL_JoyButtonEvent}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// ISDL_JoyButtonEvent to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code SDL_JoyButtonEvent.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements ISDL_JoyButtonEvent, Iterable<SDL_JoyButtonEvent> {
        public long size() {
            return segment.byteSize() / SDL_JoyButtonEvent.BYTES;
        }
        ...
        public @NotNull SDL_JoyButtonEvent at(long index) {
            return new SDL_JoyButtonEvent(segment.asSlice(index * SDL_JoyButtonEvent.BYTES, SDL_JoyButtonEvent.BYTES));
        }

        public void write(long index, @NotNull SDL_JoyButtonEvent value) {
            MemorySegment s = segment.asSlice(index * SDL_JoyButtonEvent.BYTES, SDL_JoyButtonEvent.BYTES);
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * SDL_JoyButtonEvent.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * SDL_JoyButtonEvent.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * SDL_JoyButtonEvent.BYTES,
                (end - start) * SDL_JoyButtonEvent.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * SDL_JoyButtonEvent.BYTES));
        }

        public SDL_JoyButtonEvent[] toArray() {
            SDL_JoyButtonEvent[] ret = new SDL_JoyButtonEvent[(int) size()];
        ...
        @Override
        public @NotNull Iterator<SDL_JoyButtonEvent> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<SDL_JoyButtonEvent> {
            ...
            @Override
            public SDL_JoyButtonEvent next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                SDL_JoyButtonEvent ret = new SDL_JoyButtonEvent(segment.asSlice(0, SDL_JoyButtonEvent.BYTES));
                segment = segment.asSlice(SDL_JoyButtonEvent.BYTES);
                return ret;
            }
    ...
    public static SDL_JoyButtonEvent allocate(Arena arena) {
        return new SDL_JoyButtonEvent(arena.allocate(LAYOUT));
    }

    public static SDL_JoyButtonEvent.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        return new SDL_JoyButtonEvent.Ptr(segment);
    }

    public static SDL_JoyButtonEvent clone(Arena arena, SDL_JoyButtonEvent src) {
        SDL_JoyButtonEvent ret = allocate(arena);
    ...
    public SDL_JoyButtonEvent type(@EnumType(SDL_EventType.class) int value) {
    ...
    public SDL_JoyButtonEvent timestamp(@NativeType("Uint64") @Unsigned long value) {
    ...
    public SDL_JoyButtonEvent which(@NativeType("SDL_JoystickID") @Unsigned int value) {
    ...
    public SDL_JoyButtonEvent button(@NativeType("Uint8") @Unsigned byte value) {
    ...
    public SDL_JoyButtonEvent down(@NativeType("boolean") boolean value) {
    ...
    public SDL_JoyButtonEvent padding1(@NativeType("Uint8") @Unsigned byte value) {
    ...
    public SDL_JoyButtonEvent padding2(@NativeType("Uint8") @Unsigned byte value) {
