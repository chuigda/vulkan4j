...
/// Represents an opaque handle type {@code SDL_TrayEntry}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_TrayEntry(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_TrayEntry} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_TrayEntry> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_TrayEntry read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_TrayEntry(s);
        }

        public void write(@Nullable SDL_TrayEntry value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_TrayEntry read(long index) {
        ...
        public void write(long index, @Nullable SDL_TrayEntry value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_TrayEntry[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_TrayEntry value0, @Nullable SDL_TrayEntry ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_TrayEntry[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_TrayEntry value0, @Nullable SDL_TrayEntry ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_TrayEntry> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_TrayEntry> {
            ...
            @Override
            public @Nullable SDL_TrayEntry next() {
                if (!hasNext()) {
