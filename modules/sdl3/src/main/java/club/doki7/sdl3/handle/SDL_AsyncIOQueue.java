...
/// Represents an opaque handle type {@code SDL_AsyncIOQueue}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_AsyncIOQueue(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_AsyncIOQueue} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_AsyncIOQueue> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_AsyncIOQueue read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_AsyncIOQueue(s);
        }

        public void write(@Nullable SDL_AsyncIOQueue value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_AsyncIOQueue read(long index) {
        ...
        public void write(long index, @Nullable SDL_AsyncIOQueue value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_AsyncIOQueue[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_AsyncIOQueue value0, @Nullable SDL_AsyncIOQueue ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_AsyncIOQueue[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_AsyncIOQueue value0, @Nullable SDL_AsyncIOQueue ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_AsyncIOQueue> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_AsyncIOQueue> {
            ...
            @Override
            public @Nullable SDL_AsyncIOQueue next() {
                if (!hasNext()) {
