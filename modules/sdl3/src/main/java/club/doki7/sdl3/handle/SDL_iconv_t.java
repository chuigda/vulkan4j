...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_iconv_t(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_iconv_t} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_iconv_t> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_iconv_t read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_iconv_t(s);
        }

        public void write(@Nullable SDL_iconv_t value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_iconv_t read(long index) {
        ...
        public void write(long index, @Nullable SDL_iconv_t value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_iconv_t[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_iconv_t value0, @Nullable SDL_iconv_t ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_iconv_t[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_iconv_t value0, @Nullable SDL_iconv_t ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_iconv_t> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_iconv_t> {
            ...
            @Override
            public @Nullable SDL_iconv_t next() {
                if (!hasNext()) {
