...
/// Represents an opaque handle type {@code SDL_Haptic}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Haptic(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Haptic} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Haptic> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Haptic read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Haptic(s);
        }

        public void write(@Nullable SDL_Haptic value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Haptic read(long index) {
        ...
        public void write(long index, @Nullable SDL_Haptic value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Haptic[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Haptic value0, @Nullable SDL_Haptic ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Haptic[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Haptic value0, @Nullable SDL_Haptic ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Haptic> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Haptic> {
            ...
            @Override
            public @Nullable SDL_Haptic next() {
                if (!hasNext()) {
