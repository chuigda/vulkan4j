...
/// Represents an opaque handle type {@code MSG}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record MSG(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code MSG} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<MSG> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable MSG read() {
        ...
        public void write(@Nullable MSG value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable MSG read(long index) {
        ...
        public void write(long index, @Nullable MSG value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable MSG[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable MSG value0, @Nullable MSG ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable MSG[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable MSG value0, @Nullable MSG ...values) {
        ...
        @Override
        public @NotNull Iterator<MSG> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<MSG> {
