...
/// Represents an opaque handle type {@code SDL_GPUComputePipeline}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_GPUComputePipeline(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_GPUComputePipeline} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_GPUComputePipeline> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_GPUComputePipeline read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_GPUComputePipeline(s);
        }

        public void write(@Nullable SDL_GPUComputePipeline value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_GPUComputePipeline read(long index) {
        ...
        public void write(long index, @Nullable SDL_GPUComputePipeline value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_GPUComputePipeline[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_GPUComputePipeline value0, @Nullable SDL_GPUComputePipeline ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_GPUComputePipeline[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_GPUComputePipeline value0, @Nullable SDL_GPUComputePipeline ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_GPUComputePipeline> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_GPUComputePipeline> {
            ...
            @Override
            public @Nullable SDL_GPUComputePipeline next() {
                if (!hasNext()) {
