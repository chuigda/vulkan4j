...
/// Represents an opaque handle type {@code SDL_Environment}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record SDL_Environment(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code SDL_Environment} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<SDL_Environment> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable SDL_Environment read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new SDL_Environment(s);
        }

        public void write(@Nullable SDL_Environment value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable SDL_Environment read(long index) {
        ...
        public void write(long index, @Nullable SDL_Environment value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable SDL_Environment[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable SDL_Environment value0, @Nullable SDL_Environment ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable SDL_Environment[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable SDL_Environment value0, @Nullable SDL_Environment ...values) {
        ...
        @Override
        public @NotNull Iterator<SDL_Environment> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<SDL_Environment> {
            ...
            @Override
            public @Nullable SDL_Environment next() {
                if (!hasNext()) {
