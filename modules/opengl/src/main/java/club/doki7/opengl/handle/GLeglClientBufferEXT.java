...
/// Represents an opaque handle type {@code GLeglClientBufferEXT}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record GLeglClientBufferEXT(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code GLeglClientBufferEXT} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<GLeglClientBufferEXT> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable GLeglClientBufferEXT read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new GLeglClientBufferEXT(s);
        }

        public void write(@Nullable GLeglClientBufferEXT value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable GLeglClientBufferEXT read(long index) {
        ...
        public void write(long index, @Nullable GLeglClientBufferEXT value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable GLeglClientBufferEXT[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable GLeglClientBufferEXT value0, @Nullable GLeglClientBufferEXT ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocateV(Arena arena, @Nullable GLeglClientBufferEXT value0, @Nullable GLeglClientBufferEXT ...values) {
        ...
        @Override
        public @NotNull Iterator<GLeglClientBufferEXT> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<GLeglClientBufferEXT> {
            ...
            @Override
            public @Nullable GLeglClientBufferEXT next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                MemorySegment s = segment.get(ValueLayout.ADDRESS, 0);
                segment = segment.asSlice(ValueLayout.ADDRESS.byteSize());
                return s.equals(MemorySegment.NULL) ? null : new GLeglClientBufferEXT(s);
            }
