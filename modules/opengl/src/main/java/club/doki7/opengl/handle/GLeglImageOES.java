...
/// Represents an opaque handle type {@code GLeglImageOES}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record GLeglImageOES(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code GLeglImageOES} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<GLeglImageOES> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable GLeglImageOES read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new GLeglImageOES(s);
        }

        public void write(@Nullable GLeglImageOES value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable GLeglImageOES read(long index) {
        ...
        public void write(long index, @Nullable GLeglImageOES value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable GLeglImageOES[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable GLeglImageOES value0, @Nullable GLeglImageOES ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable GLeglImageOES @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable GLeglImageOES> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable GLeglImageOES value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable GLeglImageOES value0, @Nullable GLeglImageOES ...values) {
        ...
        @Override
        public @NotNull Iterator<GLeglImageOES> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<GLeglImageOES> {
