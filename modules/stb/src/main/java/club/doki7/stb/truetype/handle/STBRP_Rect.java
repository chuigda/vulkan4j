package club.doki7.stb.truetype.handle;
...
/// Represents an opaque handle type {@code stbrp_rect}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record STBRP_Rect(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code stbrp_rect} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<STBRP_Rect> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable STBRP_Rect read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new STBRP_Rect(s);
        }

        public void write(@Nullable STBRP_Rect value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable STBRP_Rect read(long index) {
        ...
        public void write(long index, @Nullable STBRP_Rect value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable STBRP_Rect[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable STBRP_Rect value0, @Nullable STBRP_Rect ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable STBRP_Rect @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable STBRP_Rect> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable STBRP_Rect value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable STBRP_Rect value0, @Nullable STBRP_Rect ...values) {
        ...
        @Override
        public @NotNull Iterator<STBRP_Rect> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<STBRP_Rect> {
