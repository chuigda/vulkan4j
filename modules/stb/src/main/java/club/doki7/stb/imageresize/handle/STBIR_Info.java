package club.doki7.stb.imageresize.handle;
...
/// Represents an opaque handle type {@code stbir__info}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record STBIR_Info(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code stbir__info} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<STBIR_Info> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable STBIR_Info read() {
        ...
        public void write(@Nullable STBIR_Info value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable STBIR_Info read(long index) {
        ...
        public void write(long index, @Nullable STBIR_Info value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable STBIR_Info[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable STBIR_Info value0, @Nullable STBIR_Info ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable STBIR_Info @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable STBIR_Info> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable STBIR_Info value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable STBIR_Info value0, @Nullable STBIR_Info ...values) {
        ...
        @Override
        public @NotNull Iterator<STBIR_Info> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<STBIR_Info> {
