...
/// Represents an opaque handle type <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrAction.html"><code>XrAction</code></a>.
///
/// ## Contracts
///
...
///
/// @see <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrAction.html"><code>XrAction</code></a>
@ValueBasedCandidate
@UnsafeConstructor
public record XrAction(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrAction.html"><code>XrAction</code></a> handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<XrAction> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable XrAction read() {
        ...
        public void write(@Nullable XrAction value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable XrAction read(long index) {
        ...
        public void write(long index, @Nullable XrAction value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable XrAction[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable XrAction value0, @Nullable XrAction ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable XrAction @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable XrAction> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable XrAction value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable XrAction value0, @Nullable XrAction ...values) {
        ...
        @Override
        public @NotNull Iterator<XrAction> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<XrAction> {
