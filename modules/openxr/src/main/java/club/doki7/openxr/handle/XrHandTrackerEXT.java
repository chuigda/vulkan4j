...
/// Represents an opaque handle type <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrHandTrackerEXT.html"><code>XrHandTrackerEXT</code></a>.
///
/// ## Contracts
///
...
///
/// @see <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrHandTrackerEXT.html"><code>XrHandTrackerEXT</code></a>
@ValueBasedCandidate
@UnsafeConstructor
public record XrHandTrackerEXT(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrHandTrackerEXT.html"><code>XrHandTrackerEXT</code></a> handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<XrHandTrackerEXT> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable XrHandTrackerEXT read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new XrHandTrackerEXT(s);
        }

        public void write(@Nullable XrHandTrackerEXT value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable XrHandTrackerEXT read(long index) {
        ...
        public void write(long index, @Nullable XrHandTrackerEXT value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable XrHandTrackerEXT[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable XrHandTrackerEXT value0, @Nullable XrHandTrackerEXT ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable XrHandTrackerEXT @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable XrHandTrackerEXT> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable XrHandTrackerEXT value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable XrHandTrackerEXT value0, @Nullable XrHandTrackerEXT ...values) {
        ...
        @Override
        public @NotNull Iterator<XrHandTrackerEXT> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<XrHandTrackerEXT> {
