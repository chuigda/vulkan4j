...
/// Represents an opaque handle type <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrSession.html"><code>XrSession</code></a>.
///
/// ## Contracts
///
...
///
/// @see <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrSession.html"><code>XrSession</code></a>
@ValueBasedCandidate
@UnsafeConstructor
public record XrSession(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrSession.html"><code>XrSession</code></a> handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<XrSession> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable XrSession read() {
        ...
        public void write(@Nullable XrSession value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable XrSession read(long index) {
        ...
        public void write(long index, @Nullable XrSession value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable XrSession[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable XrSession value0, @Nullable XrSession ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable XrSession @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable XrSession> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable XrSession value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable XrSession value0, @Nullable XrSession ...values) {
        ...
        @Override
        public @NotNull Iterator<XrSession> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<XrSession> {
