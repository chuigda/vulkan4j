...
/// Represents an opaque handle type <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrBodyTrackerBD.html"><code>XrBodyTrackerBD</code></a>.
///
/// ## Contracts
///
...
///
/// @see <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrBodyTrackerBD.html"><code>XrBodyTrackerBD</code></a>
@ValueBasedCandidate
@UnsafeConstructor
public record XrBodyTrackerBD(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrBodyTrackerBD.html"><code>XrBodyTrackerBD</code></a> handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<XrBodyTrackerBD> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable XrBodyTrackerBD read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new XrBodyTrackerBD(s);
        }

        public void write(@Nullable XrBodyTrackerBD value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable XrBodyTrackerBD read(long index) {
        ...
        public void write(long index, @Nullable XrBodyTrackerBD value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable XrBodyTrackerBD[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable XrBodyTrackerBD value0, @Nullable XrBodyTrackerBD ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable XrBodyTrackerBD @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable XrBodyTrackerBD> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable XrBodyTrackerBD value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable XrBodyTrackerBD value0, @Nullable XrBodyTrackerBD ...values) {
        ...
        @Override
        public @NotNull Iterator<XrBodyTrackerBD> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<XrBodyTrackerBD> {
