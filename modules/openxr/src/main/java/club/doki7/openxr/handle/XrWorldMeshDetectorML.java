...
/// Represents an opaque handle type <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrWorldMeshDetectorML.html"><code>XrWorldMeshDetectorML</code></a>.
///
/// ## Contracts
///
...
///
/// @see <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrWorldMeshDetectorML.html"><code>XrWorldMeshDetectorML</code></a>
@ValueBasedCandidate
@UnsafeConstructor
public record XrWorldMeshDetectorML(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrWorldMeshDetectorML.html"><code>XrWorldMeshDetectorML</code></a> handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<XrWorldMeshDetectorML> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable XrWorldMeshDetectorML read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new XrWorldMeshDetectorML(s);
        }

        public void write(@Nullable XrWorldMeshDetectorML value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable XrWorldMeshDetectorML read(long index) {
        ...
        public void write(long index, @Nullable XrWorldMeshDetectorML value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable XrWorldMeshDetectorML[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable XrWorldMeshDetectorML value0, @Nullable XrWorldMeshDetectorML ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocateV(Arena arena, @Nullable XrWorldMeshDetectorML value0, @Nullable XrWorldMeshDetectorML ...values) {
        ...
        @Override
        public @NotNull Iterator<XrWorldMeshDetectorML> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<XrWorldMeshDetectorML> {
            ...
            @Override
            public @Nullable XrWorldMeshDetectorML next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                MemorySegment s = segment.get(ValueLayout.ADDRESS, 0);
                segment = segment.asSlice(ValueLayout.ADDRESS.byteSize());
                return s.equals(MemorySegment.NULL) ? null : new XrWorldMeshDetectorML(s);
            }
