...
/// Represents an opaque handle type <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrPassthroughLayerFB.html"><code>XrPassthroughLayerFB</code></a>.
///
/// ## Contracts
///
...
///
/// @see <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrPassthroughLayerFB.html"><code>XrPassthroughLayerFB</code></a>
@ValueBasedCandidate
@UnsafeConstructor
public record XrPassthroughLayerFB(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to <a href="https://registry.khronos.org/OpenXR/specs/1.1/man/html/XrPassthroughLayerFB.html"><code>XrPassthroughLayerFB</code></a> handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<XrPassthroughLayerFB> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable XrPassthroughLayerFB read() {
        ...
        public void write(@Nullable XrPassthroughLayerFB value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable XrPassthroughLayerFB read(long index) {
        ...
        public void write(long index, @Nullable XrPassthroughLayerFB value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable XrPassthroughLayerFB[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable XrPassthroughLayerFB value0, @Nullable XrPassthroughLayerFB ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable XrPassthroughLayerFB @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable XrPassthroughLayerFB> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable XrPassthroughLayerFB value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable XrPassthroughLayerFB value0, @Nullable XrPassthroughLayerFB ...values) {
        ...
        @Override
        public @NotNull Iterator<XrPassthroughLayerFB> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<XrPassthroughLayerFB> {
            ...
            @Override
            public @Nullable XrPassthroughLayerFB next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                MemorySegment s = segment.get(ValueLayout.ADDRESS, 0);
                segment = segment.asSlice(ValueLayout.ADDRESS.byteSize());
                return s.equals(MemorySegment.NULL) ? null : new XrPassthroughLayerFB(s);
            }
