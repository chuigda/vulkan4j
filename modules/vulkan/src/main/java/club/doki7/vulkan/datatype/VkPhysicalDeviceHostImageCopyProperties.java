...
///
/// The {@code allocate} ({@link VkPhysicalDeviceHostImageCopyProperties#allocate(Arena)}, {@link VkPhysicalDeviceHostImageCopyProperties#allocate(Arena, long)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDeviceHostImageCopyProperties(@NotNull MemorySegment segment) implements IVkPhysicalDeviceHostImageCopyProperties {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkPhysicalDeviceHostImageCopyProperties.html"><code>VkPhysicalDeviceHostImageCopyProperties</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkPhysicalDeviceHostImageCopyProperties}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkPhysicalDeviceHostImageCopyProperties to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkPhysicalDeviceHostImageCopyProperties.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkPhysicalDeviceHostImageCopyProperties, Iterable<VkPhysicalDeviceHostImageCopyProperties> {
        public long size() {
        ...
        public @NotNull VkPhysicalDeviceHostImageCopyProperties at(long index) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkPhysicalDeviceHostImageCopyProperties.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkPhysicalDeviceHostImageCopyProperties.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkPhysicalDeviceHostImageCopyProperties.BYTES,
                (end - start) * VkPhysicalDeviceHostImageCopyProperties.BYTES
            ));
        }
        ...
        @Override
        public @NotNull Iterator<VkPhysicalDeviceHostImageCopyProperties> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkPhysicalDeviceHostImageCopyProperties> {
            ...
            @Override
            public VkPhysicalDeviceHostImageCopyProperties next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkPhysicalDeviceHostImageCopyProperties ret = new VkPhysicalDeviceHostImageCopyProperties(segment.asSlice(0, VkPhysicalDeviceHostImageCopyProperties.BYTES));
                segment = segment.asSlice(VkPhysicalDeviceHostImageCopyProperties.BYTES);
                return ret;
            }
    ...
    public static VkPhysicalDeviceHostImageCopyProperties.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkPhysicalDeviceHostImageCopyProperties.Ptr ret = new VkPhysicalDeviceHostImageCopyProperties.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.PHYSICAL_DEVICE_HOST_IMAGE_COPY_PROPERTIES);
        }
        return ret;
    }
    ...
    public VkPhysicalDeviceHostImageCopyProperties sType(@MagicConstant(valuesFromClass = VkStructureType.class) int value) {
