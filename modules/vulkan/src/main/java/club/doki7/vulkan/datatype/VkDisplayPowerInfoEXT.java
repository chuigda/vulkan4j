...
///
/// The {@code allocate} ({@link VkDisplayPowerInfoEXT#allocate(Arena)}, {@link VkDisplayPowerInfoEXT#allocate(Arena, long)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkDisplayPowerInfoEXT(@NotNull MemorySegment segment) implements IVkDisplayPowerInfoEXT {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkDisplayPowerInfoEXT.html"><code>VkDisplayPowerInfoEXT</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkDisplayPowerInfoEXT}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkDisplayPowerInfoEXT to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkDisplayPowerInfoEXT.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkDisplayPowerInfoEXT, Iterable<VkDisplayPowerInfoEXT> {
        public long size() {
            return segment.byteSize() / VkDisplayPowerInfoEXT.BYTES;
        }
        ...
        public @NotNull VkDisplayPowerInfoEXT at(long index) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkDisplayPowerInfoEXT.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkDisplayPowerInfoEXT.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkDisplayPowerInfoEXT.BYTES,
                (end - start) * VkDisplayPowerInfoEXT.BYTES
            ));
        }
        ...
        private static final class Iter implements Iterator<VkDisplayPowerInfoEXT> {
            ...
            @Override
            public VkDisplayPowerInfoEXT next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkDisplayPowerInfoEXT ret = new VkDisplayPowerInfoEXT(segment.asSlice(0, VkDisplayPowerInfoEXT.BYTES));
                segment = segment.asSlice(VkDisplayPowerInfoEXT.BYTES);
                return ret;
            }
    ...
    public static VkDisplayPowerInfoEXT.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkDisplayPowerInfoEXT.Ptr ret = new VkDisplayPowerInfoEXT.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.DISPLAY_POWER_INFO_EXT);
        }
        return ret;
    }
    ...
    public VkDisplayPowerInfoEXT sType(@MagicConstant(valuesFromClass = VkStructureType.class) int value) {
    ...
    public @MagicConstant(valuesFromClass = VkDisplayPowerStateEXT.class) int powerState() {
        return segment.get(LAYOUT$powerState, OFFSET$powerState);
    }

    public VkDisplayPowerInfoEXT powerState(@MagicConstant(valuesFromClass = VkDisplayPowerStateEXT.class) int value) {
