...
@ValueBasedCandidate
@UnsafeConstructor
public record VkExtent3D(@NotNull MemorySegment segment) implements IVkExtent3D {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkExtent3D.html"><code>VkExtent3D</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkExtent3D}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkExtent3D to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkExtent3D.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkExtent3D, Iterable<VkExtent3D> {
        public long size() {
            return segment.byteSize() / VkExtent3D.BYTES;
        }
        ...
        public @NotNull VkExtent3D at(long index) {
            return new VkExtent3D(segment.asSlice(index * VkExtent3D.BYTES, VkExtent3D.BYTES));
        }

        public void at(long index, @NotNull Consumer<@NotNull VkExtent3D> consumer) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkExtent3D.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkExtent3D.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkExtent3D.BYTES,
                (end - start) * VkExtent3D.BYTES
            ));
        }
        ...
        @Override
        public @NotNull Iterator<VkExtent3D> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkExtent3D> {
            ...
            @Override
            public VkExtent3D next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkExtent3D ret = new VkExtent3D(segment.asSlice(0, VkExtent3D.BYTES));
                segment = segment.asSlice(VkExtent3D.BYTES);
                return ret;
            }
    ...
    public VkExtent3D width(@Unsigned int value) {
    ...
    public VkExtent3D height(@Unsigned int value) {
    ...
    public VkExtent3D depth(@Unsigned int value) {
