...
public record VkFormatProperties(@NotNull MemorySegment segment) implements IVkFormatProperties {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkFormatProperties, Iterable<VkFormatProperties> {
        public long size() {
        ...
        public void at(long index, @NotNull Consumer<@NotNull VkFormatProperties> consumer) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkFormatProperties.BYTES));
        }
        ...
        @Override
        public @NotNull Iterator<VkFormatProperties> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkFormatProperties> {
            ...
            @Override
            public VkFormatProperties next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkFormatProperties ret = new VkFormatProperties(segment.asSlice(0, VkFormatProperties.BYTES));
                segment = segment.asSlice(VkFormatProperties.BYTES);
                return ret;
            }
