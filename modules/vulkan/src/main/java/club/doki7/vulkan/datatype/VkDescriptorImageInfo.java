...
public record VkDescriptorImageInfo(@NotNull MemorySegment segment) implements IVkDescriptorImageInfo {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkDescriptorImageInfo, Iterable<VkDescriptorImageInfo> {
        public long size() {
        ...
        public void at(long index, @NotNull Consumer<@NotNull VkDescriptorImageInfo> consumer) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkDescriptorImageInfo.BYTES));
        }
        ...
        private static final class Iter implements Iterator<VkDescriptorImageInfo> {
            ...
            @Override
            public VkDescriptorImageInfo next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkDescriptorImageInfo ret = new VkDescriptorImageInfo(segment.asSlice(0, VkDescriptorImageInfo.BYTES));
                segment = segment.asSlice(VkDescriptorImageInfo.BYTES);
                return ret;
            }
