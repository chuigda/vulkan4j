...
public record VkPrivateDataSlot(@NotNull MemorySegment segment) implements IPointer {
    ...
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<VkPrivateDataSlot> {
        ...
        public void write(@Nullable VkPrivateDataSlot[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable VkPrivateDataSlot value0, @Nullable VkPrivateDataSlot ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable VkPrivateDataSlot @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable VkPrivateDataSlot> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable VkPrivateDataSlot value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        @Override
        public @NotNull Iterator<VkPrivateDataSlot> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<VkPrivateDataSlot> {
