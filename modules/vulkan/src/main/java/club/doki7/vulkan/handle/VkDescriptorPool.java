...
public record VkDescriptorPool(@NotNull MemorySegment segment) implements IPointer {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<VkDescriptorPool> {
        public long size() {
        ...
        public void write(@Nullable VkDescriptorPool[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable VkDescriptorPool value0, @Nullable VkDescriptorPool ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable VkDescriptorPool @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable VkDescriptorPool> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable VkDescriptorPool value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable VkDescriptorPool value0, @Nullable VkDescriptorPool ...values) {
        ...
        @Override
        public @NotNull Iterator<VkDescriptorPool> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<VkDescriptorPool> {
