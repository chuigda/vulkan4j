...
///
/// The {@code allocate} ({@link VkRenderPassAttachmentBeginInfo#allocate(Arena)}, {@link VkRenderPassAttachmentBeginInfo#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkRenderPassAttachmentBeginInfo#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkRenderPassAttachmentBeginInfo(@NotNull MemorySegment segment) implements IVkRenderPassAttachmentBeginInfo {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkRenderPassAttachmentBeginInfo.html"><code>VkRenderPassAttachmentBeginInfo</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkRenderPassAttachmentBeginInfo}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkRenderPassAttachmentBeginInfo to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkRenderPassAttachmentBeginInfo.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkRenderPassAttachmentBeginInfo, Iterable<VkRenderPassAttachmentBeginInfo> {
        public long size() {
            return segment.byteSize() / VkRenderPassAttachmentBeginInfo.BYTES;
        }
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkRenderPassAttachmentBeginInfo.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkRenderPassAttachmentBeginInfo.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkRenderPassAttachmentBeginInfo.BYTES,
                (end - start) * VkRenderPassAttachmentBeginInfo.BYTES
            ));
        }
        ...
        private static final class Iter implements Iterator<VkRenderPassAttachmentBeginInfo> {
            ...
            @Override
            public VkRenderPassAttachmentBeginInfo next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkRenderPassAttachmentBeginInfo ret = new VkRenderPassAttachmentBeginInfo(segment.asSlice(0, VkRenderPassAttachmentBeginInfo.BYTES));
                segment = segment.asSlice(VkRenderPassAttachmentBeginInfo.BYTES);
                return ret;
            }
    ...
    public static VkRenderPassAttachmentBeginInfo.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkRenderPassAttachmentBeginInfo.Ptr ret = new VkRenderPassAttachmentBeginInfo.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.RENDER_PASS_ATTACHMENT_BEGIN_INFO);
        }
        return ret;
    }
    ...
    public VkRenderPassAttachmentBeginInfo sType(@MagicConstant(valuesFromClass = VkStructureType.class) int value) {
