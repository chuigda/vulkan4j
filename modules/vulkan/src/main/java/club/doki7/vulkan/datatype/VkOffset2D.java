...
public record VkOffset2D(@NotNull MemorySegment segment) implements IVkOffset2D {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkOffset2D, Iterable<VkOffset2D> {
        public long size() {
        ...
        public void at(long index, @NotNull Consumer<@NotNull VkOffset2D> consumer) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkOffset2D.BYTES));
        }
        ...
        private static final class Iter implements Iterator<VkOffset2D> {
            ...
            @Override
            public VkOffset2D next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkOffset2D ret = new VkOffset2D(segment.asSlice(0, VkOffset2D.BYTES));
                segment = segment.asSlice(VkOffset2D.BYTES);
                return ret;
            }
