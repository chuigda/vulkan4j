...
///
/// The {@code allocate} ({@link VkDeviceBufferMemoryRequirements#allocate(Arena)}, {@link VkDeviceBufferMemoryRequirements#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkDeviceBufferMemoryRequirements#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkDeviceBufferMemoryRequirements(@NotNull MemorySegment segment) implements IVkDeviceBufferMemoryRequirements {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkDeviceBufferMemoryRequirements.html"><code>VkDeviceBufferMemoryRequirements</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkDeviceBufferMemoryRequirements}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkDeviceBufferMemoryRequirements to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkDeviceBufferMemoryRequirements.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkDeviceBufferMemoryRequirements, Iterable<VkDeviceBufferMemoryRequirements> {
        public long size() {
            return segment.byteSize() / VkDeviceBufferMemoryRequirements.BYTES;
        }
        ...
        public @NotNull VkDeviceBufferMemoryRequirements at(long index) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkDeviceBufferMemoryRequirements.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkDeviceBufferMemoryRequirements.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkDeviceBufferMemoryRequirements.BYTES,
                (end - start) * VkDeviceBufferMemoryRequirements.BYTES
            ));
        }
        ...
        @Override
        public @NotNull Iterator<VkDeviceBufferMemoryRequirements> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkDeviceBufferMemoryRequirements> {
            ...
            @Override
            public VkDeviceBufferMemoryRequirements next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkDeviceBufferMemoryRequirements ret = new VkDeviceBufferMemoryRequirements(segment.asSlice(0, VkDeviceBufferMemoryRequirements.BYTES));
                segment = segment.asSlice(VkDeviceBufferMemoryRequirements.BYTES);
                return ret;
            }
    ...
    public static VkDeviceBufferMemoryRequirements.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkDeviceBufferMemoryRequirements.Ptr ret = new VkDeviceBufferMemoryRequirements.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.DEVICE_BUFFER_MEMORY_REQUIREMENTS);
        }
        return ret;
    }
    ...
    public VkDeviceBufferMemoryRequirements pCreateInfo(@Nullable IVkBufferCreateInfo value) {
        MemorySegment s = value == null ? MemorySegment.NULL : value.segment();
        pCreateInfoRaw(s);
        return this;
    }

    @Unsafe public @Nullable VkBufferCreateInfo.Ptr pCreateInfo(int assumedCount) {
