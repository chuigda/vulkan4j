...
public record VkMemoryHeap(@NotNull MemorySegment segment) implements IVkMemoryHeap {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkMemoryHeap, Iterable<VkMemoryHeap> {
        public long size() {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkMemoryHeap.BYTES));
        }
        ...
        public VkMemoryHeap[] toArray() {
            VkMemoryHeap[] ret = new VkMemoryHeap[(int) size()];
        ...
        private static final class Iter implements Iterator<VkMemoryHeap> {
            ...
            @Override
            public VkMemoryHeap next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkMemoryHeap ret = new VkMemoryHeap(segment.asSlice(0, VkMemoryHeap.BYTES));
                segment = segment.asSlice(VkMemoryHeap.BYTES);
                return ret;
            }
    ...
    public @MagicConstant(valuesFromClass = VkMemoryHeapFlags.class) int flags() {
        return segment.get(LAYOUT$flags, OFFSET$flags);
    }

    public VkMemoryHeap flags(@MagicConstant(valuesFromClass = VkMemoryHeapFlags.class) int value) {
