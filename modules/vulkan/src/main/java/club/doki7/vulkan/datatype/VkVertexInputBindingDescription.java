...
public record VkVertexInputBindingDescription(@NotNull MemorySegment segment) implements IVkVertexInputBindingDescription {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkVertexInputBindingDescription, Iterable<VkVertexInputBindingDescription> {
        public long size() {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkVertexInputBindingDescription.BYTES));
        }
        ...
        @Override
        public @NotNull Iterator<VkVertexInputBindingDescription> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkVertexInputBindingDescription> {
            ...
            @Override
            public VkVertexInputBindingDescription next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkVertexInputBindingDescription ret = new VkVertexInputBindingDescription(segment.asSlice(0, VkVertexInputBindingDescription.BYTES));
                segment = segment.asSlice(VkVertexInputBindingDescription.BYTES);
                return ret;
            }
