...
public record VkSurfaceFormatKHR(@NotNull MemorySegment segment) implements IVkSurfaceFormatKHR {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkSurfaceFormatKHR, Iterable<VkSurfaceFormatKHR> {
        public long size() {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkSurfaceFormatKHR.BYTES));
        }
        ...
        @Override
        public @NotNull Iterator<VkSurfaceFormatKHR> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkSurfaceFormatKHR> {
            ...
            @Override
            public VkSurfaceFormatKHR next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkSurfaceFormatKHR ret = new VkSurfaceFormatKHR(segment.asSlice(0, VkSurfaceFormatKHR.BYTES));
                segment = segment.asSlice(VkSurfaceFormatKHR.BYTES);
                return ret;
            }
