...
///
/// The {@code allocate} ({@link VkDrmFormatModifierPropertiesListEXT#allocate(Arena)}, {@link VkDrmFormatModifierPropertiesListEXT#allocate(Arena, long)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkDrmFormatModifierPropertiesListEXT(@NotNull MemorySegment segment) implements IVkDrmFormatModifierPropertiesListEXT {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkDrmFormatModifierPropertiesListEXT.html"><code>VkDrmFormatModifierPropertiesListEXT</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkDrmFormatModifierPropertiesListEXT}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkDrmFormatModifierPropertiesListEXT to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkDrmFormatModifierPropertiesListEXT.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkDrmFormatModifierPropertiesListEXT, Iterable<VkDrmFormatModifierPropertiesListEXT> {
        public long size() {
            return segment.byteSize() / VkDrmFormatModifierPropertiesListEXT.BYTES;
        }
        ...
        public @NotNull VkDrmFormatModifierPropertiesListEXT at(long index) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkDrmFormatModifierPropertiesListEXT.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkDrmFormatModifierPropertiesListEXT.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkDrmFormatModifierPropertiesListEXT.BYTES,
                (end - start) * VkDrmFormatModifierPropertiesListEXT.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VkDrmFormatModifierPropertiesListEXT.BYTES));
        }

        public VkDrmFormatModifierPropertiesListEXT[] toArray() {
            VkDrmFormatModifierPropertiesListEXT[] ret = new VkDrmFormatModifierPropertiesListEXT[(int) size()];
        ...
        @Override
        public @NotNull Iterator<VkDrmFormatModifierPropertiesListEXT> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkDrmFormatModifierPropertiesListEXT> {
            ...
            @Override
            public VkDrmFormatModifierPropertiesListEXT next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkDrmFormatModifierPropertiesListEXT ret = new VkDrmFormatModifierPropertiesListEXT(segment.asSlice(0, VkDrmFormatModifierPropertiesListEXT.BYTES));
                segment = segment.asSlice(VkDrmFormatModifierPropertiesListEXT.BYTES);
                return ret;
            }
    ...
    public static VkDrmFormatModifierPropertiesListEXT.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkDrmFormatModifierPropertiesListEXT.Ptr ret = new VkDrmFormatModifierPropertiesListEXT.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.DRM_FORMAT_MODIFIER_PROPERTIES_LIST_EXT);
        }
        return ret;
    }
    ...
    public VkDrmFormatModifierPropertiesListEXT sType(@MagicConstant(valuesFromClass = VkStructureType.class) int value) {
    ...
    @Unsafe public @Nullable VkDrmFormatModifierPropertiesEXT.Ptr pDrmFormatModifierProperties(int assumedCount) {
        ...
        return new VkDrmFormatModifierPropertiesEXT.Ptr(s);
    }
