...
///
/// The {@code allocate} ({@link VkPhysicalDeviceSubgroupProperties#allocate(Arena)}, {@link VkPhysicalDeviceSubgroupProperties#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkPhysicalDeviceSubgroupProperties#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDeviceSubgroupProperties(@NotNull MemorySegment segment) implements IVkPhysicalDeviceSubgroupProperties {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkPhysicalDeviceSubgroupProperties.html"><code>VkPhysicalDeviceSubgroupProperties</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkPhysicalDeviceSubgroupProperties}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkPhysicalDeviceSubgroupProperties to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkPhysicalDeviceSubgroupProperties.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkPhysicalDeviceSubgroupProperties, Iterable<VkPhysicalDeviceSubgroupProperties> {
        public long size() {
            return segment.byteSize() / VkPhysicalDeviceSubgroupProperties.BYTES;
        }
        ...
        public @NotNull VkPhysicalDeviceSubgroupProperties at(long index) {
        ...
        public void at(long index, @NotNull Consumer<@NotNull VkPhysicalDeviceSubgroupProperties> consumer) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkPhysicalDeviceSubgroupProperties.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkPhysicalDeviceSubgroupProperties.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkPhysicalDeviceSubgroupProperties.BYTES,
                (end - start) * VkPhysicalDeviceSubgroupProperties.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VkPhysicalDeviceSubgroupProperties.BYTES));
        }

        public VkPhysicalDeviceSubgroupProperties[] toArray() {
            VkPhysicalDeviceSubgroupProperties[] ret = new VkPhysicalDeviceSubgroupProperties[(int) size()];
        ...
        @Override
        public @NotNull Iterator<VkPhysicalDeviceSubgroupProperties> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkPhysicalDeviceSubgroupProperties> {
            ...
            @Override
            public VkPhysicalDeviceSubgroupProperties next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkPhysicalDeviceSubgroupProperties ret = new VkPhysicalDeviceSubgroupProperties(segment.asSlice(0, VkPhysicalDeviceSubgroupProperties.BYTES));
                segment = segment.asSlice(VkPhysicalDeviceSubgroupProperties.BYTES);
                return ret;
            }
    ...
    public static VkPhysicalDeviceSubgroupProperties.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkPhysicalDeviceSubgroupProperties.Ptr ret = new VkPhysicalDeviceSubgroupProperties.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.PHYSICAL_DEVICE_SUBGROUP_PROPERTIES);
        }
        return ret;
    }
