...
public record VkOpaqueCaptureDescriptorDataCreateInfoEXT(@NotNull MemorySegment segment) implements IVkOpaqueCaptureDescriptorDataCreateInfoEXT {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkOpaqueCaptureDescriptorDataCreateInfoEXT, Iterable<VkOpaqueCaptureDescriptorDataCreateInfoEXT> {
        public long size() {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkOpaqueCaptureDescriptorDataCreateInfoEXT.BYTES));
        }
        ...
        @Override
        public @NotNull Iterator<VkOpaqueCaptureDescriptorDataCreateInfoEXT> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkOpaqueCaptureDescriptorDataCreateInfoEXT> {
            ...
            @Override
            public VkOpaqueCaptureDescriptorDataCreateInfoEXT next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkOpaqueCaptureDescriptorDataCreateInfoEXT ret = new VkOpaqueCaptureDescriptorDataCreateInfoEXT(segment.asSlice(0, VkOpaqueCaptureDescriptorDataCreateInfoEXT.BYTES));
                segment = segment.asSlice(VkOpaqueCaptureDescriptorDataCreateInfoEXT.BYTES);
                return ret;
            }
