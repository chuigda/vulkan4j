...
public record VkDeviceQueueInfo2(@NotNull MemorySegment segment) implements IVkDeviceQueueInfo2 {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkDeviceQueueInfo2, Iterable<VkDeviceQueueInfo2> {
        public long size() {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkDeviceQueueInfo2.BYTES));
        }
        ...
        @Override
        public @NotNull Iterator<VkDeviceQueueInfo2> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkDeviceQueueInfo2> {
            ...
            @Override
            public VkDeviceQueueInfo2 next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkDeviceQueueInfo2 ret = new VkDeviceQueueInfo2(segment.asSlice(0, VkDeviceQueueInfo2.BYTES));
                segment = segment.asSlice(VkDeviceQueueInfo2.BYTES);
                return ret;
            }
