...
///
/// The {@code allocate} ({@link VkQueryPoolCreateInfo#allocate(Arena)}, {@link VkQueryPoolCreateInfo#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkQueryPoolCreateInfo#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkQueryPoolCreateInfo(@NotNull MemorySegment segment) implements IVkQueryPoolCreateInfo {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkQueryPoolCreateInfo.html"><code>VkQueryPoolCreateInfo</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkQueryPoolCreateInfo}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkQueryPoolCreateInfo to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkQueryPoolCreateInfo.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkQueryPoolCreateInfo, Iterable<VkQueryPoolCreateInfo> {
        public long size() {
        ...
        public @NotNull VkQueryPoolCreateInfo at(long index) {
            return new VkQueryPoolCreateInfo(segment.asSlice(index * VkQueryPoolCreateInfo.BYTES, VkQueryPoolCreateInfo.BYTES));
        }

        public void at(long index, @NotNull Consumer<@NotNull VkQueryPoolCreateInfo> consumer) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkQueryPoolCreateInfo.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkQueryPoolCreateInfo.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkQueryPoolCreateInfo.BYTES,
                (end - start) * VkQueryPoolCreateInfo.BYTES
            ));
        }
        ...
        @Override
        public @NotNull Iterator<VkQueryPoolCreateInfo> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkQueryPoolCreateInfo> {
            ...
            @Override
            public VkQueryPoolCreateInfo next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkQueryPoolCreateInfo ret = new VkQueryPoolCreateInfo(segment.asSlice(0, VkQueryPoolCreateInfo.BYTES));
                segment = segment.asSlice(VkQueryPoolCreateInfo.BYTES);
                return ret;
            }
    ...
    public static VkQueryPoolCreateInfo.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkQueryPoolCreateInfo.Ptr ret = new VkQueryPoolCreateInfo.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.QUERY_POOL_CREATE_INFO);
        }
        return ret;
    }
