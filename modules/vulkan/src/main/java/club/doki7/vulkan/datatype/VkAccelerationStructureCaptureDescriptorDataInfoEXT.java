...
///     VkAccelerationStructureKHR accelerationStructure; // optional // @link substring="VkAccelerationStructureKHR" target="VkAccelerationStructureKHR" @link substring="accelerationStructure" target="#accelerationStructure"
///     VkAccelerationStructureNV accelerationStructureNV; // optional // @link substring="VkAccelerationStructureNV" target="VkAccelerationStructureNV" @link substring="accelerationStructureNV" target="#accelerationStructureNV"
...
///
/// The {@code allocate} ({@link VkAccelerationStructureCaptureDescriptorDataInfoEXT#allocate(Arena)}, {@link VkAccelerationStructureCaptureDescriptorDataInfoEXT#allocate(Arena, long)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkAccelerationStructureCaptureDescriptorDataInfoEXT(@NotNull MemorySegment segment) implements IVkAccelerationStructureCaptureDescriptorDataInfoEXT {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkAccelerationStructureCaptureDescriptorDataInfoEXT.html"><code>VkAccelerationStructureCaptureDescriptorDataInfoEXT</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkAccelerationStructureCaptureDescriptorDataInfoEXT}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkAccelerationStructureCaptureDescriptorDataInfoEXT to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkAccelerationStructureCaptureDescriptorDataInfoEXT.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkAccelerationStructureCaptureDescriptorDataInfoEXT, Iterable<VkAccelerationStructureCaptureDescriptorDataInfoEXT> {
        public long size() {
            return segment.byteSize() / VkAccelerationStructureCaptureDescriptorDataInfoEXT.BYTES;
        }
        ...
        public @NotNull VkAccelerationStructureCaptureDescriptorDataInfoEXT at(long index) {
        ...
        public VkAccelerationStructureCaptureDescriptorDataInfoEXT.Ptr at(long index, @NotNull Consumer<@NotNull VkAccelerationStructureCaptureDescriptorDataInfoEXT> consumer) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkAccelerationStructureCaptureDescriptorDataInfoEXT.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkAccelerationStructureCaptureDescriptorDataInfoEXT.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkAccelerationStructureCaptureDescriptorDataInfoEXT.BYTES,
                (end - start) * VkAccelerationStructureCaptureDescriptorDataInfoEXT.BYTES
            ));
        }
        ...
        private static final class Iter implements Iterator<VkAccelerationStructureCaptureDescriptorDataInfoEXT> {
            ...
            @Override
            public VkAccelerationStructureCaptureDescriptorDataInfoEXT next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkAccelerationStructureCaptureDescriptorDataInfoEXT ret = new VkAccelerationStructureCaptureDescriptorDataInfoEXT(segment.asSlice(0, VkAccelerationStructureCaptureDescriptorDataInfoEXT.BYTES));
                segment = segment.asSlice(VkAccelerationStructureCaptureDescriptorDataInfoEXT.BYTES);
                return ret;
            }
    ...
    public static VkAccelerationStructureCaptureDescriptorDataInfoEXT.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkAccelerationStructureCaptureDescriptorDataInfoEXT.Ptr ret = new VkAccelerationStructureCaptureDescriptorDataInfoEXT.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.ACCELERATION_STRUCTURE_CAPTURE_DESCRIPTOR_DATA_INFO_EXT);
        }
        return ret;
    }
    ...
    public VkAccelerationStructureCaptureDescriptorDataInfoEXT pNext(@Pointer(comment="void*") @NotNull MemorySegment value) {
