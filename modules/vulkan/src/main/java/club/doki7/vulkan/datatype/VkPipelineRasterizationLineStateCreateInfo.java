...
///
/// The {@code allocate} ({@link VkPipelineRasterizationLineStateCreateInfo#allocate(Arena)}, {@link VkPipelineRasterizationLineStateCreateInfo#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkPipelineRasterizationLineStateCreateInfo#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPipelineRasterizationLineStateCreateInfo(@NotNull MemorySegment segment) implements IVkPipelineRasterizationLineStateCreateInfo {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkPipelineRasterizationLineStateCreateInfo.html"><code>VkPipelineRasterizationLineStateCreateInfo</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkPipelineRasterizationLineStateCreateInfo}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkPipelineRasterizationLineStateCreateInfo to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkPipelineRasterizationLineStateCreateInfo.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkPipelineRasterizationLineStateCreateInfo, Iterable<VkPipelineRasterizationLineStateCreateInfo> {
        public long size() {
            return segment.byteSize() / VkPipelineRasterizationLineStateCreateInfo.BYTES;
        }
        ...
        public @NotNull VkPipelineRasterizationLineStateCreateInfo at(long index) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkPipelineRasterizationLineStateCreateInfo.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkPipelineRasterizationLineStateCreateInfo.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkPipelineRasterizationLineStateCreateInfo.BYTES,
                (end - start) * VkPipelineRasterizationLineStateCreateInfo.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VkPipelineRasterizationLineStateCreateInfo.BYTES));
        }

        public VkPipelineRasterizationLineStateCreateInfo[] toArray() {
            VkPipelineRasterizationLineStateCreateInfo[] ret = new VkPipelineRasterizationLineStateCreateInfo[(int) size()];
        ...
        @Override
        public @NotNull Iterator<VkPipelineRasterizationLineStateCreateInfo> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkPipelineRasterizationLineStateCreateInfo> {
            ...
            @Override
            public VkPipelineRasterizationLineStateCreateInfo next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkPipelineRasterizationLineStateCreateInfo ret = new VkPipelineRasterizationLineStateCreateInfo(segment.asSlice(0, VkPipelineRasterizationLineStateCreateInfo.BYTES));
                segment = segment.asSlice(VkPipelineRasterizationLineStateCreateInfo.BYTES);
                return ret;
            }
    ...
    public static VkPipelineRasterizationLineStateCreateInfo.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkPipelineRasterizationLineStateCreateInfo.Ptr ret = new VkPipelineRasterizationLineStateCreateInfo.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.PIPELINE_RASTERIZATION_LINE_STATE_CREATE_INFO);
        }
        return ret;
    }
    ...
    public VkPipelineRasterizationLineStateCreateInfo sType(@MagicConstant(valuesFromClass = VkStructureType.class) int value) {
    ...
    public @MagicConstant(valuesFromClass = VkLineRasterizationMode.class) int lineRasterizationMode() {
        return segment.get(LAYOUT$lineRasterizationMode, OFFSET$lineRasterizationMode);
    }

    public VkPipelineRasterizationLineStateCreateInfo lineRasterizationMode(@MagicConstant(valuesFromClass = VkLineRasterizationMode.class) int value) {
