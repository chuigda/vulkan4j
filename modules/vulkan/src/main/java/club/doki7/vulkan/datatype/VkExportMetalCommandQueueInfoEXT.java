...
///
/// The {@code allocate} ({@link VkExportMetalCommandQueueInfoEXT#allocate(Arena)}, {@link VkExportMetalCommandQueueInfoEXT#allocate(Arena, long)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkExportMetalCommandQueueInfoEXT(@NotNull MemorySegment segment) implements IVkExportMetalCommandQueueInfoEXT {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkExportMetalCommandQueueInfoEXT.html"><code>VkExportMetalCommandQueueInfoEXT</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkExportMetalCommandQueueInfoEXT}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkExportMetalCommandQueueInfoEXT to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkExportMetalCommandQueueInfoEXT.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkExportMetalCommandQueueInfoEXT, Iterable<VkExportMetalCommandQueueInfoEXT> {
        public long size() {
            return segment.byteSize() / VkExportMetalCommandQueueInfoEXT.BYTES;
        }
        ...
        public @NotNull VkExportMetalCommandQueueInfoEXT at(long index) {
        ...
        public void write(long index, @NotNull VkExportMetalCommandQueueInfoEXT value) {
            MemorySegment s = segment.asSlice(index * VkExportMetalCommandQueueInfoEXT.BYTES, VkExportMetalCommandQueueInfoEXT.BYTES);
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkExportMetalCommandQueueInfoEXT.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkExportMetalCommandQueueInfoEXT.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkExportMetalCommandQueueInfoEXT.BYTES,
                (end - start) * VkExportMetalCommandQueueInfoEXT.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VkExportMetalCommandQueueInfoEXT.BYTES));
        }

        public VkExportMetalCommandQueueInfoEXT[] toArray() {
            VkExportMetalCommandQueueInfoEXT[] ret = new VkExportMetalCommandQueueInfoEXT[(int) size()];
        ...
        @Override
        public @NotNull Iterator<VkExportMetalCommandQueueInfoEXT> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkExportMetalCommandQueueInfoEXT> {
            ...
            @Override
            public VkExportMetalCommandQueueInfoEXT next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkExportMetalCommandQueueInfoEXT ret = new VkExportMetalCommandQueueInfoEXT(segment.asSlice(0, VkExportMetalCommandQueueInfoEXT.BYTES));
                segment = segment.asSlice(VkExportMetalCommandQueueInfoEXT.BYTES);
                return ret;
            }
    ...
    public static VkExportMetalCommandQueueInfoEXT.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkExportMetalCommandQueueInfoEXT.Ptr ret = new VkExportMetalCommandQueueInfoEXT.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.EXPORT_METAL_COMMAND_QUEUE_INFO_EXT);
        }
        return ret;
    }
    ...
    public VkExportMetalCommandQueueInfoEXT sType(@MagicConstant(valuesFromClass = VkStructureType.class) int value) {
