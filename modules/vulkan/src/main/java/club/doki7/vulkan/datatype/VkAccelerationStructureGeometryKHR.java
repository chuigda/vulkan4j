...
///
/// The {@code allocate} ({@link VkAccelerationStructureGeometryKHR#allocate(Arena)}, {@link VkAccelerationStructureGeometryKHR#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkAccelerationStructureGeometryKHR#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkAccelerationStructureGeometryKHR(@NotNull MemorySegment segment) implements IVkAccelerationStructureGeometryKHR {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkAccelerationStructureGeometryKHR.html"><code>VkAccelerationStructureGeometryKHR</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkAccelerationStructureGeometryKHR}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkAccelerationStructureGeometryKHR to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkAccelerationStructureGeometryKHR.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkAccelerationStructureGeometryKHR, Iterable<VkAccelerationStructureGeometryKHR> {
        public long size() {
            return segment.byteSize() / VkAccelerationStructureGeometryKHR.BYTES;
        }
        ...
        public @NotNull VkAccelerationStructureGeometryKHR at(long index) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkAccelerationStructureGeometryKHR.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkAccelerationStructureGeometryKHR.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkAccelerationStructureGeometryKHR.BYTES,
                (end - start) * VkAccelerationStructureGeometryKHR.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VkAccelerationStructureGeometryKHR.BYTES));
        }

        public VkAccelerationStructureGeometryKHR[] toArray() {
            VkAccelerationStructureGeometryKHR[] ret = new VkAccelerationStructureGeometryKHR[(int) size()];
        ...
        @Override
        public @NotNull Iterator<VkAccelerationStructureGeometryKHR> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkAccelerationStructureGeometryKHR> {
            ...
            @Override
            public VkAccelerationStructureGeometryKHR next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkAccelerationStructureGeometryKHR ret = new VkAccelerationStructureGeometryKHR(segment.asSlice(0, VkAccelerationStructureGeometryKHR.BYTES));
                segment = segment.asSlice(VkAccelerationStructureGeometryKHR.BYTES);
                return ret;
            }
    ...
    public static VkAccelerationStructureGeometryKHR.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkAccelerationStructureGeometryKHR.Ptr ret = new VkAccelerationStructureGeometryKHR.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.ACCELERATION_STRUCTURE_GEOMETRY_KHR);
        }
        return ret;
    }
    ...
    public VkAccelerationStructureGeometryKHR geometry(Consumer<@NotNull VkAccelerationStructureGeometryDataKHR> consumer) {
