...
/// Represents an opaque handle type <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkDescriptorSetLayout.html"><code>VkDescriptorSetLayout</code></a>.
///
/// ## Contracts
///
...
///
/// @see <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkDescriptorSetLayout.html"><code>VkDescriptorSetLayout</code></a>
@ValueBasedCandidate
@UnsafeConstructor
public record VkDescriptorSetLayout(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkDescriptorSetLayout.html"><code>VkDescriptorSetLayout</code></a> handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<VkDescriptorSetLayout> {
        ...
        public @Nullable VkDescriptorSetLayout read() {
        ...
        public void write(@Nullable VkDescriptorSetLayout value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable VkDescriptorSetLayout read(long index) {
        ...
        public void write(long index, @Nullable VkDescriptorSetLayout value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable VkDescriptorSetLayout[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable VkDescriptorSetLayout value0, @Nullable VkDescriptorSetLayout ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable VkDescriptorSetLayout @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable VkDescriptorSetLayout> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable VkDescriptorSetLayout value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable VkDescriptorSetLayout value0, @Nullable VkDescriptorSetLayout ...values) {
        ...
        @Override
        public @NotNull Iterator<VkDescriptorSetLayout> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<VkDescriptorSetLayout> {
            ...
            @Override
            public @Nullable VkDescriptorSetLayout next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                MemorySegment s = segment.get(ValueLayout.ADDRESS, 0);
                segment = segment.asSlice(ValueLayout.ADDRESS.byteSize());
                return s.equals(MemorySegment.NULL) ? null : new VkDescriptorSetLayout(s);
            }
