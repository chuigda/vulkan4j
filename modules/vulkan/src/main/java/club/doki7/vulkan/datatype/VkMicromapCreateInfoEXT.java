...
///
/// The {@code allocate} ({@link VkMicromapCreateInfoEXT#allocate(Arena)}, {@link VkMicromapCreateInfoEXT#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkMicromapCreateInfoEXT#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkMicromapCreateInfoEXT(@NotNull MemorySegment segment) implements IVkMicromapCreateInfoEXT {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkMicromapCreateInfoEXT.html"><code>VkMicromapCreateInfoEXT</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkMicromapCreateInfoEXT}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkMicromapCreateInfoEXT to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkMicromapCreateInfoEXT.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkMicromapCreateInfoEXT, Iterable<VkMicromapCreateInfoEXT> {
        public long size() {
            return segment.byteSize() / VkMicromapCreateInfoEXT.BYTES;
        }
        ...
        public @NotNull VkMicromapCreateInfoEXT at(long index) {
            return new VkMicromapCreateInfoEXT(segment.asSlice(index * VkMicromapCreateInfoEXT.BYTES, VkMicromapCreateInfoEXT.BYTES));
        }

        public VkMicromapCreateInfoEXT.Ptr at(long index, @NotNull Consumer<@NotNull VkMicromapCreateInfoEXT> consumer) {
            consumer.accept(at(index));
            return this;
        }

        public void write(long index, @NotNull VkMicromapCreateInfoEXT value) {
            MemorySegment s = segment.asSlice(index * VkMicromapCreateInfoEXT.BYTES, VkMicromapCreateInfoEXT.BYTES);
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkMicromapCreateInfoEXT.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkMicromapCreateInfoEXT.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkMicromapCreateInfoEXT.BYTES,
                (end - start) * VkMicromapCreateInfoEXT.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VkMicromapCreateInfoEXT.BYTES));
        }

        public VkMicromapCreateInfoEXT[] toArray() {
            VkMicromapCreateInfoEXT[] ret = new VkMicromapCreateInfoEXT[(int) size()];
        ...
        @Override
        public @NotNull Iterator<VkMicromapCreateInfoEXT> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkMicromapCreateInfoEXT> {
            ...
            @Override
            public VkMicromapCreateInfoEXT next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkMicromapCreateInfoEXT ret = new VkMicromapCreateInfoEXT(segment.asSlice(0, VkMicromapCreateInfoEXT.BYTES));
                segment = segment.asSlice(VkMicromapCreateInfoEXT.BYTES);
                return ret;
            }
    ...
    public static VkMicromapCreateInfoEXT.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkMicromapCreateInfoEXT.Ptr ret = new VkMicromapCreateInfoEXT.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.MICROMAP_CREATE_INFO_EXT);
        }
        return ret;
    }
