...
@ValueBasedCandidate
@UnsafeConstructor
public record VkOffset3D(@NotNull MemorySegment segment) implements IVkOffset3D {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkOffset3D.html"><code>VkOffset3D</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkOffset3D}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkOffset3D to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkOffset3D.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkOffset3D, Iterable<VkOffset3D> {
        public long size() {
        ...
        public @NotNull VkOffset3D at(long index) {
            return new VkOffset3D(segment.asSlice(index * VkOffset3D.BYTES, VkOffset3D.BYTES));
        }

        public void at(long index, @NotNull Consumer<@NotNull VkOffset3D> consumer) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkOffset3D.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkOffset3D.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkOffset3D.BYTES,
                (end - start) * VkOffset3D.BYTES
            ));
        }
        ...
        private static final class Iter implements Iterator<VkOffset3D> {
            ...
            @Override
            public VkOffset3D next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkOffset3D ret = new VkOffset3D(segment.asSlice(0, VkOffset3D.BYTES));
                segment = segment.asSlice(VkOffset3D.BYTES);
                return ret;
            }
    ...
    public static VkOffset3D.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        return new VkOffset3D.Ptr(segment);
    }
