...
///
/// The {@code allocate} ({@link VkRefreshObjectListKHR#allocate(Arena)}, {@link VkRefreshObjectListKHR#allocate(Arena, long)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkRefreshObjectListKHR(@NotNull MemorySegment segment) implements IVkRefreshObjectListKHR {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkRefreshObjectListKHR.html"><code>VkRefreshObjectListKHR</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkRefreshObjectListKHR}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkRefreshObjectListKHR to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkRefreshObjectListKHR.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkRefreshObjectListKHR, Iterable<VkRefreshObjectListKHR> {
        public long size() {
            return segment.byteSize() / VkRefreshObjectListKHR.BYTES;
        }
        ...
        public @NotNull VkRefreshObjectListKHR at(long index) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkRefreshObjectListKHR.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkRefreshObjectListKHR.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkRefreshObjectListKHR.BYTES,
                (end - start) * VkRefreshObjectListKHR.BYTES
            ));
        }
        ...
        private static final class Iter implements Iterator<VkRefreshObjectListKHR> {
            ...
            @Override
            public VkRefreshObjectListKHR next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkRefreshObjectListKHR ret = new VkRefreshObjectListKHR(segment.asSlice(0, VkRefreshObjectListKHR.BYTES));
                segment = segment.asSlice(VkRefreshObjectListKHR.BYTES);
                return ret;
            }
    ...
    public static VkRefreshObjectListKHR.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkRefreshObjectListKHR.Ptr ret = new VkRefreshObjectListKHR.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.REFRESH_OBJECT_LIST_KHR);
        }
        return ret;
    }
    ...
    public VkRefreshObjectListKHR sType(@MagicConstant(valuesFromClass = VkStructureType.class) int value) {
