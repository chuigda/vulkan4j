...
///
/// The {@code allocate} ({@link VkExportMemorySciBufInfoNV#allocate(Arena)}, {@link VkExportMemorySciBufInfoNV#allocate(Arena, long)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkExportMemorySciBufInfoNV(@NotNull MemorySegment segment) implements IVkExportMemorySciBufInfoNV {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkExportMemorySciBufInfoNV.html"><code>VkExportMemorySciBufInfoNV</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkExportMemorySciBufInfoNV}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkExportMemorySciBufInfoNV to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkExportMemorySciBufInfoNV.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkExportMemorySciBufInfoNV, Iterable<VkExportMemorySciBufInfoNV> {
        public long size() {
            return segment.byteSize() / VkExportMemorySciBufInfoNV.BYTES;
        }
        ...
        public @NotNull VkExportMemorySciBufInfoNV at(long index) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkExportMemorySciBufInfoNV.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkExportMemorySciBufInfoNV.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkExportMemorySciBufInfoNV.BYTES,
                (end - start) * VkExportMemorySciBufInfoNV.BYTES
            ));
        }
        ...
        private static final class Iter implements Iterator<VkExportMemorySciBufInfoNV> {
            ...
            @Override
            public VkExportMemorySciBufInfoNV next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkExportMemorySciBufInfoNV ret = new VkExportMemorySciBufInfoNV(segment.asSlice(0, VkExportMemorySciBufInfoNV.BYTES));
                segment = segment.asSlice(VkExportMemorySciBufInfoNV.BYTES);
                return ret;
            }
    ...
    public static VkExportMemorySciBufInfoNV.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkExportMemorySciBufInfoNV.Ptr ret = new VkExportMemorySciBufInfoNV.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.EXPORT_MEMORY_SCI_BUF_INFO_NV);
        }
        return ret;
    }
    ...
    public VkExportMemorySciBufInfoNV sType(@MagicConstant(valuesFromClass = VkStructureType.class) int value) {
