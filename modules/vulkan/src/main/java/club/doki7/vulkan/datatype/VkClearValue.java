...
public record VkClearValue(@NotNull MemorySegment segment) implements IVkClearValue {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkClearValue, Iterable<VkClearValue> {
        public long size() {
        ...
        public void at(long index, @NotNull Consumer<@NotNull VkClearValue> consumer) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkClearValue.BYTES));
        }
        ...
        @Override
        public @NotNull Iterator<VkClearValue> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkClearValue> {
            ...
            @Override
            public VkClearValue next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkClearValue ret = new VkClearValue(segment.asSlice(0, VkClearValue.BYTES));
                segment = segment.asSlice(VkClearValue.BYTES);
                return ret;
            }
