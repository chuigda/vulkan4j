...
///     VkShaderEXT shader; // @link substring="VkShaderEXT" target="VkShaderEXT" @link substring="shader" target="#shader"
...
///
/// The {@code allocate} ({@link VkWriteIndirectExecutionSetShaderEXT#allocate(Arena)}, {@link VkWriteIndirectExecutionSetShaderEXT#allocate(Arena, long)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkWriteIndirectExecutionSetShaderEXT(@NotNull MemorySegment segment) implements IVkWriteIndirectExecutionSetShaderEXT {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkWriteIndirectExecutionSetShaderEXT.html"><code>VkWriteIndirectExecutionSetShaderEXT</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkWriteIndirectExecutionSetShaderEXT}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkWriteIndirectExecutionSetShaderEXT to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkWriteIndirectExecutionSetShaderEXT.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkWriteIndirectExecutionSetShaderEXT, Iterable<VkWriteIndirectExecutionSetShaderEXT> {
        public long size() {
            return segment.byteSize() / VkWriteIndirectExecutionSetShaderEXT.BYTES;
        }
        ...
        public @NotNull VkWriteIndirectExecutionSetShaderEXT at(long index) {
        ...
        public void at(long index, @NotNull Consumer<@NotNull VkWriteIndirectExecutionSetShaderEXT> consumer) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkWriteIndirectExecutionSetShaderEXT.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkWriteIndirectExecutionSetShaderEXT.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkWriteIndirectExecutionSetShaderEXT.BYTES,
                (end - start) * VkWriteIndirectExecutionSetShaderEXT.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VkWriteIndirectExecutionSetShaderEXT.BYTES));
        }

        public VkWriteIndirectExecutionSetShaderEXT[] toArray() {
            VkWriteIndirectExecutionSetShaderEXT[] ret = new VkWriteIndirectExecutionSetShaderEXT[(int) size()];
        ...
        @Override
        public @NotNull Iterator<VkWriteIndirectExecutionSetShaderEXT> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkWriteIndirectExecutionSetShaderEXT> {
            ...
            @Override
            public VkWriteIndirectExecutionSetShaderEXT next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkWriteIndirectExecutionSetShaderEXT ret = new VkWriteIndirectExecutionSetShaderEXT(segment.asSlice(0, VkWriteIndirectExecutionSetShaderEXT.BYTES));
                segment = segment.asSlice(VkWriteIndirectExecutionSetShaderEXT.BYTES);
                return ret;
            }
    ...
    public static VkWriteIndirectExecutionSetShaderEXT.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkWriteIndirectExecutionSetShaderEXT.Ptr ret = new VkWriteIndirectExecutionSetShaderEXT.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.WRITE_INDIRECT_EXECUTION_SET_SHADER_EXT);
        }
        return ret;
    }
    ...
    public VkWriteIndirectExecutionSetShaderEXT pNext(@Pointer(comment="void*") @NotNull MemorySegment value) {
    ...
    public static final PathElement PATH$shader = PathElement.groupElement("shader");
