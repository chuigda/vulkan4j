...
public record VkDescriptorSetAllocateInfo(@NotNull MemorySegment segment) implements IVkDescriptorSetAllocateInfo {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkDescriptorSetAllocateInfo, Iterable<VkDescriptorSetAllocateInfo> {
        public long size() {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VkDescriptorSetAllocateInfo.BYTES));
        }
        ...
        @Override
        public @NotNull Iterator<VkDescriptorSetAllocateInfo> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VkDescriptorSetAllocateInfo> {
            ...
            @Override
            public VkDescriptorSetAllocateInfo next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkDescriptorSetAllocateInfo ret = new VkDescriptorSetAllocateInfo(segment.asSlice(0, VkDescriptorSetAllocateInfo.BYTES));
                segment = segment.asSlice(VkDescriptorSetAllocateInfo.BYTES);
                return ret;
            }
