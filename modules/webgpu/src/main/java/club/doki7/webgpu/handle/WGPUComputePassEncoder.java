...
/// Represents an opaque handle type {@code WGPUComputePassEncoder}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPUComputePassEncoder(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPUComputePassEncoder} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPUComputePassEncoder> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPUComputePassEncoder read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new WGPUComputePassEncoder(s);
        }

        public void write(@Nullable WGPUComputePassEncoder value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPUComputePassEncoder read(long index) {
        ...
        public void write(long index, @Nullable WGPUComputePassEncoder value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPUComputePassEncoder[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPUComputePassEncoder value0, @Nullable WGPUComputePassEncoder ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPUComputePassEncoder @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPUComputePassEncoder> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPUComputePassEncoder value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPUComputePassEncoder value0, @Nullable WGPUComputePassEncoder ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPUComputePassEncoder> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPUComputePassEncoder> {
            ...
            @Override
            public @Nullable WGPUComputePassEncoder next() {
                if (!hasNext()) {
