...
/// Represents an opaque handle type {@code Buffer}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record Buffer(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code Buffer} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<Buffer> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable Buffer read() {
        ...
        public void write(@Nullable Buffer value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable Buffer read(long index) {
        ...
        public void write(long index, @Nullable Buffer value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable Buffer[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable Buffer value0, @Nullable Buffer ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable Buffer[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable Buffer value0, @Nullable Buffer ...values) {
        ...
        @Override
        public @NotNull Iterator<Buffer> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<Buffer> {
