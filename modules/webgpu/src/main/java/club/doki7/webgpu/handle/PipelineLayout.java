...
/// Represents an opaque handle type {@code PipelineLayout}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record PipelineLayout(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code PipelineLayout} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<PipelineLayout> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable PipelineLayout read() {
        ...
        public void write(@Nullable PipelineLayout value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable PipelineLayout read(long index) {
        ...
        public void write(long index, @Nullable PipelineLayout value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable PipelineLayout[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable PipelineLayout value0, @Nullable PipelineLayout ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable PipelineLayout[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable PipelineLayout value0, @Nullable PipelineLayout ...values) {
        ...
        @Override
        public @NotNull Iterator<PipelineLayout> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<PipelineLayout> {
