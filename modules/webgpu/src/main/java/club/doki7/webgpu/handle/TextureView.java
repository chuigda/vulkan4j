...
/// Represents an opaque handle type {@code TextureView}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record TextureView(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code TextureView} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<TextureView> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable TextureView read() {
        ...
        public void write(@Nullable TextureView value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable TextureView read(long index) {
        ...
        public void write(long index, @Nullable TextureView value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable TextureView[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable TextureView value0, @Nullable TextureView ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable TextureView[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable TextureView value0, @Nullable TextureView ...values) {
        ...
        @Override
        public @NotNull Iterator<TextureView> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<TextureView> {
