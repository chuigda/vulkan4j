...
/// Represents an opaque handle type {@code Instance}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record Instance(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code Instance} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<Instance> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable Instance read() {
        ...
        public void write(@Nullable Instance value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable Instance read(long index) {
        ...
        public void write(long index, @Nullable Instance value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable Instance[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable Instance value0, @Nullable Instance ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable Instance[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable Instance value0, @Nullable Instance ...values) {
        ...
        private static class Iter implements Iterator<Instance> {
