...
/// Represents an opaque handle type {@code Device}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record Device(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code Device} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<Device> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable Device read() {
        ...
        public void write(@Nullable Device value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable Device read(long index) {
        ...
        public void write(long index, @Nullable Device value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable Device[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable Device value0, @Nullable Device ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable Device[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable Device value0, @Nullable Device ...values) {
        ...
        @Override
        public @NotNull Iterator<Device> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<Device> {
