...
///     TextureViewDimension dimension; // @link substring="TextureViewDimension" target="TextureViewDimension" @link substring="dimension" target="#dimension"
...
///     TextureAspect aspect; // @link substring="TextureAspect" target="TextureAspect" @link substring="aspect" target="#aspect"
...
@ValueBasedCandidate
@UnsafeConstructor
public record TextureViewDescriptor(@NotNull MemorySegment segment) implements ITextureViewDescriptor {
    /// Represents a pointer to / an array of null structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link TextureViewDescriptor}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// ITextureViewDescriptor to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code TextureViewDescriptor.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements ITextureViewDescriptor, Iterable<TextureViewDescriptor> {
        public long size() {
            return segment.byteSize() / TextureViewDescriptor.BYTES;
        }
        ...
        public @NotNull TextureViewDescriptor at(long index) {
            return new TextureViewDescriptor(segment.asSlice(index * TextureViewDescriptor.BYTES, TextureViewDescriptor.BYTES));
        }

        public void write(long index, @NotNull TextureViewDescriptor value) {
            MemorySegment s = segment.asSlice(index * TextureViewDescriptor.BYTES, TextureViewDescriptor.BYTES);
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * TextureViewDescriptor.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * TextureViewDescriptor.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * TextureViewDescriptor.BYTES,
                (end - start) * TextureViewDescriptor.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * TextureViewDescriptor.BYTES));
        }

        public TextureViewDescriptor[] toArray() {
            TextureViewDescriptor[] ret = new TextureViewDescriptor[(int) size()];
        ...
        @Override
        public @NotNull Iterator<TextureViewDescriptor> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<TextureViewDescriptor> {
            ...
            @Override
            public TextureViewDescriptor next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                TextureViewDescriptor ret = new TextureViewDescriptor(segment.asSlice(0, TextureViewDescriptor.BYTES));
                segment = segment.asSlice(TextureViewDescriptor.BYTES);
                return ret;
            }
    ...
    public static TextureViewDescriptor allocate(Arena arena) {
        return new TextureViewDescriptor(arena.allocate(LAYOUT));
    }

    public static TextureViewDescriptor.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        return new TextureViewDescriptor.Ptr(segment);
    }

    public static TextureViewDescriptor clone(Arena arena, TextureViewDescriptor src) {
        TextureViewDescriptor ret = allocate(arena);
        ret.segment.copyFrom(src.segment);
        return ret;
    }

    public TextureViewDescriptor nextInChain(@Nullable IChainedStruct value) {
    ...
    public TextureViewDescriptor label(@NotNull StringView value) {
        MemorySegment.copy(value.segment(), 0, segment, OFFSET$label, SIZE$label);
        return this;
    }

    public TextureViewDescriptor label(Consumer<@NotNull StringView> consumer) {
    ...
    public TextureViewDescriptor format(@EnumType(TextureFormat.class) int value) {
        segment.set(LAYOUT$format, OFFSET$format, value);
        return this;
    }

    public @EnumType(TextureViewDimension.class) int dimension() {
        return segment.get(LAYOUT$dimension, OFFSET$dimension);
    }

    public TextureViewDescriptor dimension(@EnumType(TextureViewDimension.class) int value) {
    ...
    public TextureViewDescriptor baseMipLevel(@Unsigned int value) {
    ...
    public TextureViewDescriptor mipLevelCount(@Unsigned int value) {
    ...
    public TextureViewDescriptor baseArrayLayer(@Unsigned int value) {
    ...
    public TextureViewDescriptor arrayLayerCount(@Unsigned int value) {
        segment.set(LAYOUT$arrayLayerCount, OFFSET$arrayLayerCount, value);
        return this;
    }

    public @EnumType(TextureAspect.class) int aspect() {
        return segment.get(LAYOUT$aspect, OFFSET$aspect);
    }

    public TextureViewDescriptor aspect(@EnumType(TextureAspect.class) int value) {
    ...
    public TextureViewDescriptor usage(@EnumType(TextureUsage.class) long value) {
