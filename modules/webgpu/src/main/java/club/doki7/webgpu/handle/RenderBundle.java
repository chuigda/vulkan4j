...
/// Represents an opaque handle type {@code RenderBundle}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record RenderBundle(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code RenderBundle} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<RenderBundle> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable RenderBundle read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new RenderBundle(s);
        }

        public void write(@Nullable RenderBundle value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable RenderBundle read(long index) {
        ...
        public void write(long index, @Nullable RenderBundle value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable RenderBundle[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable RenderBundle value0, @Nullable RenderBundle ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable RenderBundle[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable RenderBundle value0, @Nullable RenderBundle ...values) {
        ...
        @Override
        public @NotNull Iterator<RenderBundle> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<RenderBundle> {
