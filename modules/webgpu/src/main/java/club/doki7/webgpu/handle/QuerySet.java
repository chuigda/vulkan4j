...
/// Represents an opaque handle type {@code QuerySet}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record QuerySet(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code QuerySet} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<QuerySet> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable QuerySet read() {
        ...
        public void write(@Nullable QuerySet value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable QuerySet read(long index) {
        ...
        public void write(long index, @Nullable QuerySet value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable QuerySet[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable QuerySet value0, @Nullable QuerySet ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable QuerySet[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable QuerySet value0, @Nullable QuerySet ...values) {
        ...
        @Override
        public @NotNull Iterator<QuerySet> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<QuerySet> {
