...
/// Represents an opaque handle type {@code Adapter}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record Adapter(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code Adapter} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<Adapter> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable Adapter read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new Adapter(s);
        }

        public void write(@Nullable Adapter value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable Adapter read(long index) {
        ...
        public void write(long index, @Nullable Adapter value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable Adapter[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable Adapter value0, @Nullable Adapter ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable Adapter[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable Adapter value0, @Nullable Adapter ...values) {
        ...
        @Override
        public @NotNull Iterator<Adapter> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<Adapter> {
