...
/// Represents an opaque handle type {@code WGPUQuerySet}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPUQuerySet(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPUQuerySet} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPUQuerySet> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPUQuerySet read() {
        ...
        public void write(@Nullable WGPUQuerySet value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPUQuerySet read(long index) {
        ...
        public void write(long index, @Nullable WGPUQuerySet value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPUQuerySet[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPUQuerySet value0, @Nullable WGPUQuerySet ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPUQuerySet @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPUQuerySet> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPUQuerySet value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPUQuerySet value0, @Nullable WGPUQuerySet ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPUQuerySet> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPUQuerySet> {
            ...
            @Override
            public @Nullable WGPUQuerySet next() {
                if (!hasNext()) {
