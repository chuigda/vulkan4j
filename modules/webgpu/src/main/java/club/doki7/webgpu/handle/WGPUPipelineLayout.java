...
/// Represents an opaque handle type {@code WGPUPipelineLayout}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPUPipelineLayout(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPUPipelineLayout} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPUPipelineLayout> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPUPipelineLayout read() {
        ...
        public void write(@Nullable WGPUPipelineLayout value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPUPipelineLayout read(long index) {
        ...
        public void write(long index, @Nullable WGPUPipelineLayout value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPUPipelineLayout[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPUPipelineLayout value0, @Nullable WGPUPipelineLayout ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPUPipelineLayout @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPUPipelineLayout> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPUPipelineLayout value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPUPipelineLayout value0, @Nullable WGPUPipelineLayout ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPUPipelineLayout> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPUPipelineLayout> {
            ...
            @Override
            public @Nullable WGPUPipelineLayout next() {
                if (!hasNext()) {
