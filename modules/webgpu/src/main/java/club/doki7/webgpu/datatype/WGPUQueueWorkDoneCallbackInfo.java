...
///     WGPUQueueWorkDoneCallback callback; // optional // @link substring="WGPUQueueWorkDoneCallback" target="IWGPUQueueWorkDoneCallback" @link substring="callback" target="#callback"
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPUQueueWorkDoneCallbackInfo(@NotNull MemorySegment segment) implements IWGPUQueueWorkDoneCallbackInfo {
    /// Represents a pointer to / an array of null structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link WGPUQueueWorkDoneCallbackInfo}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IWGPUQueueWorkDoneCallbackInfo to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code WGPUQueueWorkDoneCallbackInfo.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IWGPUQueueWorkDoneCallbackInfo, Iterable<WGPUQueueWorkDoneCallbackInfo> {
        public long size() {
            return segment.byteSize() / WGPUQueueWorkDoneCallbackInfo.BYTES;
        }
        ...
        public @NotNull WGPUQueueWorkDoneCallbackInfo at(long index) {
            return new WGPUQueueWorkDoneCallbackInfo(segment.asSlice(index * WGPUQueueWorkDoneCallbackInfo.BYTES, WGPUQueueWorkDoneCallbackInfo.BYTES));
        }

        public WGPUQueueWorkDoneCallbackInfo.Ptr at(long index, @NotNull Consumer<@NotNull WGPUQueueWorkDoneCallbackInfo> consumer) {
            consumer.accept(at(index));
            return this;
        }

        public void write(long index, @NotNull WGPUQueueWorkDoneCallbackInfo value) {
            MemorySegment s = segment.asSlice(index * WGPUQueueWorkDoneCallbackInfo.BYTES, WGPUQueueWorkDoneCallbackInfo.BYTES);
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * WGPUQueueWorkDoneCallbackInfo.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * WGPUQueueWorkDoneCallbackInfo.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * WGPUQueueWorkDoneCallbackInfo.BYTES,
                (end - start) * WGPUQueueWorkDoneCallbackInfo.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * WGPUQueueWorkDoneCallbackInfo.BYTES));
        }

        public WGPUQueueWorkDoneCallbackInfo[] toArray() {
            WGPUQueueWorkDoneCallbackInfo[] ret = new WGPUQueueWorkDoneCallbackInfo[(int) size()];
        ...
        @Override
        public @NotNull Iterator<WGPUQueueWorkDoneCallbackInfo> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<WGPUQueueWorkDoneCallbackInfo> {
            ...
            @Override
            public WGPUQueueWorkDoneCallbackInfo next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                WGPUQueueWorkDoneCallbackInfo ret = new WGPUQueueWorkDoneCallbackInfo(segment.asSlice(0, WGPUQueueWorkDoneCallbackInfo.BYTES));
                segment = segment.asSlice(WGPUQueueWorkDoneCallbackInfo.BYTES);
                return ret;
            }
    ...
    public static WGPUQueueWorkDoneCallbackInfo allocate(Arena arena) {
        return new WGPUQueueWorkDoneCallbackInfo(arena.allocate(LAYOUT));
    }

    public static WGPUQueueWorkDoneCallbackInfo.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        return new WGPUQueueWorkDoneCallbackInfo.Ptr(segment);
    }

    public static WGPUQueueWorkDoneCallbackInfo clone(Arena arena, WGPUQueueWorkDoneCallbackInfo src) {
        WGPUQueueWorkDoneCallbackInfo ret = allocate(arena);
        ret.segment.copyFrom(src.segment);
        return ret;
    }

    public WGPUQueueWorkDoneCallbackInfo nextInChain(@Nullable IWGPUChainedStruct value) {
    ...
    public WGPUQueueWorkDoneCallbackInfo mode(@EnumType(WGPUCallbackMode.class) int value) {
        segment.set(LAYOUT$mode, OFFSET$mode, value);
        return this;
    }

    public @Pointer(comment="WGPUQueueWorkDoneCallback") @NotNull MemorySegment callback() {
    ...
    public WGPUQueueWorkDoneCallbackInfo userdata1(@Pointer(comment="void*") @NotNull MemorySegment value) {
        segment.set(LAYOUT$userdata1, OFFSET$userdata1, value);
        return this;
    }

    public WGPUQueueWorkDoneCallbackInfo userdata1(@Nullable IPointer pointer) {
    ...
    public WGPUQueueWorkDoneCallbackInfo userdata2(@Pointer(comment="void*") @NotNull MemorySegment value) {
        segment.set(LAYOUT$userdata2, OFFSET$userdata2, value);
        return this;
    }

    public WGPUQueueWorkDoneCallbackInfo userdata2(@Nullable IPointer pointer) {
