...
/// Represents an opaque handle type {@code RenderPipeline}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record RenderPipeline(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code RenderPipeline} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<RenderPipeline> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable RenderPipeline read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new RenderPipeline(s);
        }

        public void write(@Nullable RenderPipeline value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable RenderPipeline read(long index) {
        ...
        public void write(long index, @Nullable RenderPipeline value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable RenderPipeline[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable RenderPipeline value0, @Nullable RenderPipeline ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable RenderPipeline[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable RenderPipeline value0, @Nullable RenderPipeline ...values) {
        ...
        @Override
        public @NotNull Iterator<RenderPipeline> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<RenderPipeline> {
