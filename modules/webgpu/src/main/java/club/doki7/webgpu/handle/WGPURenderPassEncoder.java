...
/// Represents an opaque handle type {@code WGPURenderPassEncoder}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPURenderPassEncoder(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPURenderPassEncoder} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPURenderPassEncoder> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPURenderPassEncoder read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new WGPURenderPassEncoder(s);
        }

        public void write(@Nullable WGPURenderPassEncoder value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPURenderPassEncoder read(long index) {
        ...
        public void write(long index, @Nullable WGPURenderPassEncoder value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPURenderPassEncoder[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPURenderPassEncoder value0, @Nullable WGPURenderPassEncoder ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPURenderPassEncoder @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPURenderPassEncoder> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPURenderPassEncoder value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPURenderPassEncoder value0, @Nullable WGPURenderPassEncoder ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPURenderPassEncoder> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPURenderPassEncoder> {
            ...
            @Override
            public @Nullable WGPURenderPassEncoder next() {
                if (!hasNext()) {
