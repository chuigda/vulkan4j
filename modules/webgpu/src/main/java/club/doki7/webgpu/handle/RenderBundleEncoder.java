...
/// Represents an opaque handle type {@code RenderBundleEncoder}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record RenderBundleEncoder(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code RenderBundleEncoder} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<RenderBundleEncoder> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable RenderBundleEncoder read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new RenderBundleEncoder(s);
        }

        public void write(@Nullable RenderBundleEncoder value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable RenderBundleEncoder read(long index) {
        ...
        public void write(long index, @Nullable RenderBundleEncoder value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable RenderBundleEncoder[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable RenderBundleEncoder value0, @Nullable RenderBundleEncoder ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable RenderBundleEncoder[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable RenderBundleEncoder value0, @Nullable RenderBundleEncoder ...values) {
        ...
        @Override
        public @NotNull Iterator<RenderBundleEncoder> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<RenderBundleEncoder> {
