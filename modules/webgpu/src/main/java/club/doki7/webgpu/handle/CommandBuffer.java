...
/// Represents an opaque handle type {@code CommandBuffer}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record CommandBuffer(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code CommandBuffer} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<CommandBuffer> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable CommandBuffer read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new CommandBuffer(s);
        }

        public void write(@Nullable CommandBuffer value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable CommandBuffer read(long index) {
        ...
        public void write(long index, @Nullable CommandBuffer value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable CommandBuffer[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable CommandBuffer value0, @Nullable CommandBuffer ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable CommandBuffer[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable CommandBuffer value0, @Nullable CommandBuffer ...values) {
        ...
        @Override
        public @NotNull Iterator<CommandBuffer> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<CommandBuffer> {
