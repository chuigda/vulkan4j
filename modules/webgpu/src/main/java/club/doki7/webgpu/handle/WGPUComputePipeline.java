...
/// Represents an opaque handle type {@code WGPUComputePipeline}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPUComputePipeline(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPUComputePipeline} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPUComputePipeline> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPUComputePipeline read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new WGPUComputePipeline(s);
        }

        public void write(@Nullable WGPUComputePipeline value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPUComputePipeline read(long index) {
        ...
        public void write(long index, @Nullable WGPUComputePipeline value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPUComputePipeline[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPUComputePipeline value0, @Nullable WGPUComputePipeline ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPUComputePipeline @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPUComputePipeline> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPUComputePipeline value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPUComputePipeline value0, @Nullable WGPUComputePipeline ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPUComputePipeline> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPUComputePipeline> {
            ...
            @Override
            public @Nullable WGPUComputePipeline next() {
                if (!hasNext()) {
