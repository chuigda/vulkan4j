...
@ValueBasedCandidate
@UnsafeConstructor
public record Color(@NotNull MemorySegment segment) implements IColor {
    /// Represents a pointer to / an array of null structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link Color}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IColor to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code Color.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IColor, Iterable<Color> {
        public long size() {
            return segment.byteSize() / Color.BYTES;
        }
        ...
        public @NotNull Color at(long index) {
            return new Color(segment.asSlice(index * Color.BYTES, Color.BYTES));
        }

        public void write(long index, @NotNull Color value) {
            MemorySegment s = segment.asSlice(index * Color.BYTES, Color.BYTES);
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * Color.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * Color.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * Color.BYTES,
                (end - start) * Color.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * Color.BYTES));
        }

        public Color[] toArray() {
            Color[] ret = new Color[(int) size()];
        ...
        private static final class Iter implements Iterator<Color> {
            ...
            public Color next() {
                ...
                Color ret = new Color(segment.asSlice(0, Color.BYTES));
                segment = segment.asSlice(Color.BYTES);
                return ret;
            }
    ...
    public static Color allocate(Arena arena) {
        return new Color(arena.allocate(LAYOUT));
    }

    public static Color.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        return new Color.Ptr(segment);
    }

    public static Color clone(Arena arena, Color src) {
        Color ret = allocate(arena);
