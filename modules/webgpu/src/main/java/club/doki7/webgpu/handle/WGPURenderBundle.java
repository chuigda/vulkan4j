...
/// Represents an opaque handle type {@code WGPURenderBundle}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPURenderBundle(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPURenderBundle} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPURenderBundle> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPURenderBundle read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new WGPURenderBundle(s);
        }

        public void write(@Nullable WGPURenderBundle value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPURenderBundle read(long index) {
        ...
        public void write(long index, @Nullable WGPURenderBundle value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPURenderBundle[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPURenderBundle value0, @Nullable WGPURenderBundle ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPURenderBundle @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPURenderBundle> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPURenderBundle value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPURenderBundle value0, @Nullable WGPURenderBundle ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPURenderBundle> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPURenderBundle> {
            ...
            @Override
            public @Nullable WGPURenderBundle next() {
                if (!hasNext()) {
