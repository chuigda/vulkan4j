...
/// Represents an opaque handle type {@code WGPUBindGroupLayout}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPUBindGroupLayout(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPUBindGroupLayout} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPUBindGroupLayout> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPUBindGroupLayout read() {
        ...
        public void write(@Nullable WGPUBindGroupLayout value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPUBindGroupLayout read(long index) {
        ...
        public void write(long index, @Nullable WGPUBindGroupLayout value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPUBindGroupLayout[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPUBindGroupLayout value0, @Nullable WGPUBindGroupLayout ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPUBindGroupLayout @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPUBindGroupLayout> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPUBindGroupLayout value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPUBindGroupLayout value0, @Nullable WGPUBindGroupLayout ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPUBindGroupLayout> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPUBindGroupLayout> {
            ...
            @Override
            public @Nullable WGPUBindGroupLayout next() {
                if (!hasNext()) {
