...
/// Represents an opaque handle type {@code RenderPassEncoder}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record RenderPassEncoder(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code RenderPassEncoder} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<RenderPassEncoder> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable RenderPassEncoder read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new RenderPassEncoder(s);
        }

        public void write(@Nullable RenderPassEncoder value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable RenderPassEncoder read(long index) {
        ...
        public void write(long index, @Nullable RenderPassEncoder value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable RenderPassEncoder[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable RenderPassEncoder value0, @Nullable RenderPassEncoder ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable RenderPassEncoder[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable RenderPassEncoder value0, @Nullable RenderPassEncoder ...values) {
        ...
        @Override
        public @NotNull Iterator<RenderPassEncoder> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<RenderPassEncoder> {
