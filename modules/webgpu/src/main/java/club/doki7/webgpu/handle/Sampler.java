...
/// Represents an opaque handle type {@code Sampler}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record Sampler(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code Sampler} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<Sampler> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable Sampler read() {
        ...
        public void write(@Nullable Sampler value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable Sampler read(long index) {
        ...
        public void write(long index, @Nullable Sampler value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable Sampler[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable Sampler value0, @Nullable Sampler ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable Sampler[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable Sampler value0, @Nullable Sampler ...values) {
        ...
        @Override
        public @NotNull Iterator<Sampler> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<Sampler> {
