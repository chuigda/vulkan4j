...
/// Represents an opaque handle type {@code WGPUShaderModule}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPUShaderModule(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPUShaderModule} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPUShaderModule> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPUShaderModule read() {
        ...
        public void write(@Nullable WGPUShaderModule value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPUShaderModule read(long index) {
        ...
        public void write(long index, @Nullable WGPUShaderModule value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPUShaderModule[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPUShaderModule value0, @Nullable WGPUShaderModule ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPUShaderModule @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPUShaderModule> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPUShaderModule value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPUShaderModule value0, @Nullable WGPUShaderModule ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPUShaderModule> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPUShaderModule> {
            ...
            @Override
            public @Nullable WGPUShaderModule next() {
                if (!hasNext()) {
