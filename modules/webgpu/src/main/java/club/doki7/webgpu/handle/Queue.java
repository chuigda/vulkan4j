...
/// Represents an opaque handle type {@code Queue}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record Queue(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code Queue} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<Queue> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable Queue read() {
        ...
        public void write(@Nullable Queue value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable Queue read(long index) {
        ...
        public void write(long index, @Nullable Queue value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable Queue[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable Queue value0, @Nullable Queue ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable Queue[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable Queue value0, @Nullable Queue ...values) {
        ...
        @Override
        public @NotNull Iterator<Queue> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<Queue> {
