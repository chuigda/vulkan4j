...
/// Represents an opaque handle type {@code Texture}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record Texture(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code Texture} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<Texture> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable Texture read() {
        ...
        public void write(@Nullable Texture value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable Texture read(long index) {
        ...
        public void write(long index, @Nullable Texture value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable Texture[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable Texture value0, @Nullable Texture ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable Texture[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable Texture value0, @Nullable Texture ...values) {
        ...
        @Override
        public @NotNull Iterator<Texture> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<Texture> {
