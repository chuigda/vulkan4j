...
/// Represents an opaque handle type {@code WGPUSampler}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPUSampler(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPUSampler} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPUSampler> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPUSampler read() {
        ...
        public void write(@Nullable WGPUSampler value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPUSampler read(long index) {
        ...
        public void write(long index, @Nullable WGPUSampler value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPUSampler[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPUSampler value0, @Nullable WGPUSampler ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPUSampler @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPUSampler> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPUSampler value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPUSampler value0, @Nullable WGPUSampler ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPUSampler> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPUSampler> {
            ...
            @Override
            public @Nullable WGPUSampler next() {
                if (!hasNext()) {
