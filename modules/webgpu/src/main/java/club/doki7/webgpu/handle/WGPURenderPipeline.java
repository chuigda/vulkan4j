...
/// Represents an opaque handle type {@code WGPURenderPipeline}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPURenderPipeline(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPURenderPipeline} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPURenderPipeline> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPURenderPipeline read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new WGPURenderPipeline(s);
        }

        public void write(@Nullable WGPURenderPipeline value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPURenderPipeline read(long index) {
        ...
        public void write(long index, @Nullable WGPURenderPipeline value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPURenderPipeline[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPURenderPipeline value0, @Nullable WGPURenderPipeline ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPURenderPipeline @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPURenderPipeline> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPURenderPipeline value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPURenderPipeline value0, @Nullable WGPURenderPipeline ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPURenderPipeline> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPURenderPipeline> {
            ...
            @Override
            public @Nullable WGPURenderPipeline next() {
                if (!hasNext()) {
