...
/// Represents an opaque handle type {@code ComputePassEncoder}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record ComputePassEncoder(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code ComputePassEncoder} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<ComputePassEncoder> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable ComputePassEncoder read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new ComputePassEncoder(s);
        }

        public void write(@Nullable ComputePassEncoder value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable ComputePassEncoder read(long index) {
        ...
        public void write(long index, @Nullable ComputePassEncoder value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable ComputePassEncoder[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable ComputePassEncoder value0, @Nullable ComputePassEncoder ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable ComputePassEncoder[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable ComputePassEncoder value0, @Nullable ComputePassEncoder ...values) {
        ...
        @Override
        public @NotNull Iterator<ComputePassEncoder> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<ComputePassEncoder> {
