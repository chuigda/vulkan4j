...
/// Represents an opaque handle type {@code ComputePipeline}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record ComputePipeline(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code ComputePipeline} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<ComputePipeline> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable ComputePipeline read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new ComputePipeline(s);
        }

        public void write(@Nullable ComputePipeline value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable ComputePipeline read(long index) {
        ...
        public void write(long index, @Nullable ComputePipeline value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable ComputePipeline[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable ComputePipeline value0, @Nullable ComputePipeline ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable ComputePipeline[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable ComputePipeline value0, @Nullable ComputePipeline ...values) {
        ...
        @Override
        public @NotNull Iterator<ComputePipeline> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<ComputePipeline> {
