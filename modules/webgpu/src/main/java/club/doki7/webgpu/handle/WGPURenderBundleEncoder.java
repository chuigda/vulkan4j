...
/// Represents an opaque handle type {@code WGPURenderBundleEncoder}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record WGPURenderBundleEncoder(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code WGPURenderBundleEncoder} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<WGPURenderBundleEncoder> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable WGPURenderBundleEncoder read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new WGPURenderBundleEncoder(s);
        }

        public void write(@Nullable WGPURenderBundleEncoder value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable WGPURenderBundleEncoder read(long index) {
        ...
        public void write(long index, @Nullable WGPURenderBundleEncoder value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable WGPURenderBundleEncoder[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable WGPURenderBundleEncoder value0, @Nullable WGPURenderBundleEncoder ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable WGPURenderBundleEncoder @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable WGPURenderBundleEncoder> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable WGPURenderBundleEncoder value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable WGPURenderBundleEncoder value0, @Nullable WGPURenderBundleEncoder ...values) {
        ...
        @Override
        public @NotNull Iterator<WGPURenderBundleEncoder> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<WGPURenderBundleEncoder> {
            ...
            @Override
            public @Nullable WGPURenderBundleEncoder next() {
                if (!hasNext()) {
