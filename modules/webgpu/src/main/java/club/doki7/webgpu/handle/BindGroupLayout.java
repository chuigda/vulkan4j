...
/// Represents an opaque handle type {@code BindGroupLayout}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record BindGroupLayout(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code BindGroupLayout} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<BindGroupLayout> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable BindGroupLayout read() {
        ...
        public void write(@Nullable BindGroupLayout value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable BindGroupLayout read(long index) {
        ...
        public void write(long index, @Nullable BindGroupLayout value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable BindGroupLayout[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable BindGroupLayout value0, @Nullable BindGroupLayout ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable BindGroupLayout[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable BindGroupLayout value0, @Nullable BindGroupLayout ...values) {
        ...
        @Override
        public @NotNull Iterator<BindGroupLayout> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<BindGroupLayout> {
