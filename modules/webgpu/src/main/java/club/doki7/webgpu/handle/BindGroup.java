...
/// Represents an opaque handle type {@code BindGroup}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record BindGroup(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code BindGroup} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<BindGroup> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable BindGroup read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new BindGroup(s);
        }

        public void write(@Nullable BindGroup value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable BindGroup read(long index) {
        ...
        public void write(long index, @Nullable BindGroup value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable BindGroup[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable BindGroup value0, @Nullable BindGroup ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable BindGroup[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable BindGroup value0, @Nullable BindGroup ...values) {
        ...
        @Override
        public @NotNull Iterator<BindGroup> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<BindGroup> {
