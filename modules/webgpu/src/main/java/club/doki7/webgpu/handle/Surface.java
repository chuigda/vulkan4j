...
/// Represents an opaque handle type {@code Surface}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record Surface(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code Surface} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<Surface> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable Surface read() {
        ...
        public void write(@Nullable Surface value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable Surface read(long index) {
        ...
        public void write(long index, @Nullable Surface value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable Surface[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable Surface value0, @Nullable Surface ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable Surface[] values) {
        ...
        public static Ptr allocateV(Arena arena, @Nullable Surface value0, @Nullable Surface ...values) {
        ...
        @Override
        public @NotNull Iterator<Surface> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<Surface> {
