...
@ValueBasedCandidate
@UnsafeConstructor
public record ALCdevice(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code ALCdevice} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<ALCdevice> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable ALCdevice read() {
        ...
        public void write(@Nullable ALCdevice value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable ALCdevice read(long index) {
        ...
        public void write(long index, @Nullable ALCdevice value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable ALCdevice[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable ALCdevice value0, @Nullable ALCdevice ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable ALCdevice @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable ALCdevice> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable ALCdevice value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable ALCdevice value0, @Nullable ALCdevice ...values) {
        ...
        @Override
        public @NotNull Iterator<ALCdevice> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<ALCdevice> {
