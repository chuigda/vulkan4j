package club.doki7.openal.handle;
...
/// Represents an opaque handle type {@code _GUID}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record _GUID(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code _GUID} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<_GUID> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable _GUID read() {
        ...
        public void write(@Nullable _GUID value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable _GUID read(long index) {
        ...
        public void write(long index, @Nullable _GUID value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable _GUID[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable _GUID value0, @Nullable _GUID ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable _GUID @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable _GUID> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable _GUID value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable _GUID value0, @Nullable _GUID ...values) {
        ...
        @Override
        public @NotNull Iterator<_GUID> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<_GUID> {
