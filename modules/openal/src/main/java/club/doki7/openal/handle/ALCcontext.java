...
@ValueBasedCandidate
@UnsafeConstructor
public record ALCcontext(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code ALCcontext} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<ALCcontext> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable ALCcontext read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new ALCcontext(s);
        }

        public void write(@Nullable ALCcontext value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable ALCcontext read(long index) {
        ...
        public void write(long index, @Nullable ALCcontext value) {
        ...
        public static Ptr allocate(Arena arena, @Nullable ALCcontext @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable ALCcontext> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable ALCcontext value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        private static class Iter implements Iterator<ALCcontext> {
            ...
            @Override
            public @Nullable ALCcontext next() {
                if (!hasNext()) {
