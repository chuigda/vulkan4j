...
public final class GLFWLoader {
    ...
    ///  On Windows it will try {@code "glfw3"}
    ...
    public static ISharedLibrary loadGLFWLibrary() {
        if (System.getProperty("os.name").toLowerCase().contains("windows")) {
            return WindowsLibraryLoader.INSTANCE.loadLibrary("glfw3");
        } else {
            return ILibraryLoader.platformLoader().loadLibrary("glfw");
        }
    }

    /// Load GLFW functions.
    ///
    /// This function is implemented in terms of {@link ISharedLibrary#load}. If any of the functions is not
    ...
    public static GLFW loadGLFW(ISharedLibrary libGLFW) {
        return new GLFW(libGLFW);
    }
}
