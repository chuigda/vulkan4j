...
@ValueBasedCandidate
@UnsafeConstructor
public record GLFWwindow(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code GLFWwindow} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<GLFWwindow> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable GLFWwindow read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new GLFWwindow(s);
        }

        public void write(@Nullable GLFWwindow value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable GLFWwindow read(long index) {
        ...
        public void write(long index, @Nullable GLFWwindow value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable GLFWwindow[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable GLFWwindow value0, @Nullable GLFWwindow ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocateV(Arena arena, @Nullable GLFWwindow value0, @Nullable GLFWwindow ...values) {
        ...
        private static class Iter implements Iterator<GLFWwindow> {
            ...
            @Override
            public @Nullable GLFWwindow next() {
                if (!hasNext()) {
