...
public record GLFWimage(@NotNull MemorySegment segment) implements IGLFWimage {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IGLFWimage, Iterable<GLFWimage> {
        public long size() {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * GLFWimage.BYTES));
        }
        ...
        private static final class Iter implements Iterator<GLFWimage> {
            ...
            @Override
            public GLFWimage next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                GLFWimage ret = new GLFWimage(segment.asSlice(0, GLFWimage.BYTES));
                segment = segment.asSlice(GLFWimage.BYTES);
                return ret;
            }
