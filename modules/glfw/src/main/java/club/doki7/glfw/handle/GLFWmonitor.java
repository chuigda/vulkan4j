...
@ValueBasedCandidate
@UnsafeConstructor
public record GLFWmonitor(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code GLFWmonitor} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<GLFWmonitor> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable GLFWmonitor read() {
        ...
        public void write(@Nullable GLFWmonitor value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable GLFWmonitor read(long index) {
        ...
        public void write(long index, @Nullable GLFWmonitor value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable GLFWmonitor[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable GLFWmonitor value0, @Nullable GLFWmonitor ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable GLFWmonitor @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable GLFWmonitor> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable GLFWmonitor value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable GLFWmonitor value0, @Nullable GLFWmonitor ...values) {
        ...
        @Override
        public @NotNull Iterator<GLFWmonitor> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<GLFWmonitor> {
