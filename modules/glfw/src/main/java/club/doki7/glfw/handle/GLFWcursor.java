...
@ValueBasedCandidate
@UnsafeConstructor
public record GLFWcursor(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code GLFWcursor} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<GLFWcursor> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable GLFWcursor read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new GLFWcursor(s);
        }

        public void write(@Nullable GLFWcursor value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable GLFWcursor read(long index) {
        ...
        public void write(long index, @Nullable GLFWcursor value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable GLFWcursor[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable GLFWcursor value0, @Nullable GLFWcursor ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable GLFWcursor @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable GLFWcursor> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable GLFWcursor value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        @Override
        public @NotNull Iterator<GLFWcursor> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<GLFWcursor> {
            ...
            @Override
            public @Nullable GLFWcursor next() {
                if (!hasNext()) {
