...
/// Represents an opaque handle type {@code shaderc_compiler_t}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record ShadercCompiler(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code shaderc_compiler_t} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<ShadercCompiler> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable ShadercCompiler read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new ShadercCompiler(s);
        }

        public void write(@Nullable ShadercCompiler value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable ShadercCompiler read(long index) {
        ...
        public void write(long index, @Nullable ShadercCompiler value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable ShadercCompiler[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable ShadercCompiler value0, @Nullable ShadercCompiler ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable ShadercCompiler @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable ShadercCompiler> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable ShadercCompiler value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable ShadercCompiler value0, @Nullable ShadercCompiler ...values) {
        ...
        @Override
        public @NotNull Iterator<ShadercCompiler> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<ShadercCompiler> {
            ...
            @Override
            public @Nullable ShadercCompiler next() {
                if (!hasNext()) {
