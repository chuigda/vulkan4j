...
/// Represents an opaque handle type {@code shaderc_compilation_result_t}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record ShadercCompilationResult(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code shaderc_compilation_result_t} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<ShadercCompilationResult> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable ShadercCompilationResult read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new ShadercCompilationResult(s);
        }

        public void write(@Nullable ShadercCompilationResult value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable ShadercCompilationResult read(long index) {
        ...
        public void write(long index, @Nullable ShadercCompilationResult value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable ShadercCompilationResult[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable ShadercCompilationResult value0, @Nullable ShadercCompilationResult ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable ShadercCompilationResult @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable ShadercCompilationResult> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable ShadercCompilationResult value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable ShadercCompilationResult value0, @Nullable ShadercCompilationResult ...values) {
        ...
        @Override
        public @NotNull Iterator<ShadercCompilationResult> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<ShadercCompilationResult> {
            ...
            @Override
            public @Nullable ShadercCompilationResult next() {
                if (!hasNext()) {
