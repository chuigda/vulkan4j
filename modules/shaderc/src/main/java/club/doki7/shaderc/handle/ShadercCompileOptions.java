...
/// Represents an opaque handle type {@code shaderc_compile_options_t}.
///
/// ## Contracts
///
...
@ValueBasedCandidate
@UnsafeConstructor
public record ShadercCompileOptions(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code shaderc_compile_options_t} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<ShadercCompileOptions> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable ShadercCompileOptions read() {
            MemorySegment s = readRaw();
            if (s.equals(MemorySegment.NULL)) {
                return null;
            }
            return new ShadercCompileOptions(s);
        }

        public void write(@Nullable ShadercCompileOptions value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable ShadercCompileOptions read(long index) {
        ...
        public void write(long index, @Nullable ShadercCompileOptions value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable ShadercCompileOptions[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable ShadercCompileOptions value0, @Nullable ShadercCompileOptions ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocateV(Arena arena, @Nullable ShadercCompileOptions value0, @Nullable ShadercCompileOptions ...values) {
        ...
        @Override
        public @NotNull Iterator<ShadercCompileOptions> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<ShadercCompileOptions> {
            ...
            @Override
            public @Nullable ShadercCompileOptions next() {
                if (!hasNext()) {
