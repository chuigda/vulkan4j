...
public record VmaVirtualAllocationInfo(@NotNull MemorySegment segment) implements IVmaVirtualAllocationInfo {
    ...
    ///
    /// Technically speaking, this type has no difference with {@link VmaVirtualAllocationInfo}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVmaVirtualAllocationInfo to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VmaVirtualAllocationInfo.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVmaVirtualAllocationInfo, Iterable<VmaVirtualAllocationInfo> {
        public long size() {
            return segment.byteSize() / VmaVirtualAllocationInfo.BYTES;
        }
        ...
        public @NotNull VmaVirtualAllocationInfo at(long index) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VmaVirtualAllocationInfo.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VmaVirtualAllocationInfo.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VmaVirtualAllocationInfo.BYTES,
                (end - start) * VmaVirtualAllocationInfo.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VmaVirtualAllocationInfo.BYTES));
        }

        public VmaVirtualAllocationInfo[] toArray() {
            VmaVirtualAllocationInfo[] ret = new VmaVirtualAllocationInfo[(int) size()];
        ...
        private static final class Iter implements Iterator<VmaVirtualAllocationInfo> {
            ...
            @Override
            public VmaVirtualAllocationInfo next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VmaVirtualAllocationInfo ret = new VmaVirtualAllocationInfo(segment.asSlice(0, VmaVirtualAllocationInfo.BYTES));
                segment = segment.asSlice(VmaVirtualAllocationInfo.BYTES);
                return ret;
            }
    ...
    public static VmaVirtualAllocationInfo.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        return new VmaVirtualAllocationInfo.Ptr(segment);
    }
    ...
    public VmaVirtualAllocationInfo offset(@NativeType("VkDeviceSize") @Unsigned long value) {
    ...
    public VmaVirtualAllocationInfo size(@NativeType("VkDeviceSize") @Unsigned long value) {
