...
@ValueBasedCandidate
@UnsafeConstructor
public record VmaPool(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code VmaPool} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<VmaPool> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable VmaPool read() {
        ...
        public void write(@Nullable VmaPool value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable VmaPool read(long index) {
        ...
        public void write(long index, @Nullable VmaPool value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable VmaPool[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable VmaPool value0, @Nullable VmaPool ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable VmaPool @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable VmaPool> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable VmaPool value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable VmaPool value0, @Nullable VmaPool ...values) {
        ...
        @Override
        public @NotNull Iterator<VmaPool> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<VmaPool> {
