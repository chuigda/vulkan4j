...
public record VmaVirtualAllocation(@NotNull MemorySegment segment) implements IPointer {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<VmaVirtualAllocation> {
        public long size() {
        ...
        public void write(@Nullable VmaVirtualAllocation[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable VmaVirtualAllocation value0, @Nullable VmaVirtualAllocation ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable VmaVirtualAllocation @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable VmaVirtualAllocation> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable VmaVirtualAllocation value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable VmaVirtualAllocation value0, @Nullable VmaVirtualAllocation ...values) {
