...
@ValueBasedCandidate
@UnsafeConstructor
public record VmaAllocation(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code VmaAllocation} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<VmaAllocation> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable VmaAllocation read() {
        ...
        public void write(@Nullable VmaAllocation value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable VmaAllocation read(long index) {
        ...
        public void write(long index, @Nullable VmaAllocation value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable VmaAllocation[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable VmaAllocation value0, @Nullable VmaAllocation ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable VmaAllocation @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable VmaAllocation> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable VmaAllocation value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable VmaAllocation value0, @Nullable VmaAllocation ...values) {
        ...
        @Override
        public @NotNull Iterator<VmaAllocation> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the handles.
        private static class Iter implements Iterator<VmaAllocation> {
