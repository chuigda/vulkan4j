...
@ValueBasedCandidate
@UnsafeConstructor
public record VmaVirtualBlock(@NotNull MemorySegment segment) implements IPointer {
    /// Represents a pointer to {@code VmaVirtualBlock} handle(s) in native memory.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<VmaVirtualBlock> {
        public long size() {
            return segment.byteSize() / ValueLayout.ADDRESS.byteSize();
        }

        public @Nullable VmaVirtualBlock read() {
        ...
        public void write(@Nullable VmaVirtualBlock value) {
            writeRaw(value == null ? MemorySegment.NULL : value.segment());
        }

        public @Nullable VmaVirtualBlock read(long index) {
        ...
        public void write(long index, @Nullable VmaVirtualBlock value) {
            writeRaw(index, value == null ? MemorySegment.NULL : value.segment());
        }

        public void write(@Nullable VmaVirtualBlock[] values) {
            for (int i = 0; i < values.length; i++) {
                write(i, values[i]);
            }
        }

        public void writeV(@Nullable VmaVirtualBlock value0, @Nullable VmaVirtualBlock ...values) {
            write(value0);
            for (int i = 0; i < values.length; i++) {
                write(i + 1, values[i]);
            }
        ...
        public static Ptr allocate(Arena arena, @Nullable VmaVirtualBlock @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable VmaVirtualBlock> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable VmaVirtualBlock value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
        ...
        public static Ptr allocateV(Arena arena, @Nullable VmaVirtualBlock value0, @Nullable VmaVirtualBlock ...values) {
