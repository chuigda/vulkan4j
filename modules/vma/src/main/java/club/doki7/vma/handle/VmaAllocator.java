...
public record VmaAllocator(@NotNull MemorySegment segment) implements IPointer {
    ...
    public record Ptr(@NotNull MemorySegment segment) implements IPointer, Iterable<VmaAllocator> {
        ...
        public static Ptr allocate(Arena arena, @Nullable VmaAllocator @NotNull [] values) {
        ...
        public static Ptr allocate(Arena arena, @NotNull Collection<@Nullable VmaAllocator> values) {
            Ptr ret = allocate(arena, values.size());
            int i = 0;
            for (@Nullable VmaAllocator value : values) {
                ret.write(i, value);
                i += 1;
            }
            return ret;
