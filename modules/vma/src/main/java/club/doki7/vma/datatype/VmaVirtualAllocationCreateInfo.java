...
public record VmaVirtualAllocationCreateInfo(@NotNull MemorySegment segment) implements IVmaVirtualAllocationCreateInfo {
    ...
    ///
    /// Technically speaking, this type has no difference with {@link VmaVirtualAllocationCreateInfo}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVmaVirtualAllocationCreateInfo to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VmaVirtualAllocationCreateInfo.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVmaVirtualAllocationCreateInfo, Iterable<VmaVirtualAllocationCreateInfo> {
        public long size() {
            return segment.byteSize() / VmaVirtualAllocationCreateInfo.BYTES;
        }
        ...
        public @NotNull VmaVirtualAllocationCreateInfo at(long index) {
        ...
        public @NotNull Ptr reinterpret(long newSize) {
            return new Ptr(segment.reinterpret(newSize * VmaVirtualAllocationCreateInfo.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VmaVirtualAllocationCreateInfo.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VmaVirtualAllocationCreateInfo.BYTES,
                (end - start) * VmaVirtualAllocationCreateInfo.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VmaVirtualAllocationCreateInfo.BYTES));
        }

        public VmaVirtualAllocationCreateInfo[] toArray() {
            VmaVirtualAllocationCreateInfo[] ret = new VmaVirtualAllocationCreateInfo[(int) size()];
        ...
        @Override
        public @NotNull Iterator<VmaVirtualAllocationCreateInfo> iterator() {
            return new Iter(this.segment());
        }

        /// An iterator over the structures.
        private static final class Iter implements Iterator<VmaVirtualAllocationCreateInfo> {
            ...
            @Override
            public VmaVirtualAllocationCreateInfo next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VmaVirtualAllocationCreateInfo ret = new VmaVirtualAllocationCreateInfo(segment.asSlice(0, VmaVirtualAllocationCreateInfo.BYTES));
                segment = segment.asSlice(VmaVirtualAllocationCreateInfo.BYTES);
                return ret;
            }
    ...
    public static VmaVirtualAllocationCreateInfo.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        return new VmaVirtualAllocationCreateInfo.Ptr(segment);
    }
    ...
    public @MagicConstant(valuesFromClass = VmaVirtualAllocationCreateFlags.class) int flags() {
        return segment.get(LAYOUT$flags, OFFSET$flags);
    }

    public VmaVirtualAllocationCreateInfo flags(@MagicConstant(valuesFromClass = VmaVirtualAllocationCreateFlags.class) int value) {
