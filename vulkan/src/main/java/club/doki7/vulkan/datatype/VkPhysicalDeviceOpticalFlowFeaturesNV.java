...
///
/// The {@link VkPhysicalDeviceOpticalFlowFeaturesNV#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceOpticalFlowFeaturesNV#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceOpticalFlowFeaturesNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceOpticalFlowFeaturesNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceOpticalFlowFeaturesNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_OPTICAL_FLOW_FEATURES_NV);
        }
        return ret;
    }
