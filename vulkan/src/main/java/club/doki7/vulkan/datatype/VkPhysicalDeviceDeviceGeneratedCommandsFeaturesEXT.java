...
///
/// The {@code allocate} ({@link VkPhysicalDeviceDeviceGeneratedCommandsFeaturesEXT#allocate(Arena)}, {@link VkPhysicalDeviceDeviceGeneratedCommandsFeaturesEXT#allocate(Arena, int)})
...
public record VkPhysicalDeviceDeviceGeneratedCommandsFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceDeviceGeneratedCommandsFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceDeviceGeneratedCommandsFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_DEVICE_GENERATED_COMMANDS_FEATURES_EXT);
        }
        return ret;
    }
