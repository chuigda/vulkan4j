...
///
/// The {@code allocate} ({@link VkPhysicalDeviceShaderMaximalReconvergenceFeaturesKHR#allocate(Arena)}, {@link VkPhysicalDeviceShaderMaximalReconvergenceFeaturesKHR#allocate(Arena, int)})
...
public record VkPhysicalDeviceShaderMaximalReconvergenceFeaturesKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceShaderMaximalReconvergenceFeaturesKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceShaderMaximalReconvergenceFeaturesKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SHADER_MAXIMAL_RECONVERGENCE_FEATURES_KHR);
        }
        return ret;
    }
