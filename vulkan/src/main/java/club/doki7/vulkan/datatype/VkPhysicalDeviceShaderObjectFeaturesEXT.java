...
///
/// The {@link VkPhysicalDeviceShaderObjectFeaturesEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceShaderObjectFeaturesEXT#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceShaderObjectFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceShaderObjectFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceShaderObjectFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SHADER_OBJECT_FEATURES_EXT);
        }
        return ret;
    }
