...
///
/// The {@link VkSemaphoreSignalInfo#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkSemaphoreSignalInfo#autoInit} to initialize these fields manually for
...
public record VkSemaphoreSignalInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkSemaphoreSignalInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkSemaphoreSignalInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.SEMAPHORE_SIGNAL_INFO);
        }
        return ret;
    }
