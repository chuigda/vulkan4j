...
///
/// The {@code allocate} ({@link VkSemaphoreGetSciSyncInfoNV#allocate(Arena)}, {@link VkSemaphoreGetSciSyncInfoNV#allocate(Arena, int)})
...
public record VkSemaphoreGetSciSyncInfoNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkSemaphoreGetSciSyncInfoNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkSemaphoreGetSciSyncInfoNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.SEMAPHORE_GET_SCI_SYNC_INFO_NV);
        }
        return ret;
    }
