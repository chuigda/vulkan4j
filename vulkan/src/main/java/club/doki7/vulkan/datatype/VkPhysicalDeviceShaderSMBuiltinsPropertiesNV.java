...
///
/// The {@code allocate} ({@link VkPhysicalDeviceShaderSMBuiltinsPropertiesNV#allocate(Arena)}, {@link VkPhysicalDeviceShaderSMBuiltinsPropertiesNV#allocate(Arena, int)})
...
public record VkPhysicalDeviceShaderSMBuiltinsPropertiesNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceShaderSMBuiltinsPropertiesNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceShaderSMBuiltinsPropertiesNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SHADER_SM_BUILTINS_PROPERTIES_NV);
        }
        return ret;
    }
