...
///     uint64_t firstDrawTimestamp; // @link substring="firstDrawTimestamp" target="#firstDrawTimestamp"
///     uint64_t swapBufferTimestamp; // @link substring="swapBufferTimestamp" target="#swapBufferTimestamp"
...
///
/// The {@code allocate} ({@link VkAmigoProfilingSubmitInfoSEC#allocate(Arena)}, {@link VkAmigoProfilingSubmitInfoSEC#allocate(Arena, long)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkAmigoProfilingSubmitInfoSEC(@NotNull MemorySegment segment) implements IVkAmigoProfilingSubmitInfoSEC {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkAmigoProfilingSubmitInfoSEC.html"><code>VkAmigoProfilingSubmitInfoSEC</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkAmigoProfilingSubmitInfoSEC}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkAmigoProfilingSubmitInfoSEC to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkAmigoProfilingSubmitInfoSEC.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkAmigoProfilingSubmitInfoSEC, Iterable<VkAmigoProfilingSubmitInfoSEC> {
        public long size() {
            return segment.byteSize() / VkAmigoProfilingSubmitInfoSEC.BYTES;
        }
        ...
        public @NotNull VkAmigoProfilingSubmitInfoSEC at(long index) {
        ...
        public @NotNull Ptr reinterpret(long index) {
            return new Ptr(segment.asSlice(index * VkAmigoProfilingSubmitInfoSEC.BYTES, VkAmigoProfilingSubmitInfoSEC.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkAmigoProfilingSubmitInfoSEC.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkAmigoProfilingSubmitInfoSEC.BYTES,
                (end - start) * VkAmigoProfilingSubmitInfoSEC.BYTES
            ));
        }
        ...
        public static final class Iter implements Iterator<VkAmigoProfilingSubmitInfoSEC> {
            ...
            public boolean hasNext() {
                return (segment.byteSize() / VkAmigoProfilingSubmitInfoSEC.BYTES) > 0;
            }

            @Override
            public VkAmigoProfilingSubmitInfoSEC next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkAmigoProfilingSubmitInfoSEC ret = new VkAmigoProfilingSubmitInfoSEC(segment.asSlice(0, VkAmigoProfilingSubmitInfoSEC.BYTES));
                segment = segment.asSlice(VkAmigoProfilingSubmitInfoSEC.BYTES);
                return ret;
            }
    ...
    public static VkAmigoProfilingSubmitInfoSEC.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkAmigoProfilingSubmitInfoSEC.Ptr ret = new VkAmigoProfilingSubmitInfoSEC.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.AMIGO_PROFILING_SUBMIT_INFO_SEC);
        }
        return ret;
    }
