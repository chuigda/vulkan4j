...
///
/// The {@link VkCopyImageToMemoryInfo#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkCopyImageToMemoryInfo#autoInit} to initialize these fields manually for
...
public record VkCopyImageToMemoryInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkCopyImageToMemoryInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkCopyImageToMemoryInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.COPY_IMAGE_TO_MEMORY_INFO);
        }
        return ret;
    }
