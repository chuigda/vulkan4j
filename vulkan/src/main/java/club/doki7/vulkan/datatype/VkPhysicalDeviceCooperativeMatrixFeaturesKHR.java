...
///
/// The {@code allocate} ({@link VkPhysicalDeviceCooperativeMatrixFeaturesKHR#allocate(Arena)}, {@link VkPhysicalDeviceCooperativeMatrixFeaturesKHR#allocate(Arena, int)})
...
public record VkPhysicalDeviceCooperativeMatrixFeaturesKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceCooperativeMatrixFeaturesKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceCooperativeMatrixFeaturesKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_COOPERATIVE_MATRIX_FEATURES_KHR);
        }
        return ret;
    }
