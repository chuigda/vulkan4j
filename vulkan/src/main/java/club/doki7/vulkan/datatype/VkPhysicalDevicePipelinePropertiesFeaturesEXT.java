...
///
/// The {@code allocate} ({@link VkPhysicalDevicePipelinePropertiesFeaturesEXT#allocate(Arena)}, {@link VkPhysicalDevicePipelinePropertiesFeaturesEXT#allocate(Arena, int)})
...
public record VkPhysicalDevicePipelinePropertiesFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDevicePipelinePropertiesFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDevicePipelinePropertiesFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_PIPELINE_PROPERTIES_FEATURES_EXT);
        }
        return ret;
    }
