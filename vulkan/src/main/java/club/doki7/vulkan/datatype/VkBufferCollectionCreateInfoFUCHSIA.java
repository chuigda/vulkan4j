...
///
/// The {@link VkBufferCollectionCreateInfoFUCHSIA#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkBufferCollectionCreateInfoFUCHSIA#autoInit} to initialize these fields manually for
...
public record VkBufferCollectionCreateInfoFUCHSIA(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkBufferCollectionCreateInfoFUCHSIA[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkBufferCollectionCreateInfoFUCHSIA(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.BUFFER_COLLECTION_CREATE_INFO_FUCHSIA);
        }
        return ret;
    }
