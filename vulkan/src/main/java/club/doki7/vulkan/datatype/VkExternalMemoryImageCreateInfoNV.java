...
///
/// The {@code allocate} ({@link VkExternalMemoryImageCreateInfoNV#allocate(Arena)}, {@link VkExternalMemoryImageCreateInfoNV#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkExternalMemoryImageCreateInfoNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkExternalMemoryImageCreateInfoNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkExternalMemoryImageCreateInfoNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.EXTERNAL_MEMORY_IMAGE_CREATE_INFO_NV);
        }
        return ret;
    }
