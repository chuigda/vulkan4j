...
///
/// The {@code allocate} ({@link VkPhysicalDeviceShaderSMBuiltinsFeaturesNV#allocate(Arena)}, {@link VkPhysicalDeviceShaderSMBuiltinsFeaturesNV#allocate(Arena, int)})
...
public record VkPhysicalDeviceShaderSMBuiltinsFeaturesNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceShaderSMBuiltinsFeaturesNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceShaderSMBuiltinsFeaturesNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SHADER_SM_BUILTINS_FEATURES_NV);
        }
        return ret;
    }
