...
///
/// The {@link VkRenderingAreaInfo#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkRenderingAreaInfo#autoInit} to initialize these fields manually for
...
public record VkRenderingAreaInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkRenderingAreaInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkRenderingAreaInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.RENDERING_AREA_INFO);
        }
        return ret;
    }
