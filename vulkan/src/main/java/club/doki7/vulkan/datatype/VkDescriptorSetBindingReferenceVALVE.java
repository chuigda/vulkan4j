...
///
/// The {@code allocate} ({@link VkDescriptorSetBindingReferenceVALVE#allocate(Arena)}, {@link VkDescriptorSetBindingReferenceVALVE#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkDescriptorSetBindingReferenceVALVE#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkDescriptorSetBindingReferenceVALVE(@NotNull MemorySegment segment) implements IVkDescriptorSetBindingReferenceVALVE {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkDescriptorSetBindingReferenceVALVE.html"><code>VkDescriptorSetBindingReferenceVALVE</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkDescriptorSetBindingReferenceVALVE}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkDescriptorSetBindingReferenceVALVE to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkDescriptorSetBindingReferenceVALVE.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkDescriptorSetBindingReferenceVALVE, Iterable<VkDescriptorSetBindingReferenceVALVE> {
        public long size() {
            return segment.byteSize() / VkDescriptorSetBindingReferenceVALVE.BYTES;
        }
        ...
        public @NotNull VkDescriptorSetBindingReferenceVALVE at(long index) {
        ...
        public @NotNull Ptr reinterpret(long index) {
            return new Ptr(segment.asSlice(index * VkDescriptorSetBindingReferenceVALVE.BYTES, VkDescriptorSetBindingReferenceVALVE.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkDescriptorSetBindingReferenceVALVE.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkDescriptorSetBindingReferenceVALVE.BYTES,
                (end - start) * VkDescriptorSetBindingReferenceVALVE.BYTES
            ));
        }
        ...
        public static final class Iter implements Iterator<VkDescriptorSetBindingReferenceVALVE> {
            ...
            public boolean hasNext() {
                return (segment.byteSize() / VkDescriptorSetBindingReferenceVALVE.BYTES) > 0;
            }

            @Override
            public VkDescriptorSetBindingReferenceVALVE next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkDescriptorSetBindingReferenceVALVE ret = new VkDescriptorSetBindingReferenceVALVE(segment.asSlice(0, VkDescriptorSetBindingReferenceVALVE.BYTES));
                segment = segment.asSlice(VkDescriptorSetBindingReferenceVALVE.BYTES);
                return ret;
            }
    ...
    public static VkDescriptorSetBindingReferenceVALVE.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkDescriptorSetBindingReferenceVALVE.Ptr ret = new VkDescriptorSetBindingReferenceVALVE.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.DESCRIPTOR_SET_BINDING_REFERENCE_VALVE);
        }
        return ret;
    }
