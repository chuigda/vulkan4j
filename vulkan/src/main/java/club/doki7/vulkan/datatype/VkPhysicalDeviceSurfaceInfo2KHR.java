...
///
/// The {@code allocate} ({@link VkPhysicalDeviceSurfaceInfo2KHR#allocate(Arena)}, {@link VkPhysicalDeviceSurfaceInfo2KHR#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDeviceSurfaceInfo2KHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceSurfaceInfo2KHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceSurfaceInfo2KHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SURFACE_INFO_2_KHR);
        }
        return ret;
    }
