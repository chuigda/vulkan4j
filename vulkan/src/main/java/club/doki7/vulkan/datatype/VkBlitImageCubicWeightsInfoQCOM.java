...
///
/// The {@link VkBlitImageCubicWeightsInfoQCOM#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkBlitImageCubicWeightsInfoQCOM#autoInit} to initialize these fields manually for
...
public record VkBlitImageCubicWeightsInfoQCOM(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkBlitImageCubicWeightsInfoQCOM[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkBlitImageCubicWeightsInfoQCOM(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.BLIT_IMAGE_CUBIC_WEIGHTS_INFO_QCOM);
        }
        return ret;
    }
