...
///
/// The {@code allocate} ({@link VkSamplerCubicWeightsCreateInfoQCOM#allocate(Arena)}, {@link VkSamplerCubicWeightsCreateInfoQCOM#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkSamplerCubicWeightsCreateInfoQCOM(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkSamplerCubicWeightsCreateInfoQCOM[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkSamplerCubicWeightsCreateInfoQCOM(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.SAMPLER_CUBIC_WEIGHTS_CREATE_INFO_QCOM);
        }
        return ret;
    }
