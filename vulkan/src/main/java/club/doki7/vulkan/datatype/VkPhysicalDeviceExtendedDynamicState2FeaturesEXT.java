...
///
/// The {@link VkPhysicalDeviceExtendedDynamicState2FeaturesEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceExtendedDynamicState2FeaturesEXT#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceExtendedDynamicState2FeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceExtendedDynamicState2FeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceExtendedDynamicState2FeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_EXTENDED_DYNAMIC_STATE_2_FEATURES_EXT);
        }
        return ret;
    }
