...
///
/// The {@code allocate} ({@link VkMultisamplePropertiesEXT#allocate(Arena)}, {@link VkMultisamplePropertiesEXT#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkMultisamplePropertiesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkMultisamplePropertiesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkMultisamplePropertiesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.MULTISAMPLE_PROPERTIES_EXT);
        }
        return ret;
    }
