...
///
/// The {@code allocate} ({@link VkMemoryZirconHandlePropertiesFUCHSIA#allocate(Arena)}, {@link VkMemoryZirconHandlePropertiesFUCHSIA#allocate(Arena, int)})
...
public record VkMemoryZirconHandlePropertiesFUCHSIA(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkMemoryZirconHandlePropertiesFUCHSIA[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkMemoryZirconHandlePropertiesFUCHSIA(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.MEMORY_ZIRCON_HANDLE_PROPERTIES_FUCHSIA);
        }
        return ret;
    }
