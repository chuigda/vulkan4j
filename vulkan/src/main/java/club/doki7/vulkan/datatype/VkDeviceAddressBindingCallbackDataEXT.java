...
///
/// The {@code allocate} ({@link VkDeviceAddressBindingCallbackDataEXT#allocate(Arena)}, {@link VkDeviceAddressBindingCallbackDataEXT#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkDeviceAddressBindingCallbackDataEXT#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkDeviceAddressBindingCallbackDataEXT(@NotNull MemorySegment segment) implements IVkDeviceAddressBindingCallbackDataEXT {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkDeviceAddressBindingCallbackDataEXT.html"><code>VkDeviceAddressBindingCallbackDataEXT</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkDeviceAddressBindingCallbackDataEXT}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkDeviceAddressBindingCallbackDataEXT to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkDeviceAddressBindingCallbackDataEXT.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkDeviceAddressBindingCallbackDataEXT, Iterable<VkDeviceAddressBindingCallbackDataEXT> {
        public long size() {
            return segment.byteSize() / VkDeviceAddressBindingCallbackDataEXT.BYTES;
        }
        ...
        public @NotNull VkDeviceAddressBindingCallbackDataEXT at(long index) {
        ...
        public @NotNull Ptr reinterpret(long index) {
            return new Ptr(segment.asSlice(index * VkDeviceAddressBindingCallbackDataEXT.BYTES, VkDeviceAddressBindingCallbackDataEXT.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkDeviceAddressBindingCallbackDataEXT.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkDeviceAddressBindingCallbackDataEXT.BYTES,
                (end - start) * VkDeviceAddressBindingCallbackDataEXT.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VkDeviceAddressBindingCallbackDataEXT.BYTES));
        }

        public VkDeviceAddressBindingCallbackDataEXT[] toArray() {
            VkDeviceAddressBindingCallbackDataEXT[] ret = new VkDeviceAddressBindingCallbackDataEXT[(int) size()];
        ...
        public static final class Iter implements Iterator<VkDeviceAddressBindingCallbackDataEXT> {
            ...
            public boolean hasNext() {
                return (segment.byteSize() / VkDeviceAddressBindingCallbackDataEXT.BYTES) > 0;
            }

            @Override
            public VkDeviceAddressBindingCallbackDataEXT next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkDeviceAddressBindingCallbackDataEXT ret = new VkDeviceAddressBindingCallbackDataEXT(segment.asSlice(0, VkDeviceAddressBindingCallbackDataEXT.BYTES));
                segment = segment.asSlice(VkDeviceAddressBindingCallbackDataEXT.BYTES);
                return ret;
            }
    ...
    public static VkDeviceAddressBindingCallbackDataEXT.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkDeviceAddressBindingCallbackDataEXT.Ptr ret = new VkDeviceAddressBindingCallbackDataEXT.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.DEVICE_ADDRESS_BINDING_CALLBACK_DATA_EXT);
        }
        return ret;
    }
