...
///
/// The {@code allocate} ({@link VkPhysicalDeviceYcbcr2Plane444FormatsFeaturesEXT#allocate(Arena)}, {@link VkPhysicalDeviceYcbcr2Plane444FormatsFeaturesEXT#allocate(Arena, int)})
...
public record VkPhysicalDeviceYcbcr2Plane444FormatsFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceYcbcr2Plane444FormatsFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceYcbcr2Plane444FormatsFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_YCBCR_2_PLANE_444_FORMATS_FEATURES_EXT);
        }
        return ret;
    }
