...
///
/// The {@code allocate} ({@link VkMemoryGetWin32HandleInfoKHR#allocate(Arena)}, {@link VkMemoryGetWin32HandleInfoKHR#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkMemoryGetWin32HandleInfoKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkMemoryGetWin32HandleInfoKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkMemoryGetWin32HandleInfoKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.MEMORY_GET_WIN32_HANDLE_INFO_KHR);
        }
        return ret;
    }
