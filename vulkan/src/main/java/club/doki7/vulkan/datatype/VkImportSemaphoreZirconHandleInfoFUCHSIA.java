...
///
/// The {@code allocate} ({@link VkImportSemaphoreZirconHandleInfoFUCHSIA#allocate(Arena)}, {@link VkImportSemaphoreZirconHandleInfoFUCHSIA#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkImportSemaphoreZirconHandleInfoFUCHSIA(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkImportSemaphoreZirconHandleInfoFUCHSIA[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkImportSemaphoreZirconHandleInfoFUCHSIA(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.IMPORT_SEMAPHORE_ZIRCON_HANDLE_INFO_FUCHSIA);
        }
        return ret;
    }
    ...
    public static final PathElement PATH$zirconHandle = PathElement.groupElement("PATH$zirconHandle");
