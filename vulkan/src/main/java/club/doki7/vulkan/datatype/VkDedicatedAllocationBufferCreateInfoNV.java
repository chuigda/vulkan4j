...
///
/// The {@code allocate} ({@link VkDedicatedAllocationBufferCreateInfoNV#allocate(Arena)}, {@link VkDedicatedAllocationBufferCreateInfoNV#allocate(Arena, int)})
...
public record VkDedicatedAllocationBufferCreateInfoNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkDedicatedAllocationBufferCreateInfoNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkDedicatedAllocationBufferCreateInfoNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.DEDICATED_ALLOCATION_BUFFER_CREATE_INFO_NV);
        }
        return ret;
    }
