...
///
/// The {@code allocate} ({@link VkProtectedSubmitInfo#allocate(Arena)}, {@link VkProtectedSubmitInfo#allocate(Arena, int)})
...
public record VkProtectedSubmitInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkProtectedSubmitInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkProtectedSubmitInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PROTECTED_SUBMIT_INFO);
        }
        return ret;
    }
