...
///
/// The {@link VkPhysicalDeviceImageAlignmentControlPropertiesMESA#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceImageAlignmentControlPropertiesMESA#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceImageAlignmentControlPropertiesMESA(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceImageAlignmentControlPropertiesMESA[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceImageAlignmentControlPropertiesMESA(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_IMAGE_ALIGNMENT_CONTROL_PROPERTIES_MESA);
        }
        return ret;
    }
