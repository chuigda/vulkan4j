...
///
/// The {@link VkPhysicalDevicePointClippingProperties#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDevicePointClippingProperties#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDevicePointClippingProperties(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDevicePointClippingProperties[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDevicePointClippingProperties(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_POINT_CLIPPING_PROPERTIES);
        }
        return ret;
    }
