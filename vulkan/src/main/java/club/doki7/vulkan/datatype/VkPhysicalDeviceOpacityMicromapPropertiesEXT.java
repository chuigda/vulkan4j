...
///
/// The {@code allocate} ({@link VkPhysicalDeviceOpacityMicromapPropertiesEXT#allocate(Arena)}, {@link VkPhysicalDeviceOpacityMicromapPropertiesEXT#allocate(Arena, int)})
...
public record VkPhysicalDeviceOpacityMicromapPropertiesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceOpacityMicromapPropertiesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceOpacityMicromapPropertiesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_OPACITY_MICROMAP_PROPERTIES_EXT);
        }
        return ret;
    }
