...
///
/// The {@code allocate} ({@link VkViSurfaceCreateInfoNN#allocate(Arena)}, {@link VkViSurfaceCreateInfoNN#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkViSurfaceCreateInfoNN(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkViSurfaceCreateInfoNN[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkViSurfaceCreateInfoNN(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.VI_SURFACE_CREATE_INFO_NN);
        }
        return ret;
    }
    ...
    public static VkViSurfaceCreateInfoNN[] clone(Arena arena, VkViSurfaceCreateInfoNN[] src) {
        VkViSurfaceCreateInfoNN[] ret = allocate(arena, src.length);
