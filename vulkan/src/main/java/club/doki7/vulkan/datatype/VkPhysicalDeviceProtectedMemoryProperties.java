...
///
/// The {@code allocate} ({@link VkPhysicalDeviceProtectedMemoryProperties#allocate(Arena)}, {@link VkPhysicalDeviceProtectedMemoryProperties#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDeviceProtectedMemoryProperties(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceProtectedMemoryProperties[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceProtectedMemoryProperties(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_PROTECTED_MEMORY_PROPERTIES);
        }
        return ret;
    }
