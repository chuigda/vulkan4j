...
///
/// The {@code allocate} ({@link VkPhysicalDevicePipelineCreationCacheControlFeatures#allocate(Arena)}, {@link VkPhysicalDevicePipelineCreationCacheControlFeatures#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDevicePipelineCreationCacheControlFeatures(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDevicePipelineCreationCacheControlFeatures[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDevicePipelineCreationCacheControlFeatures(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_PIPELINE_CREATION_CACHE_CONTROL_FEATURES);
        }
        return ret;
    }
