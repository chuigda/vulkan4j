...
///
/// The {@code allocate} ({@link VkIOSSurfaceCreateInfoMVK#allocate(Arena)}, {@link VkIOSSurfaceCreateInfoMVK#allocate(Arena, int)})
...
public record VkIOSSurfaceCreateInfoMVK(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkIOSSurfaceCreateInfoMVK[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkIOSSurfaceCreateInfoMVK(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.IOS_SURFACE_CREATE_INFO_MVK);
        }
        return ret;
    }
