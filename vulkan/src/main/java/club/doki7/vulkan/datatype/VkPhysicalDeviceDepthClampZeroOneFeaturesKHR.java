...
///
/// The {@code allocate} ({@link VkPhysicalDeviceDepthClampZeroOneFeaturesKHR#allocate(Arena)}, {@link VkPhysicalDeviceDepthClampZeroOneFeaturesKHR#allocate(Arena, int)})
...
public record VkPhysicalDeviceDepthClampZeroOneFeaturesKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceDepthClampZeroOneFeaturesKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceDepthClampZeroOneFeaturesKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_DEPTH_CLAMP_ZERO_ONE_FEATURES_KHR);
        }
        return ret;
    }
