...
///
/// The {@code allocate} ({@link VkPhysicalDeviceImagelessFramebufferFeatures#allocate(Arena)}, {@link VkPhysicalDeviceImagelessFramebufferFeatures#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDeviceImagelessFramebufferFeatures(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceImagelessFramebufferFeatures[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceImagelessFramebufferFeatures(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_IMAGELESS_FRAMEBUFFER_FEATURES);
        }
        return ret;
    }
