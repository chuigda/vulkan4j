...
///
/// The {@link VkAccelerationStructureBuildSizesInfoKHR#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkAccelerationStructureBuildSizesInfoKHR#autoInit} to initialize these fields manually for
...
public record VkAccelerationStructureBuildSizesInfoKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkAccelerationStructureBuildSizesInfoKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkAccelerationStructureBuildSizesInfoKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR);
        }
        return ret;
    }
