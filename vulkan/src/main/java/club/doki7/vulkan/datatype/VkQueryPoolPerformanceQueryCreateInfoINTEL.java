...
///
/// The {@link VkQueryPoolPerformanceQueryCreateInfoINTEL#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkQueryPoolPerformanceQueryCreateInfoINTEL#autoInit} to initialize these fields manually for
...
public record VkQueryPoolPerformanceQueryCreateInfoINTEL(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkQueryPoolPerformanceQueryCreateInfoINTEL[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkQueryPoolPerformanceQueryCreateInfoINTEL(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.QUERY_POOL_PERFORMANCE_QUERY_CREATE_INFO_INTEL);
        }
        return ret;
    }
