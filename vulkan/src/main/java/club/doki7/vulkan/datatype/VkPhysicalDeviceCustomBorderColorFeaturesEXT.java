...
///
/// The {@code allocate} ({@link VkPhysicalDeviceCustomBorderColorFeaturesEXT#allocate(Arena)}, {@link VkPhysicalDeviceCustomBorderColorFeaturesEXT#allocate(Arena, int)})
...
public record VkPhysicalDeviceCustomBorderColorFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceCustomBorderColorFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceCustomBorderColorFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_CUSTOM_BORDER_COLOR_FEATURES_EXT);
        }
        return ret;
    }
