...
///
/// The {@code allocate} ({@link VkBufferDeviceAddressCreateInfoEXT#allocate(Arena)}, {@link VkBufferDeviceAddressCreateInfoEXT#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkBufferDeviceAddressCreateInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkBufferDeviceAddressCreateInfoEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkBufferDeviceAddressCreateInfoEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.BUFFER_DEVICE_ADDRESS_CREATE_INFO_EXT);
        }
        return ret;
    }
