...
///
/// The {@code allocate} ({@link VkImageStencilUsageCreateInfo#allocate(Arena)}, {@link VkImageStencilUsageCreateInfo#allocate(Arena, int)})
...
public record VkImageStencilUsageCreateInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkImageStencilUsageCreateInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkImageStencilUsageCreateInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.IMAGE_STENCIL_USAGE_CREATE_INFO);
        }
        return ret;
    }
