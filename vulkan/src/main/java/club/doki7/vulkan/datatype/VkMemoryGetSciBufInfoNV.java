...
///
/// The {@code allocate} ({@link VkMemoryGetSciBufInfoNV#allocate(Arena)}, {@link VkMemoryGetSciBufInfoNV#allocate(Arena, int)})
...
public record VkMemoryGetSciBufInfoNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkMemoryGetSciBufInfoNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkMemoryGetSciBufInfoNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.MEMORY_GET_SCI_BUF_INFO_NV);
        }
        return ret;
    }
