...
///
/// The {@code allocate} ({@link VkExportMemoryWin32HandleInfoKHR#allocate(Arena)}, {@link VkExportMemoryWin32HandleInfoKHR#allocate(Arena, int)})
...
public record VkExportMemoryWin32HandleInfoKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkExportMemoryWin32HandleInfoKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkExportMemoryWin32HandleInfoKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.EXPORT_MEMORY_WIN32_HANDLE_INFO_KHR);
        }
        return ret;
    }
