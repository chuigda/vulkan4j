...
///
/// The {@code allocate} ({@link VkPhysicalDeviceExternalMemoryScreenBufferFeaturesQNX#allocate(Arena)}, {@link VkPhysicalDeviceExternalMemoryScreenBufferFeaturesQNX#allocate(Arena, int)})
...
public record VkPhysicalDeviceExternalMemoryScreenBufferFeaturesQNX(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceExternalMemoryScreenBufferFeaturesQNX[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceExternalMemoryScreenBufferFeaturesQNX(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_EXTERNAL_MEMORY_SCREEN_BUFFER_FEATURES_QNX);
        }
        return ret;
    }
