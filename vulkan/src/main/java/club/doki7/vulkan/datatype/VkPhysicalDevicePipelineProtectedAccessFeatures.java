...
///
/// The {@code allocate} ({@link VkPhysicalDevicePipelineProtectedAccessFeatures#allocate(Arena)}, {@link VkPhysicalDevicePipelineProtectedAccessFeatures#allocate(Arena, int)})
...
public record VkPhysicalDevicePipelineProtectedAccessFeatures(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDevicePipelineProtectedAccessFeatures[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDevicePipelineProtectedAccessFeatures(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_PIPELINE_PROTECTED_ACCESS_FEATURES);
        }
        return ret;
    }
