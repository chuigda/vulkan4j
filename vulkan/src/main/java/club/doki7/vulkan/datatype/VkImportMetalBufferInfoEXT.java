...
///
/// The {@code allocate} ({@link VkImportMetalBufferInfoEXT#allocate(Arena)}, {@link VkImportMetalBufferInfoEXT#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkImportMetalBufferInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkImportMetalBufferInfoEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkImportMetalBufferInfoEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.IMPORT_METAL_BUFFER_INFO_EXT);
        }
        return ret;
    }
