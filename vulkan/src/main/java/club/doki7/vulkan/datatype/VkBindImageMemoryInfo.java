...
///
/// The {@link VkBindImageMemoryInfo#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkBindImageMemoryInfo#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkBindImageMemoryInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkBindImageMemoryInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkBindImageMemoryInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.BIND_IMAGE_MEMORY_INFO);
        }
        return ret;
    }
