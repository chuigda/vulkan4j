...
///
/// The {@code allocate} ({@link VkExportSemaphoreCreateInfo#allocate(Arena)}, {@link VkExportSemaphoreCreateInfo#allocate(Arena, long)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkExportSemaphoreCreateInfo(@NotNull MemorySegment segment) implements IVkExportSemaphoreCreateInfo {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkExportSemaphoreCreateInfo.html"><code>VkExportSemaphoreCreateInfo</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkExportSemaphoreCreateInfo}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkExportSemaphoreCreateInfo to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkExportSemaphoreCreateInfo.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkExportSemaphoreCreateInfo, Iterable<VkExportSemaphoreCreateInfo> {
        public long size() {
            return segment.byteSize() / VkExportSemaphoreCreateInfo.BYTES;
        }
        ...
        public @NotNull VkExportSemaphoreCreateInfo at(long index) {
        ...
        public @NotNull Ptr reinterpret(long index) {
            return new Ptr(segment.asSlice(index * VkExportSemaphoreCreateInfo.BYTES, VkExportSemaphoreCreateInfo.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkExportSemaphoreCreateInfo.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkExportSemaphoreCreateInfo.BYTES,
                (end - start) * VkExportSemaphoreCreateInfo.BYTES
            ));
        }
        ...
        public static final class Iter implements Iterator<VkExportSemaphoreCreateInfo> {
            ...
            public boolean hasNext() {
                return (segment.byteSize() / VkExportSemaphoreCreateInfo.BYTES) > 0;
            }

            @Override
            public VkExportSemaphoreCreateInfo next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkExportSemaphoreCreateInfo ret = new VkExportSemaphoreCreateInfo(segment.asSlice(0, VkExportSemaphoreCreateInfo.BYTES));
                segment = segment.asSlice(VkExportSemaphoreCreateInfo.BYTES);
                return ret;
            }
    ...
    public static VkExportSemaphoreCreateInfo.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkExportSemaphoreCreateInfo.Ptr ret = new VkExportSemaphoreCreateInfo.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.EXPORT_SEMAPHORE_CREATE_INFO);
        }
        return ret;
    }
