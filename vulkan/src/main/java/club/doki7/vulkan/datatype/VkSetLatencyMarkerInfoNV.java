...
///
/// The {@code allocate} ({@link VkSetLatencyMarkerInfoNV#allocate(Arena)}, {@link VkSetLatencyMarkerInfoNV#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkSetLatencyMarkerInfoNV#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkSetLatencyMarkerInfoNV(@NotNull MemorySegment segment) implements IVkSetLatencyMarkerInfoNV {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkSetLatencyMarkerInfoNV.html"><code>VkSetLatencyMarkerInfoNV</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkSetLatencyMarkerInfoNV}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkSetLatencyMarkerInfoNV to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkSetLatencyMarkerInfoNV.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkSetLatencyMarkerInfoNV, Iterable<VkSetLatencyMarkerInfoNV> {
        public long size() {
            return segment.byteSize() / VkSetLatencyMarkerInfoNV.BYTES;
        }
        ...
        public @NotNull VkSetLatencyMarkerInfoNV at(long index) {
        ...
        public @NotNull Ptr reinterpret(long index) {
            return new Ptr(segment.asSlice(index * VkSetLatencyMarkerInfoNV.BYTES, VkSetLatencyMarkerInfoNV.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkSetLatencyMarkerInfoNV.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkSetLatencyMarkerInfoNV.BYTES,
                (end - start) * VkSetLatencyMarkerInfoNV.BYTES
            ));
        }
        ...
        public static final class Iter implements Iterator<VkSetLatencyMarkerInfoNV> {
            ...
            public boolean hasNext() {
                return (segment.byteSize() / VkSetLatencyMarkerInfoNV.BYTES) > 0;
            }

            @Override
            public VkSetLatencyMarkerInfoNV next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkSetLatencyMarkerInfoNV ret = new VkSetLatencyMarkerInfoNV(segment.asSlice(0, VkSetLatencyMarkerInfoNV.BYTES));
                segment = segment.asSlice(VkSetLatencyMarkerInfoNV.BYTES);
                return ret;
            }
    ...
    public static VkSetLatencyMarkerInfoNV.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkSetLatencyMarkerInfoNV.Ptr ret = new VkSetLatencyMarkerInfoNV.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.SET_LATENCY_MARKER_INFO_NV);
        }
        return ret;
    }
