...
///
/// The {@code allocate} ({@link VkExportMemoryAllocateInfoNV#allocate(Arena)}, {@link VkExportMemoryAllocateInfoNV#allocate(Arena, int)})
...
public record VkExportMemoryAllocateInfoNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkExportMemoryAllocateInfoNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkExportMemoryAllocateInfoNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.EXPORT_MEMORY_ALLOCATE_INFO_NV);
        }
        return ret;
    }
