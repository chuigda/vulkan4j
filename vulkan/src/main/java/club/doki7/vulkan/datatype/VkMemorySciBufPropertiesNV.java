...
///
/// The {@link VkMemorySciBufPropertiesNV#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkMemorySciBufPropertiesNV#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkMemorySciBufPropertiesNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkMemorySciBufPropertiesNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkMemorySciBufPropertiesNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.MEMORY_SCI_BUF_PROPERTIES_NV);
        }
        return ret;
    }
    ...
    public static VkMemorySciBufPropertiesNV[] clone(Arena arena, VkMemorySciBufPropertiesNV[] src) {
        VkMemorySciBufPropertiesNV[] ret = allocate(arena, src.length);
