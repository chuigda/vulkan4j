...
///
/// The {@code allocate} ({@link VkPhysicalDeviceMutableDescriptorTypeFeaturesEXT#allocate(Arena)}, {@link VkPhysicalDeviceMutableDescriptorTypeFeaturesEXT#allocate(Arena, int)})
...
public record VkPhysicalDeviceMutableDescriptorTypeFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceMutableDescriptorTypeFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceMutableDescriptorTypeFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_MUTABLE_DESCRIPTOR_TYPE_FEATURES_EXT);
        }
        return ret;
    }
