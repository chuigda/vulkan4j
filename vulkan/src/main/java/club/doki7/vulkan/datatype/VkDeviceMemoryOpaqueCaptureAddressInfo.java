...
///
/// The {@code allocate} ({@link VkDeviceMemoryOpaqueCaptureAddressInfo#allocate(Arena)}, {@link VkDeviceMemoryOpaqueCaptureAddressInfo#allocate(Arena, int)})
...
public record VkDeviceMemoryOpaqueCaptureAddressInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkDeviceMemoryOpaqueCaptureAddressInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkDeviceMemoryOpaqueCaptureAddressInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.DEVICE_MEMORY_OPAQUE_CAPTURE_ADDRESS_INFO);
        }
        return ret;
    }
