...
///
/// The {@code allocate} ({@link VkDeviceImageMemoryRequirements#allocate(Arena)}, {@link VkDeviceImageMemoryRequirements#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkDeviceImageMemoryRequirements#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkDeviceImageMemoryRequirements(@NotNull MemorySegment segment) implements IVkDeviceImageMemoryRequirements {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkDeviceImageMemoryRequirements.html"><code>VkDeviceImageMemoryRequirements</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkDeviceImageMemoryRequirements}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkDeviceImageMemoryRequirements to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkDeviceImageMemoryRequirements.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkDeviceImageMemoryRequirements, Iterable<VkDeviceImageMemoryRequirements> {
        public long size() {
            return segment.byteSize() / VkDeviceImageMemoryRequirements.BYTES;
        }
        ...
        public @NotNull VkDeviceImageMemoryRequirements at(long index) {
        ...
        public void write(long index, @NotNull VkDeviceImageMemoryRequirements value) {
            MemorySegment s = segment.asSlice(index * VkDeviceImageMemoryRequirements.BYTES, VkDeviceImageMemoryRequirements.BYTES);
        ...
        public @NotNull Ptr reinterpret(long index) {
            return new Ptr(segment.asSlice(index * VkDeviceImageMemoryRequirements.BYTES, VkDeviceImageMemoryRequirements.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkDeviceImageMemoryRequirements.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkDeviceImageMemoryRequirements.BYTES,
                (end - start) * VkDeviceImageMemoryRequirements.BYTES
            ));
        }
        ...
        public static final class Iter implements Iterator<VkDeviceImageMemoryRequirements> {
            ...
            public boolean hasNext() {
                return (segment.byteSize() / VkDeviceImageMemoryRequirements.BYTES) > 0;
            }

            @Override
            public VkDeviceImageMemoryRequirements next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkDeviceImageMemoryRequirements ret = new VkDeviceImageMemoryRequirements(segment.asSlice(0, VkDeviceImageMemoryRequirements.BYTES));
                segment = segment.asSlice(VkDeviceImageMemoryRequirements.BYTES);
                return ret;
            }
    ...
    public static VkDeviceImageMemoryRequirements.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkDeviceImageMemoryRequirements.Ptr ret = new VkDeviceImageMemoryRequirements.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.DEVICE_IMAGE_MEMORY_REQUIREMENTS);
        }
        return ret;
    }
