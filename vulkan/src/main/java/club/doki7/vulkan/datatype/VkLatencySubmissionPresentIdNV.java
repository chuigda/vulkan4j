...
///
/// The {@code allocate} ({@link VkLatencySubmissionPresentIdNV#allocate(Arena)}, {@link VkLatencySubmissionPresentIdNV#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkLatencySubmissionPresentIdNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkLatencySubmissionPresentIdNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkLatencySubmissionPresentIdNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.LATENCY_SUBMISSION_PRESENT_ID_NV);
        }
        return ret;
    }
