...
///
/// The {@link VkPhysicalDeviceSamplerYcbcrConversionFeatures#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceSamplerYcbcrConversionFeatures#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceSamplerYcbcrConversionFeatures(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceSamplerYcbcrConversionFeatures[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceSamplerYcbcrConversionFeatures(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SAMPLER_YCBCR_CONVERSION_FEATURES);
        }
        return ret;
    }
