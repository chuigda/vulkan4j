...
///
/// The {@code allocate} ({@link VkPhysicalDevicePageableDeviceLocalMemoryFeaturesEXT#allocate(Arena)}, {@link VkPhysicalDevicePageableDeviceLocalMemoryFeaturesEXT#allocate(Arena, int)})
...
public record VkPhysicalDevicePageableDeviceLocalMemoryFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDevicePageableDeviceLocalMemoryFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDevicePageableDeviceLocalMemoryFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_PAGEABLE_DEVICE_LOCAL_MEMORY_FEATURES_EXT);
        }
        return ret;
    }
