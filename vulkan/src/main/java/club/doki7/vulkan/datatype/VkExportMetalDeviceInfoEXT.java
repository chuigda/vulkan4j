...
///
/// The {@link VkExportMetalDeviceInfoEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkExportMetalDeviceInfoEXT#autoInit} to initialize these fields manually for
...
public record VkExportMetalDeviceInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkExportMetalDeviceInfoEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkExportMetalDeviceInfoEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.EXPORT_METAL_DEVICE_INFO_EXT);
        }
        return ret;
    }
