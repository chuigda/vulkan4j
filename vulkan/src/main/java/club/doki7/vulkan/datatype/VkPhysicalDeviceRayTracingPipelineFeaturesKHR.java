...
///
/// The {@link VkPhysicalDeviceRayTracingPipelineFeaturesKHR#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceRayTracingPipelineFeaturesKHR#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceRayTracingPipelineFeaturesKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceRayTracingPipelineFeaturesKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceRayTracingPipelineFeaturesKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_FEATURES_KHR);
        }
        return ret;
    }
