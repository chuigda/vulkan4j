...
///
/// The {@code allocate} ({@link VkPhysicalDeviceVertexInputDynamicStateFeaturesEXT#allocate(Arena)}, {@link VkPhysicalDeviceVertexInputDynamicStateFeaturesEXT#allocate(Arena, int)})
...
public record VkPhysicalDeviceVertexInputDynamicStateFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceVertexInputDynamicStateFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceVertexInputDynamicStateFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_VERTEX_INPUT_DYNAMIC_STATE_FEATURES_EXT);
        }
        return ret;
    }
