...
///
/// The {@code allocate} ({@link VkPipelineCacheCreateInfo#allocate(Arena)}, {@link VkPipelineCacheCreateInfo#allocate(Arena, int)})
...
public record VkPipelineCacheCreateInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPipelineCacheCreateInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPipelineCacheCreateInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PIPELINE_CACHE_CREATE_INFO);
        }
        return ret;
    }
