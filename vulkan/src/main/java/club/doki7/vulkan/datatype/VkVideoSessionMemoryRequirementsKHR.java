...
///
/// The {@code allocate} ({@link VkVideoSessionMemoryRequirementsKHR#allocate(Arena)}, {@link VkVideoSessionMemoryRequirementsKHR#allocate(Arena, int)})
...
public record VkVideoSessionMemoryRequirementsKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkVideoSessionMemoryRequirementsKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkVideoSessionMemoryRequirementsKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.VIDEO_SESSION_MEMORY_REQUIREMENTS_KHR);
        }
        return ret;
    }
