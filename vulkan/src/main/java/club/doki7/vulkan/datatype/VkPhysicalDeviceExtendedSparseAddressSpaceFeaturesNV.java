...
///
/// The {@code allocate} ({@link VkPhysicalDeviceExtendedSparseAddressSpaceFeaturesNV#allocate(Arena)}, {@link VkPhysicalDeviceExtendedSparseAddressSpaceFeaturesNV#allocate(Arena, int)})
...
public record VkPhysicalDeviceExtendedSparseAddressSpaceFeaturesNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceExtendedSparseAddressSpaceFeaturesNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceExtendedSparseAddressSpaceFeaturesNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_EXTENDED_SPARSE_ADDRESS_SPACE_FEATURES_NV);
        }
        return ret;
    }
