...
///
/// The {@code allocate} ({@link VkPhysicalDeviceShaderReplicatedCompositesFeaturesEXT#allocate(Arena)}, {@link VkPhysicalDeviceShaderReplicatedCompositesFeaturesEXT#allocate(Arena, int)})
...
public record VkPhysicalDeviceShaderReplicatedCompositesFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceShaderReplicatedCompositesFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceShaderReplicatedCompositesFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SHADER_REPLICATED_COMPOSITES_FEATURES_EXT);
        }
        return ret;
    }
