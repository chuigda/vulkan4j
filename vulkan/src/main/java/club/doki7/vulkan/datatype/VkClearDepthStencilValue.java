...
public record VkClearDepthStencilValue(@NotNull MemorySegment segment) implements IVkClearDepthStencilValue {
    ...
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkClearDepthStencilValue, Iterable<VkClearDepthStencilValue> {
        public long size() {
        ...
        public @NotNull Ptr reinterpret(long index) {
            return new Ptr(segment.asSlice(index * VkClearDepthStencilValue.BYTES, VkClearDepthStencilValue.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkClearDepthStencilValue.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkClearDepthStencilValue.BYTES,
                (end - start) * VkClearDepthStencilValue.BYTES
            ));
        }
        ...
        public static final class Iter implements Iterator<VkClearDepthStencilValue> {
            ...
            public boolean hasNext() {
                return (segment.byteSize() / VkClearDepthStencilValue.BYTES) > 0;
            }

            @Override
            public VkClearDepthStencilValue next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkClearDepthStencilValue ret = new VkClearDepthStencilValue(segment.asSlice(0, VkClearDepthStencilValue.BYTES));
                segment = segment.asSlice(VkClearDepthStencilValue.BYTES);
                return ret;
            }
