...
///
/// The {@code allocate} ({@link VkPhysicalDeviceShaderModuleIdentifierPropertiesEXT#allocate(Arena)}, {@link VkPhysicalDeviceShaderModuleIdentifierPropertiesEXT#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDeviceShaderModuleIdentifierPropertiesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceShaderModuleIdentifierPropertiesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceShaderModuleIdentifierPropertiesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SHADER_MODULE_IDENTIFIER_PROPERTIES_EXT);
        }
        return ret;
    }
    ...
    public static final PathElement PATH$shaderModuleIdentifierAlgorithmUUID = PathElement.groupElement("PATH$shaderModuleIdentifierAlgorithmUUID");
