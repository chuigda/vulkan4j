...
///
/// The {@link VkIndirectExecutionSetShaderLayoutInfoEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkIndirectExecutionSetShaderLayoutInfoEXT#autoInit} to initialize these fields manually for
...
public record VkIndirectExecutionSetShaderLayoutInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkIndirectExecutionSetShaderLayoutInfoEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkIndirectExecutionSetShaderLayoutInfoEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.INDIRECT_EXECUTION_SET_SHADER_LAYOUT_INFO_EXT);
        }
        return ret;
    }
