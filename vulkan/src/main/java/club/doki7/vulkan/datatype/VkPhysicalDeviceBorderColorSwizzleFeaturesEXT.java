...
///
/// The {@code allocate} ({@link VkPhysicalDeviceBorderColorSwizzleFeaturesEXT#allocate(Arena)}, {@link VkPhysicalDeviceBorderColorSwizzleFeaturesEXT#allocate(Arena, int)})
...
public record VkPhysicalDeviceBorderColorSwizzleFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceBorderColorSwizzleFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceBorderColorSwizzleFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_BORDER_COLOR_SWIZZLE_FEATURES_EXT);
        }
        return ret;
    }
