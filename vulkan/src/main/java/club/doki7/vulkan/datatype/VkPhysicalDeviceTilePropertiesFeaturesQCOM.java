...
///
/// The {@code allocate} ({@link VkPhysicalDeviceTilePropertiesFeaturesQCOM#allocate(Arena)}, {@link VkPhysicalDeviceTilePropertiesFeaturesQCOM#allocate(Arena, int)})
...
public record VkPhysicalDeviceTilePropertiesFeaturesQCOM(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceTilePropertiesFeaturesQCOM[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceTilePropertiesFeaturesQCOM(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_TILE_PROPERTIES_FEATURES_QCOM);
        }
        return ret;
    }
