...
///
/// The {@code allocate} ({@link VkSwapchainLatencyCreateInfoNV#allocate(Arena)}, {@link VkSwapchainLatencyCreateInfoNV#allocate(Arena, int)})
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkSwapchainLatencyCreateInfoNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkSwapchainLatencyCreateInfoNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkSwapchainLatencyCreateInfoNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.SWAPCHAIN_LATENCY_CREATE_INFO_NV);
        }
        return ret;
    }
    ...
    public static final PathElement PATH$latencyModeEnable = PathElement.groupElement("PATH$latencyModeEnable");
