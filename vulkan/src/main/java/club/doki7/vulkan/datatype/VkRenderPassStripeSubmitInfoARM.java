...
///
/// The {@code allocate} ({@link VkRenderPassStripeSubmitInfoARM#allocate(Arena)}, {@link VkRenderPassStripeSubmitInfoARM#allocate(Arena, long)})
/// functions will automatically initialize these fields. Also, you may call {@link VkRenderPassStripeSubmitInfoARM#autoInit}
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkRenderPassStripeSubmitInfoARM(@NotNull MemorySegment segment) implements IVkRenderPassStripeSubmitInfoARM {
    /// Represents a pointer to / an array of <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkRenderPassStripeSubmitInfoARM.html"><code>VkRenderPassStripeSubmitInfoARM</code></a> structure(s) in native memory.
    ///
    /// Technically speaking, this type has no difference with {@link VkRenderPassStripeSubmitInfoARM}. This type
    /// is introduced mainly for user to distinguish between a pointer to a single structure
    /// and a pointer to (potentially) an array of structure(s). APIs should use interface
    /// IVkRenderPassStripeSubmitInfoARM to handle both types uniformly. See package level documentation for more
    /// details.
    ///
    /// ## Contracts
    ///
    /// The property {@link #segment()} should always be not-null
    /// ({@code segment != NULL && !segment.equals(MemorySegment.NULL)}), and properly aligned to
    /// {@code VkRenderPassStripeSubmitInfoARM.LAYOUT.byteAlignment()} bytes. To represent null pointer, you may use a Java
    /// {@code null} instead. See the documentation of {@link IPointer#segment()} for more details.
    ///
    /// The constructor of this class is marked as {@link UnsafeConstructor}, because it does not
    /// perform any runtime check. The constructor can be useful for automatic code generators.
    @ValueBasedCandidate
    @UnsafeConstructor
    public record Ptr(@NotNull MemorySegment segment) implements IVkRenderPassStripeSubmitInfoARM, Iterable<VkRenderPassStripeSubmitInfoARM> {
        public long size() {
            return segment.byteSize() / VkRenderPassStripeSubmitInfoARM.BYTES;
        }
        ...
        public @NotNull VkRenderPassStripeSubmitInfoARM at(long index) {
        ...
        public @NotNull Ptr reinterpret(long index) {
            return new Ptr(segment.asSlice(index * VkRenderPassStripeSubmitInfoARM.BYTES, VkRenderPassStripeSubmitInfoARM.BYTES));
        }

        public @NotNull Ptr offset(long offset) {
            return new Ptr(segment.asSlice(offset * VkRenderPassStripeSubmitInfoARM.BYTES));
        }
        ...
        public @NotNull Ptr slice(long start, long end) {
            return new Ptr(segment.asSlice(
                start * VkRenderPassStripeSubmitInfoARM.BYTES,
                (end - start) * VkRenderPassStripeSubmitInfoARM.BYTES
            ));
        }

        public Ptr slice(long end) {
            return new Ptr(segment.asSlice(0, end * VkRenderPassStripeSubmitInfoARM.BYTES));
        }

        public VkRenderPassStripeSubmitInfoARM[] toArray() {
            VkRenderPassStripeSubmitInfoARM[] ret = new VkRenderPassStripeSubmitInfoARM[(int) size()];
        ...
        public static final class Iter implements Iterator<VkRenderPassStripeSubmitInfoARM> {
            ...
            public boolean hasNext() {
                return (segment.byteSize() / VkRenderPassStripeSubmitInfoARM.BYTES) > 0;
            }

            @Override
            public VkRenderPassStripeSubmitInfoARM next() {
                if (!hasNext()) {
                    throw new NoSuchElementException();
                }
                VkRenderPassStripeSubmitInfoARM ret = new VkRenderPassStripeSubmitInfoARM(segment.asSlice(0, VkRenderPassStripeSubmitInfoARM.BYTES));
                segment = segment.asSlice(VkRenderPassStripeSubmitInfoARM.BYTES);
                return ret;
            }
    ...
    public static VkRenderPassStripeSubmitInfoARM.Ptr allocate(Arena arena, long count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkRenderPassStripeSubmitInfoARM.Ptr ret = new VkRenderPassStripeSubmitInfoARM.Ptr(segment);
        for (long i = 0; i < count; i++) {
            ret.at(i).sType(VkStructureType.RENDER_PASS_STRIPE_SUBMIT_INFO_ARM);
        }
        return ret;
    }
