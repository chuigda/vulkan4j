...
///
/// The {@code allocate} ({@link VkRenderPassTransformBeginInfoQCOM#allocate(Arena)}, {@link VkRenderPassTransformBeginInfoQCOM#allocate(Arena, int)})
...
public record VkRenderPassTransformBeginInfoQCOM(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkRenderPassTransformBeginInfoQCOM[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkRenderPassTransformBeginInfoQCOM(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.RENDER_PASS_TRANSFORM_BEGIN_INFO_QCOM);
        }
        return ret;
    }
