...
///
/// The {@link VkMemoryFdPropertiesKHR#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkMemoryFdPropertiesKHR#autoInit} to initialize these fields manually for
...
public record VkMemoryFdPropertiesKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkMemoryFdPropertiesKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkMemoryFdPropertiesKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.MEMORY_FD_PROPERTIES_KHR);
        }
        return ret;
    }
