...
///
/// The {@link VkAccelerationStructureGeometryInstancesDataKHR#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkAccelerationStructureGeometryInstancesDataKHR#autoInit} to initialize these fields manually for
...
public record VkAccelerationStructureGeometryInstancesDataKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkAccelerationStructureGeometryInstancesDataKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkAccelerationStructureGeometryInstancesDataKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.ACCELERATION_STRUCTURE_GEOMETRY_INSTANCES_DATA_KHR);
        }
        return ret;
    }
