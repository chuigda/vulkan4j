...
public final class VkImageCompressionFlagsEXT {
    public static final int IMAGE_COMPRESSION_DEFAULT = 0x0;
    public static final int IMAGE_COMPRESSION_DISABLED = 0x4;
    public static final int IMAGE_COMPRESSION_FIXED_RATE_DEFAULT = 0x1;
    public static final int IMAGE_COMPRESSION_FIXED_RATE_EXPLICIT = 0x2;

    public static String explain(@enumtype(VkImageCompressionFlagsEXT.class) int flags) {
        List<String> detectedFlagBits = new ArrayList<>();
        if ((flags & IMAGE_COMPRESSION_DEFAULT) != 0) {
            detectedFlagBits.add("VK_IMAGE_COMPRESSION_DEFAULT_EXT");
        }
        if ((flags & IMAGE_COMPRESSION_DISABLED) != 0) {
            detectedFlagBits.add("VK_IMAGE_COMPRESSION_DISABLED_EXT");
        }
        if ((flags & IMAGE_COMPRESSION_FIXED_RATE_DEFAULT) != 0) {
            detectedFlagBits.add("VK_IMAGE_COMPRESSION_FIXED_RATE_DEFAULT_EXT");
        }
        if ((flags & IMAGE_COMPRESSION_FIXED_RATE_EXPLICIT) != 0) {
