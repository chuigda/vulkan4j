...
public final class VkPresentScalingFlagsEXT {
    public static final int PRESENT_SCALING_ASPECT_RATIO_STRETCH = 0x2;
    public static final int PRESENT_SCALING_ONE_TO_ONE = 0x1;
    public static final int PRESENT_SCALING_STRETCH = 0x4;

    public static String explain(@enumtype(VkPresentScalingFlagsEXT.class) int flags) {
        List<String> detectedFlagBits = new ArrayList<>();
        if ((flags & PRESENT_SCALING_ASPECT_RATIO_STRETCH) != 0) {
            detectedFlagBits.add("VK_PRESENT_SCALING_ASPECT_RATIO_STRETCH_BIT_EXT");
        }
        if ((flags & PRESENT_SCALING_ONE_TO_ONE) != 0) {
            detectedFlagBits.add("VK_PRESENT_SCALING_ONE_TO_ONE_BIT_EXT");
        }
        if ((flags & PRESENT_SCALING_STRETCH) != 0) {
