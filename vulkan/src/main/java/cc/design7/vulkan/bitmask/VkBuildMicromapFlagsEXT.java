...
public final class VkBuildMicromapFlagsEXT {
    public static final int BUILD_MICROMAP_ALLOW_COMPACTION = 0x4;
    public static final int BUILD_MICROMAP_PREFER_FAST_BUILD = 0x2;
    public static final int BUILD_MICROMAP_PREFER_FAST_TRACE = 0x1;

    public static String explain(@enumtype(VkBuildMicromapFlagsEXT.class) int flags) {
        List<String> detectedFlagBits = new ArrayList<>();
        if ((flags & BUILD_MICROMAP_ALLOW_COMPACTION) != 0) {
            detectedFlagBits.add("VK_BUILD_MICROMAP_ALLOW_COMPACTION_BIT_EXT");
        }
        if ((flags & BUILD_MICROMAP_PREFER_FAST_BUILD) != 0) {
            detectedFlagBits.add("VK_BUILD_MICROMAP_PREFER_FAST_BUILD_BIT_EXT");
        }
        if ((flags & BUILD_MICROMAP_PREFER_FAST_TRACE) != 0) {
