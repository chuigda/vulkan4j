...
public final class VkGeometryFlagsKHR {
    public static final int GEOMETRY_NO_DUPLICATE_ANY_HIT_INVOCATION = 0x2;
    public static final int GEOMETRY_OPAQUE = 0x1;

    public static String explain(@enumtype(VkGeometryFlagsKHR.class) int flags) {
        List<String> detectedFlagBits = new ArrayList<>();
        if ((flags & GEOMETRY_NO_DUPLICATE_ANY_HIT_INVOCATION) != 0) {
            detectedFlagBits.add("VK_GEOMETRY_NO_DUPLICATE_ANY_HIT_INVOCATION_BIT_KHR");
        }
        if ((flags & GEOMETRY_OPAQUE) != 0) {
