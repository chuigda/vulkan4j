...
public final class VkDescriptorBindingFlags {
    public static final int DESCRIPTOR_BINDING_PARTIALLY_BOUND = 0x4;
    public static final int DESCRIPTOR_BINDING_UPDATE_AFTER_BIND = 0x1;
    public static final int DESCRIPTOR_BINDING_UPDATE_UNUSED_WHILE_PENDING = 0x2;
    public static final int DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT = 0x8;

    public static String explain(@enumtype(VkDescriptorBindingFlags.class) int flags) {
        List<String> detectedFlagBits = new ArrayList<>();
        if ((flags & DESCRIPTOR_BINDING_PARTIALLY_BOUND) != 0) {
            detectedFlagBits.add("VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT");
        }
        if ((flags & DESCRIPTOR_BINDING_UPDATE_AFTER_BIND) != 0) {
            detectedFlagBits.add("VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT");
        }
        if ((flags & DESCRIPTOR_BINDING_UPDATE_UNUSED_WHILE_PENDING) != 0) {
            detectedFlagBits.add("VK_DESCRIPTOR_BINDING_UPDATE_UNUSED_WHILE_PENDING_BIT");
        }
        if ((flags & DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT) != 0) {
