...
public final class VkVideoDecodeUsageFlagsKHR {
    public static final int VIDEO_DECODE_USAGE_DEFAULT = 0x0;
    public static final int VIDEO_DECODE_USAGE_OFFLINE = 0x2;
    public static final int VIDEO_DECODE_USAGE_STREAMING = 0x4;
    public static final int VIDEO_DECODE_USAGE_TRANSCODING = 0x1;

    public static String explain(@enumtype(VkVideoDecodeUsageFlagsKHR.class) int flags) {
        List<String> detectedFlagBits = new ArrayList<>();
        if ((flags & VIDEO_DECODE_USAGE_DEFAULT) != 0) {
            detectedFlagBits.add("VK_VIDEO_DECODE_USAGE_DEFAULT_KHR");
        }
        if ((flags & VIDEO_DECODE_USAGE_OFFLINE) != 0) {
            detectedFlagBits.add("VK_VIDEO_DECODE_USAGE_OFFLINE_BIT_KHR");
        }
        if ((flags & VIDEO_DECODE_USAGE_STREAMING) != 0) {
            detectedFlagBits.add("VK_VIDEO_DECODE_USAGE_STREAMING_BIT_KHR");
        }
        if ((flags & VIDEO_DECODE_USAGE_TRANSCODING) != 0) {
