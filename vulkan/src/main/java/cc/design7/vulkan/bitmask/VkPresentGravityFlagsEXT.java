...
public final class VkPresentGravityFlagsEXT {
    public static final int PRESENT_GRAVITY_CENTERED = 0x4;
    public static final int PRESENT_GRAVITY_MAX = 0x2;
    public static final int PRESENT_GRAVITY_MIN = 0x1;

    public static String explain(@enumtype(VkPresentGravityFlagsEXT.class) int flags) {
        List<String> detectedFlagBits = new ArrayList<>();
        if ((flags & PRESENT_GRAVITY_CENTERED) != 0) {
            detectedFlagBits.add("VK_PRESENT_GRAVITY_CENTERED_BIT_EXT");
        }
        if ((flags & PRESENT_GRAVITY_MAX) != 0) {
            detectedFlagBits.add("VK_PRESENT_GRAVITY_MAX_BIT_EXT");
        }
        if ((flags & PRESENT_GRAVITY_MIN) != 0) {
