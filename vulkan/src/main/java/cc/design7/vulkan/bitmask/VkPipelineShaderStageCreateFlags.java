...
public final class VkPipelineShaderStageCreateFlags {
    public static final int PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE = 0x1;
    public static final int PIPELINE_SHADER_STAGE_CREATE_REQUIRE_FULL_SUBGROUPS = 0x2;

    public static String explain(@enumtype(VkPipelineShaderStageCreateFlags.class) int flags) {
        List<String> detectedFlagBits = new ArrayList<>();
        if ((flags & PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE) != 0) {
            detectedFlagBits.add("VK_PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT");
        }
        if ((flags & PIPELINE_SHADER_STAGE_CREATE_REQUIRE_FULL_SUBGROUPS) != 0) {
