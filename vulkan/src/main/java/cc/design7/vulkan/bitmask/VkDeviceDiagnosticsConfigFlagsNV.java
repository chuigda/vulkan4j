...
public final class VkDeviceDiagnosticsConfigFlagsNV {
    public static final int DEVICE_DIAGNOSTICS_CONFIG_ENABLE_AUTOMATIC_CHECKPOINTS = 0x4;
    public static final int DEVICE_DIAGNOSTICS_CONFIG_ENABLE_RESOURCE_TRACKING = 0x2;
    public static final int DEVICE_DIAGNOSTICS_CONFIG_ENABLE_SHADER_DEBUG_INFO = 0x1;
    public static final int DEVICE_DIAGNOSTICS_CONFIG_ENABLE_SHADER_ERROR_REPORTING = 0x8;

    public static String explain(@enumtype(VkDeviceDiagnosticsConfigFlagsNV.class) int flags) {
        List<String> detectedFlagBits = new ArrayList<>();
        if ((flags & DEVICE_DIAGNOSTICS_CONFIG_ENABLE_AUTOMATIC_CHECKPOINTS) != 0) {
            detectedFlagBits.add("VK_DEVICE_DIAGNOSTICS_CONFIG_ENABLE_AUTOMATIC_CHECKPOINTS_BIT_NV");
        }
        if ((flags & DEVICE_DIAGNOSTICS_CONFIG_ENABLE_RESOURCE_TRACKING) != 0) {
            detectedFlagBits.add("VK_DEVICE_DIAGNOSTICS_CONFIG_ENABLE_RESOURCE_TRACKING_BIT_NV");
        }
        if ((flags & DEVICE_DIAGNOSTICS_CONFIG_ENABLE_SHADER_DEBUG_INFO) != 0) {
            detectedFlagBits.add("VK_DEVICE_DIAGNOSTICS_CONFIG_ENABLE_SHADER_DEBUG_INFO_BIT_NV");
        }
        if ((flags & DEVICE_DIAGNOSTICS_CONFIG_ENABLE_SHADER_ERROR_REPORTING) != 0) {
