...
/// Represents a pointer to a {@code VkPhysicalDeviceCornerSampledImageFeaturesNV} structure in native memory.
///
...
public record VkPhysicalDeviceCornerSampledImageFeaturesNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceCornerSampledImageFeaturesNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceCornerSampledImageFeaturesNV(segment.asSlice(i * BYTES, BYTES));
        }
        return ret;
    }
