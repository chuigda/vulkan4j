...
///
/// The {@link VkBindImagePlaneMemoryInfo#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkBindImagePlaneMemoryInfo#autoInit} to initialize these fields manually for
...
public record VkBindImagePlaneMemoryInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkBindImagePlaneMemoryInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkBindImagePlaneMemoryInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.BIND_IMAGE_PLANE_MEMORY_INFO);
        }
        return ret;
    }
