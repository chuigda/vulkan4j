...
///
/// The {@link VkSamplerReductionModeCreateInfo#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkSamplerReductionModeCreateInfo#autoInit} to initialize these fields manually for
...
public record VkSamplerReductionModeCreateInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkSamplerReductionModeCreateInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkSamplerReductionModeCreateInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.SAMPLER_REDUCTION_MODE_CREATE_INFO);
        }
        return ret;
    }
