...
///
/// The {@link VkShaderModuleValidationCacheCreateInfoEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkShaderModuleValidationCacheCreateInfoEXT#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkShaderModuleValidationCacheCreateInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkShaderModuleValidationCacheCreateInfoEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkShaderModuleValidationCacheCreateInfoEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.SHADER_MODULE_VALIDATION_CACHE_CREATE_INFO_EXT);
        }
        return ret;
    }
    ...
    public static final PathElement PATH$validationCache = PathElement.groupElement("PATH$validationCache");
