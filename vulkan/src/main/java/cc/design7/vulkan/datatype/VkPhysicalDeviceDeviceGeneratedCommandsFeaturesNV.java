...
///
/// The {@link VkPhysicalDeviceDeviceGeneratedCommandsFeaturesNV#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceDeviceGeneratedCommandsFeaturesNV#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDeviceDeviceGeneratedCommandsFeaturesNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceDeviceGeneratedCommandsFeaturesNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceDeviceGeneratedCommandsFeaturesNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_DEVICE_GENERATED_COMMANDS_FEATURES_NV);
        }
        return ret;
    }
