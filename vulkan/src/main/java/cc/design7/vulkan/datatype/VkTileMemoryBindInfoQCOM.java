...
/// Represents a pointer to a {@code VkTileMemoryBindInfoQCOM} structure in native memory.
///
...
///
/// @see <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkTileMemoryBindInfoQCOM.html">VkTileMemoryBindInfoQCOM</a>
@ValueBasedCandidate
@UnsafeConstructor
public record VkTileMemoryBindInfoQCOM(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkTileMemoryBindInfoQCOM allocate(Arena arena) {
        return new VkTileMemoryBindInfoQCOM(arena.allocate(LAYOUT));
    }
    ...
    public static VkTileMemoryBindInfoQCOM[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkTileMemoryBindInfoQCOM(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
