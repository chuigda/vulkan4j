...
///
/// The {@link VkTextureLODGatherFormatPropertiesAMD#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkTextureLODGatherFormatPropertiesAMD#autoInit} to initialize these fields manually for
...
public record VkTextureLODGatherFormatPropertiesAMD(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkTextureLODGatherFormatPropertiesAMD[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkTextureLODGatherFormatPropertiesAMD(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.TEXTURE_LOD_GATHER_FORMAT_PROPERTIES_AMD);
        }
        return ret;
    }
