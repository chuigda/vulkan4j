...
///
/// The {@link VkRenderPassStripeInfoARM#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkRenderPassStripeInfoARM#autoInit} to initialize these fields manually for
...
public record VkRenderPassStripeInfoARM(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkRenderPassStripeInfoARM[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkRenderPassStripeInfoARM(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.RENDER_PASS_STRIPE_INFO_ARM);
        }
        return ret;
    }
