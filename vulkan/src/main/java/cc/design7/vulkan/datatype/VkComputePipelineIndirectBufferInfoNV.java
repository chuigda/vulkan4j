...
///
/// The {@link VkComputePipelineIndirectBufferInfoNV#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkComputePipelineIndirectBufferInfoNV#autoInit} to initialize these fields manually for
...
public record VkComputePipelineIndirectBufferInfoNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkComputePipelineIndirectBufferInfoNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkComputePipelineIndirectBufferInfoNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.COMPUTE_PIPELINE_INDIRECT_BUFFER_INFO_NV);
        }
        return ret;
    }
