...
/// @see <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkExternalBufferProperties.html">VkExternalBufferProperties</a>
@ValueBasedCandidate
public record VkExternalBufferProperties(@NotNull MemorySegment segment) implements IPointer {
    public static final OfInt LAYOUT$sType = ValueLayout.JAVA_INT.withName("sType");
    public static final AddressLayout LAYOUT$pNext = ValueLayout.ADDRESS.withName("pNext");
    public static final StructLayout LAYOUT$externalMemoryProperties = VkExternalMemoryProperties.LAYOUT.withName("externalMemoryProperties");

    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$externalMemoryProperties);
    public static final long SIZE = LAYOUT.byteSize();

    public static VkExternalBufferProperties allocate(Arena arena) {
        return new VkExternalBufferProperties(arena.allocate(LAYOUT));
    }
    ...
    public static VkExternalBufferProperties[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkExternalBufferProperties(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
