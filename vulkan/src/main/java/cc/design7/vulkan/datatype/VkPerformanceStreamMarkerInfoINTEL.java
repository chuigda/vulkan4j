...
///
/// The {@link VkPerformanceStreamMarkerInfoINTEL#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPerformanceStreamMarkerInfoINTEL#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPerformanceStreamMarkerInfoINTEL(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPerformanceStreamMarkerInfoINTEL[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPerformanceStreamMarkerInfoINTEL(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PERFORMANCE_STREAM_MARKER_INFO_INTEL);
        }
        return ret;
    }
    ...
    public static VkPerformanceStreamMarkerInfoINTEL[] clone(Arena arena, VkPerformanceStreamMarkerInfoINTEL[] src) {
        VkPerformanceStreamMarkerInfoINTEL[] ret = allocate(arena, src.length);
