...
public record VkRenderPassTileShadingCreateInfoQCOM(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static final StructLayout LAYOUT$tileApronSize = VkExtent2D.LAYOUT.withName("tileApronSize");

    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$flags, LAYOUT$tileApronSize);
    ...
    public static VkRenderPassTileShadingCreateInfoQCOM[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkRenderPassTileShadingCreateInfoQCOM(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
