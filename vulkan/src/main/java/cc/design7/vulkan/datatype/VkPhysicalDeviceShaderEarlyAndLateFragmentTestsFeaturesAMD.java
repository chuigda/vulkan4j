...
///
/// The {@link VkPhysicalDeviceShaderEarlyAndLateFragmentTestsFeaturesAMD#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceShaderEarlyAndLateFragmentTestsFeaturesAMD#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceShaderEarlyAndLateFragmentTestsFeaturesAMD(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceShaderEarlyAndLateFragmentTestsFeaturesAMD[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceShaderEarlyAndLateFragmentTestsFeaturesAMD(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SHADER_EARLY_AND_LATE_FRAGMENT_TESTS_FEATURES_AMD);
        }
        return ret;
    }
