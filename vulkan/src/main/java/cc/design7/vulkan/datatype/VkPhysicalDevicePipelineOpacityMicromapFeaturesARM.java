...
public record VkPhysicalDevicePipelineOpacityMicromapFeaturesARM(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static final OfInt LAYOUT$pipelineOpacityMicromap = ValueLayout.JAVA_INT.withName("pipelineOpacityMicromap");

    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$pipelineOpacityMicromap);
    ...
    public static VkPhysicalDevicePipelineOpacityMicromapFeaturesARM[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDevicePipelineOpacityMicromapFeaturesARM(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
