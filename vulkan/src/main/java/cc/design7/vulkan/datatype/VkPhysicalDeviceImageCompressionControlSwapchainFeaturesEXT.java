...
///
/// The {@link VkPhysicalDeviceImageCompressionControlSwapchainFeaturesEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceImageCompressionControlSwapchainFeaturesEXT#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDeviceImageCompressionControlSwapchainFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceImageCompressionControlSwapchainFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceImageCompressionControlSwapchainFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_IMAGE_COMPRESSION_CONTROL_SWAPCHAIN_FEATURES_EXT);
        }
        return ret;
    }
    ...
    public static final PathElement PATH$imageCompressionControlSwapchain = PathElement.groupElement("PATH$imageCompressionControlSwapchain");
