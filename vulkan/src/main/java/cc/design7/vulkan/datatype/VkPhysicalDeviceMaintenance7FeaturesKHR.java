...
///
/// The {@link VkPhysicalDeviceMaintenance7FeaturesKHR#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceMaintenance7FeaturesKHR#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceMaintenance7FeaturesKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceMaintenance7FeaturesKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceMaintenance7FeaturesKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_MAINTENANCE_7_FEATURES_KHR);
        }
        return ret;
    }
