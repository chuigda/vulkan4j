...
///
/// The {@link VkPhysicalDeviceDepthClampControlFeaturesEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceDepthClampControlFeaturesEXT#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceDepthClampControlFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceDepthClampControlFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceDepthClampControlFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_DEPTH_CLAMP_CONTROL_FEATURES_EXT);
        }
        return ret;
    }
