...
///
/// The {@link VkExportFenceSciSyncInfoNV#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkExportFenceSciSyncInfoNV#autoInit} to initialize these fields manually for
...
public record VkExportFenceSciSyncInfoNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkExportFenceSciSyncInfoNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkExportFenceSciSyncInfoNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.EXPORT_FENCE_SCI_SYNC_INFO_NV);
        }
        return ret;
    }
