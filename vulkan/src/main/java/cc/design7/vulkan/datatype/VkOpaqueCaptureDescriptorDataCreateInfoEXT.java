...
///
/// The {@link VkOpaqueCaptureDescriptorDataCreateInfoEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkOpaqueCaptureDescriptorDataCreateInfoEXT#autoInit} to initialize these fields manually for
...
public record VkOpaqueCaptureDescriptorDataCreateInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkOpaqueCaptureDescriptorDataCreateInfoEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkOpaqueCaptureDescriptorDataCreateInfoEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.OPAQUE_CAPTURE_DESCRIPTOR_DATA_CREATE_INFO_EXT);
        }
        return ret;
    }
