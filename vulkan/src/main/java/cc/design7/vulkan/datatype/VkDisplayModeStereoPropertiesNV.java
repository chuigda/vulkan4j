...
public record VkDisplayModeStereoPropertiesNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static final OfInt LAYOUT$hdmi3DSupported = ValueLayout.JAVA_INT.withName("hdmi3DSupported");

    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$hdmi3DSupported);
    ...
    public static VkDisplayModeStereoPropertiesNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkDisplayModeStereoPropertiesNV(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
