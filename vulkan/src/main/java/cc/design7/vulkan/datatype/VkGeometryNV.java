...
///
/// The {@link VkGeometryNV#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkGeometryNV#autoInit} to initialize these fields manually for
...
public record VkGeometryNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkGeometryNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkGeometryNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.GEOMETRY_NV);
        }
        return ret;
    }
