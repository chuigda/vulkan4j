...
///
/// The {@link VkPhysicalDeviceExternalBufferInfo#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceExternalBufferInfo#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceExternalBufferInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceExternalBufferInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceExternalBufferInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_EXTERNAL_BUFFER_INFO);
        }
        return ret;
    }
