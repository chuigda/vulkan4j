...
///
/// The {@link VkSurfacePresentModeCompatibilityEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkSurfacePresentModeCompatibilityEXT#autoInit} to initialize these fields manually for
...
public record VkSurfacePresentModeCompatibilityEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkSurfacePresentModeCompatibilityEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkSurfacePresentModeCompatibilityEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.SURFACE_PRESENT_MODE_COMPATIBILITY_EXT);
        }
        return ret;
    }
