...
///
/// The {@link VkCudaFunctionCreateInfoNV#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkCudaFunctionCreateInfoNV#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkCudaFunctionCreateInfoNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkCudaFunctionCreateInfoNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkCudaFunctionCreateInfoNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.CUDA_FUNCTION_CREATE_INFO_NV);
        }
        return ret;
    }
    ...
    public static VkCudaFunctionCreateInfoNV[] clone(Arena arena, VkCudaFunctionCreateInfoNV[] src) {
        VkCudaFunctionCreateInfoNV[] ret = allocate(arena, src.length);
