...
///
/// The {@link VkAttachmentReferenceStencilLayout#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkAttachmentReferenceStencilLayout#autoInit} to initialize these fields manually for
...
public record VkAttachmentReferenceStencilLayout(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkAttachmentReferenceStencilLayout[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkAttachmentReferenceStencilLayout(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.ATTACHMENT_REFERENCE_STENCIL_LAYOUT);
        }
        return ret;
    }
