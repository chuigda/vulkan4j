...
///
/// The {@link VkPerformanceQueryReservationInfoKHR#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPerformanceQueryReservationInfoKHR#autoInit} to initialize these fields manually for
...
public record VkPerformanceQueryReservationInfoKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPerformanceQueryReservationInfoKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPerformanceQueryReservationInfoKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PERFORMANCE_QUERY_RESERVATION_INFO_KHR);
        }
        return ret;
    }
