...
public record VkVideoEncodeH265DpbSlotInfoKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static final AddressLayout LAYOUT$pStdReferenceInfo = ValueLayout.ADDRESS.withTargetLayout(StdVideoEncodeH265ReferenceInfo.LAYOUT).withName("pStdReferenceInfo");
    ...
    public static VkVideoEncodeH265DpbSlotInfoKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkVideoEncodeH265DpbSlotInfoKHR(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
