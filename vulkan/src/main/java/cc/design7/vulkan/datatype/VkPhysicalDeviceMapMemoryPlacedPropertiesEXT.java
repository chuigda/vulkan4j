...
///
/// The {@link VkPhysicalDeviceMapMemoryPlacedPropertiesEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceMapMemoryPlacedPropertiesEXT#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceMapMemoryPlacedPropertiesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceMapMemoryPlacedPropertiesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceMapMemoryPlacedPropertiesEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_MAP_MEMORY_PLACED_PROPERTIES_EXT);
        }
        return ret;
    }
