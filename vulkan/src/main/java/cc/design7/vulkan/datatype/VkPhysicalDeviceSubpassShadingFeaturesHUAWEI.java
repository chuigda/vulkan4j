...
///
/// The {@link VkPhysicalDeviceSubpassShadingFeaturesHUAWEI#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceSubpassShadingFeaturesHUAWEI#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceSubpassShadingFeaturesHUAWEI(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceSubpassShadingFeaturesHUAWEI[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceSubpassShadingFeaturesHUAWEI(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SUBPASS_SHADING_FEATURES_HUAWEI);
        }
        return ret;
    }
