...
///
/// The {@link VkRenderingFragmentDensityMapAttachmentInfoEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkRenderingFragmentDensityMapAttachmentInfoEXT#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkRenderingFragmentDensityMapAttachmentInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkRenderingFragmentDensityMapAttachmentInfoEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkRenderingFragmentDensityMapAttachmentInfoEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.RENDERING_FRAGMENT_DENSITY_MAP_ATTACHMENT_INFO_EXT);
        }
        return ret;
    }
