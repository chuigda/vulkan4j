...
///
/// The {@link VkBindBufferMemoryDeviceGroupInfo#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkBindBufferMemoryDeviceGroupInfo#autoInit} to initialize these fields manually for
...
public record VkBindBufferMemoryDeviceGroupInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkBindBufferMemoryDeviceGroupInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkBindBufferMemoryDeviceGroupInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.BIND_BUFFER_MEMORY_DEVICE_GROUP_INFO);
        }
        return ret;
    }
