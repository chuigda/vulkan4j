...
///
/// The {@link VkPhysicalDeviceComputeShaderDerivativesPropertiesKHR#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceComputeShaderDerivativesPropertiesKHR#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceComputeShaderDerivativesPropertiesKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceComputeShaderDerivativesPropertiesKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceComputeShaderDerivativesPropertiesKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_COMPUTE_SHADER_DERIVATIVES_PROPERTIES_KHR);
        }
        return ret;
    }
