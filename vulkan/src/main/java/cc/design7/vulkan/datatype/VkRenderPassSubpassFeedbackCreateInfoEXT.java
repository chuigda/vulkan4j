...
public record VkRenderPassSubpassFeedbackCreateInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static final AddressLayout LAYOUT$pSubpassFeedback = ValueLayout.ADDRESS.withTargetLayout(VkRenderPassSubpassFeedbackInfoEXT.LAYOUT).withName("pSubpassFeedback");

    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$pSubpassFeedback);
    ...
    @unsafe public @Nullable VkRenderPassSubpassFeedbackInfoEXT[] pSubpassFeedback(int assumedCount) {
        ...
        s = s.reinterpret(assumedCount * VkRenderPassSubpassFeedbackInfoEXT.SIZE);
        VkRenderPassSubpassFeedbackInfoEXT[] ret = new VkRenderPassSubpassFeedbackInfoEXT[assumedCount];
        for (int i = 0; i < assumedCount; i ++) {
            ret[i] = new VkRenderPassSubpassFeedbackInfoEXT(s.asSlice(i * VkRenderPassSubpassFeedbackInfoEXT.SIZE, VkRenderPassSubpassFeedbackInfoEXT.SIZE));
        }
        return ret;
    }
