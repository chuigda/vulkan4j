...
///
/// The {@link VkPhysicalDeviceLayeredDriverPropertiesMSFT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceLayeredDriverPropertiesMSFT#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkPhysicalDeviceLayeredDriverPropertiesMSFT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceLayeredDriverPropertiesMSFT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceLayeredDriverPropertiesMSFT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_LAYERED_DRIVER_PROPERTIES_MSFT);
        }
        return ret;
    }
    ...
    public static final PathElement PATH$underlyingAPI = PathElement.groupElement("PATH$underlyingAPI");
