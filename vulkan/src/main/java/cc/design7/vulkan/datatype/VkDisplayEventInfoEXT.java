...
/// @see <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkDisplayEventInfoEXT.html">VkDisplayEventInfoEXT</a>
@ValueBasedCandidate
public record VkDisplayEventInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    public static final OfInt LAYOUT$sType = ValueLayout.JAVA_INT.withName("sType");
    public static final AddressLayout LAYOUT$pNext = ValueLayout.ADDRESS.withName("pNext");
    public static final OfInt LAYOUT$displayEvent = ValueLayout.JAVA_INT.withName("displayEvent");

    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$displayEvent);
    public static final long SIZE = LAYOUT.byteSize();

    public static VkDisplayEventInfoEXT allocate(Arena arena) {
        return new VkDisplayEventInfoEXT(arena.allocate(LAYOUT));
    }
    ...
    public static VkDisplayEventInfoEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkDisplayEventInfoEXT(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
