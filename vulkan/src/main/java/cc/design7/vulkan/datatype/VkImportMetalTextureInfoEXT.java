...
///
/// The {@link VkImportMetalTextureInfoEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkImportMetalTextureInfoEXT#autoInit} to initialize these fields manually for
...
public record VkImportMetalTextureInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkImportMetalTextureInfoEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkImportMetalTextureInfoEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.IMPORT_METAL_TEXTURE_INFO_EXT);
        }
        return ret;
    }
