...
/// @see <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkCopyImageToImageInfo.html">VkCopyImageToImageInfo</a>
@ValueBasedCandidate
public record VkCopyImageToImageInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$flags, LAYOUT$srcImage, LAYOUT$srcImageLayout, LAYOUT$dstImage, LAYOUT$dstImageLayout, LAYOUT$regionCount, LAYOUT$pRegions);
    public static final long SIZE = LAYOUT.byteSize();

    public static VkCopyImageToImageInfo allocate(Arena arena) {
        return new VkCopyImageToImageInfo(arena.allocate(LAYOUT));
    }

    public static VkCopyImageToImageInfo[] allocate(Arena arena, int count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkCopyImageToImageInfo[] ret = new VkCopyImageToImageInfo[count];
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkCopyImageToImageInfo(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
    ...
    public static VkCopyImageToImageInfo[] clone(Arena arena, VkCopyImageToImageInfo[] src) {
        VkCopyImageToImageInfo[] ret = allocate(arena, src.length);
