...
///
/// The {@link VkDeviceQueueGlobalPriorityCreateInfo#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkDeviceQueueGlobalPriorityCreateInfo#autoInit} to initialize these fields manually for
...
public record VkDeviceQueueGlobalPriorityCreateInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkDeviceQueueGlobalPriorityCreateInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkDeviceQueueGlobalPriorityCreateInfo(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.DEVICE_QUEUE_GLOBAL_PRIORITY_CREATE_INFO);
        }
        return ret;
    }
