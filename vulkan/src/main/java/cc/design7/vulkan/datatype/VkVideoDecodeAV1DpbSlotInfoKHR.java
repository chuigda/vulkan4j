...
///
/// The {@link VkVideoDecodeAV1DpbSlotInfoKHR#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkVideoDecodeAV1DpbSlotInfoKHR#autoInit} to initialize these fields manually for
...
public record VkVideoDecodeAV1DpbSlotInfoKHR(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkVideoDecodeAV1DpbSlotInfoKHR[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkVideoDecodeAV1DpbSlotInfoKHR(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.VIDEO_DECODE_AV1_DPB_SLOT_INFO_KHR);
        }
        return ret;
    }
    ...
    @unsafe public @Nullable StdVideoDecodeAV1ReferenceInfo[] pStdReferenceInfo(int assumedCount) {
        ...
        for (int i = 0; i < assumedCount; i ++) {
            ret[i] = new StdVideoDecodeAV1ReferenceInfo(s.asSlice(i * StdVideoDecodeAV1ReferenceInfo.BYTES, StdVideoDecodeAV1ReferenceInfo.BYTES));
        }
        return ret;
    }
