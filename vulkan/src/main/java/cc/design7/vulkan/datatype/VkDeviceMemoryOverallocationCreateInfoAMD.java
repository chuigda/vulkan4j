...
///
/// The {@link VkDeviceMemoryOverallocationCreateInfoAMD#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkDeviceMemoryOverallocationCreateInfoAMD#autoInit} to initialize these fields manually for
...
public record VkDeviceMemoryOverallocationCreateInfoAMD(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkDeviceMemoryOverallocationCreateInfoAMD[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkDeviceMemoryOverallocationCreateInfoAMD(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.DEVICE_MEMORY_OVERALLOCATION_CREATE_INFO_AMD);
        }
        return ret;
    }
