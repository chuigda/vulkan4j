...
/// @see <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkPipelineRobustnessCreateInfo.html">VkPipelineRobustnessCreateInfo</a>
@ValueBasedCandidate
public record VkPipelineRobustnessCreateInfo(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static final OfInt LAYOUT$storageBuffers = ValueLayout.JAVA_INT.withName("storageBuffers");
    public static final OfInt LAYOUT$uniformBuffers = ValueLayout.JAVA_INT.withName("uniformBuffers");
    public static final OfInt LAYOUT$vertexInputs = ValueLayout.JAVA_INT.withName("vertexInputs");
    public static final OfInt LAYOUT$images = ValueLayout.JAVA_INT.withName("images");

    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$storageBuffers, LAYOUT$uniformBuffers, LAYOUT$vertexInputs, LAYOUT$images);
    public static final long SIZE = LAYOUT.byteSize();

    public static VkPipelineRobustnessCreateInfo allocate(Arena arena) {
        return new VkPipelineRobustnessCreateInfo(arena.allocate(LAYOUT));
    }
    ...
    public static VkPipelineRobustnessCreateInfo[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPipelineRobustnessCreateInfo(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
