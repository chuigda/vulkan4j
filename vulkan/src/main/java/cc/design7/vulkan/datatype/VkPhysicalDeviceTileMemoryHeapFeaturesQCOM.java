...
/// @see <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkPhysicalDeviceTileMemoryHeapFeaturesQCOM.html">VkPhysicalDeviceTileMemoryHeapFeaturesQCOM</a>
@ValueBasedCandidate
public record VkPhysicalDeviceTileMemoryHeapFeaturesQCOM(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static final OfInt LAYOUT$tileMemoryHeap = ValueLayout.JAVA_INT.withName("tileMemoryHeap");

    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$tileMemoryHeap);
    public static final long SIZE = LAYOUT.byteSize();

    public static VkPhysicalDeviceTileMemoryHeapFeaturesQCOM allocate(Arena arena) {
        return new VkPhysicalDeviceTileMemoryHeapFeaturesQCOM(arena.allocate(LAYOUT));
    }
    ...
    public static VkPhysicalDeviceTileMemoryHeapFeaturesQCOM[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceTileMemoryHeapFeaturesQCOM(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
