...
///
/// The {@link VkPhysicalDeviceMemoryDecompressionPropertiesNV#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceMemoryDecompressionPropertiesNV#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceMemoryDecompressionPropertiesNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceMemoryDecompressionPropertiesNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceMemoryDecompressionPropertiesNV(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_MEMORY_DECOMPRESSION_PROPERTIES_NV);
        }
        return ret;
    }
