...
/// @see <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkPhysicalDevicePartitionedAccelerationStructurePropertiesNV.html">VkPhysicalDevicePartitionedAccelerationStructurePropertiesNV</a>
@ValueBasedCandidate
public record VkPhysicalDevicePartitionedAccelerationStructurePropertiesNV(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static final OfInt LAYOUT$maxPartitionCount = ValueLayout.JAVA_INT.withName("maxPartitionCount");

    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$maxPartitionCount);
    public static final long SIZE = LAYOUT.byteSize();

    public static VkPhysicalDevicePartitionedAccelerationStructurePropertiesNV allocate(Arena arena) {
        return new VkPhysicalDevicePartitionedAccelerationStructurePropertiesNV(arena.allocate(LAYOUT));
    }
    ...
    public static VkPhysicalDevicePartitionedAccelerationStructurePropertiesNV[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDevicePartitionedAccelerationStructurePropertiesNV(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
    ...
    public static final PathElement PATH$maxPartitionCount = PathElement.groupElement("PATH$maxPartitionCount");
