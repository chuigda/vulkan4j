...
///
/// The {@link VkExportMetalObjectsInfoEXT#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkExportMetalObjectsInfoEXT#autoInit} to initialize these fields manually for
...
@ValueBasedCandidate
@UnsafeConstructor
public record VkExportMetalObjectsInfoEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkExportMetalObjectsInfoEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkExportMetalObjectsInfoEXT(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.EXPORT_METAL_OBJECTS_INFO_EXT);
        }
        return ret;
    }
