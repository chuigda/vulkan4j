...
///
/// The {@link VkPhysicalDeviceProtectedMemoryFeatures#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceProtectedMemoryFeatures#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceProtectedMemoryFeatures(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceProtectedMemoryFeatures[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceProtectedMemoryFeatures(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_PROTECTED_MEMORY_FEATURES);
        }
        return ret;
    }
