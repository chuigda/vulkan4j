...
/// Represents a pointer to a {@code VkImageResolve} structure in native memory.
///
...
///
/// @see <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkImageResolve.html">VkImageResolve</a>
@ValueBasedCandidate
@UnsafeConstructor
public record VkImageResolve(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkImageResolve[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkImageResolve(segment.asSlice(i * BYTES, BYTES));
        }
        return ret;
    }
    ...
    public static VkImageResolve[] clone(Arena arena, VkImageResolve[] src) {
        VkImageResolve[] ret = allocate(arena, src.length);
