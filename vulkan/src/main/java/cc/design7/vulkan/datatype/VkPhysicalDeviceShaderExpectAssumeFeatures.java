...
/// @see <a href="https://registry.khronos.org/vulkan/specs/latest/man/html/VkPhysicalDeviceShaderExpectAssumeFeatures.html">VkPhysicalDeviceShaderExpectAssumeFeatures</a>
@ValueBasedCandidate
public record VkPhysicalDeviceShaderExpectAssumeFeatures(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static final MemoryLayout LAYOUT = NativeLayout.structLayout(LAYOUT$sType, LAYOUT$pNext, LAYOUT$shaderExpectAssume);
    public static final long SIZE = LAYOUT.byteSize();

    public static VkPhysicalDeviceShaderExpectAssumeFeatures allocate(Arena arena) {
        return new VkPhysicalDeviceShaderExpectAssumeFeatures(arena.allocate(LAYOUT));
    }

    public static VkPhysicalDeviceShaderExpectAssumeFeatures[] allocate(Arena arena, int count) {
        MemorySegment segment = arena.allocate(LAYOUT, count);
        VkPhysicalDeviceShaderExpectAssumeFeatures[] ret = new VkPhysicalDeviceShaderExpectAssumeFeatures[count];
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceShaderExpectAssumeFeatures(segment.asSlice(i * SIZE, SIZE));
        }
        return ret;
    }
    ...
    public static VkPhysicalDeviceShaderExpectAssumeFeatures[] clone(Arena arena, VkPhysicalDeviceShaderExpectAssumeFeatures[] src) {
        VkPhysicalDeviceShaderExpectAssumeFeatures[] ret = allocate(arena, src.length);
