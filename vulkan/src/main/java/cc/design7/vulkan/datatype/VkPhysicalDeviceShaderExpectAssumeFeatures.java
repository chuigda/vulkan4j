...
/// Represents a pointer to a {@code VkPhysicalDeviceShaderExpectAssumeFeatures} structure in native memory.
///
...
public record VkPhysicalDeviceShaderExpectAssumeFeatures(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceShaderExpectAssumeFeatures[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceShaderExpectAssumeFeatures(segment.asSlice(i * BYTES, BYTES));
        }
        return ret;
    }
