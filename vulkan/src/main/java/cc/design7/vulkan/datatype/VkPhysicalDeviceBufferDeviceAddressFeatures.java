...
///
/// The {@link VkPhysicalDeviceBufferDeviceAddressFeatures#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceBufferDeviceAddressFeatures#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceBufferDeviceAddressFeatures(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceBufferDeviceAddressFeatures[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceBufferDeviceAddressFeatures(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_BUFFER_DEVICE_ADDRESS_FEATURES);
        }
        return ret;
    }
