...
///
/// The {@link VkPhysicalDeviceHostQueryResetFeatures#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceHostQueryResetFeatures#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceHostQueryResetFeatures(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceHostQueryResetFeatures[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceHostQueryResetFeatures(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_HOST_QUERY_RESET_FEATURES);
        }
        return ret;
    }
