...
/// Represents a pointer to a {@code VkPhysicalDeviceDepthClipEnableFeaturesEXT} structure in native memory.
///
...
public record VkPhysicalDeviceDepthClipEnableFeaturesEXT(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceDepthClipEnableFeaturesEXT[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceDepthClipEnableFeaturesEXT(segment.asSlice(i * BYTES, BYTES));
        }
        return ret;
    }
