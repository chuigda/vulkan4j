...
///
/// The {@link VkPhysicalDeviceShaderIntegerDotProductFeatures#allocate} functions will automatically initialize these fields.
/// Also, you may call {@link VkPhysicalDeviceShaderIntegerDotProductFeatures#autoInit} to initialize these fields manually for
...
public record VkPhysicalDeviceShaderIntegerDotProductFeatures(@NotNull MemorySegment segment) implements IPointer {
    ...
    public static VkPhysicalDeviceShaderIntegerDotProductFeatures[] allocate(Arena arena, int count) {
        ...
        for (int i = 0; i < count; i ++) {
            ret[i] = new VkPhysicalDeviceShaderIntegerDotProductFeatures(segment.asSlice(i * BYTES, BYTES));
            ret[i].sType(VkStructureType.PHYSICAL_DEVICE_SHADER_INTEGER_DOT_PRODUCT_FEATURES);
        }
        return ret;
    }
